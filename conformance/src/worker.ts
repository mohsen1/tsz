  ...
  if (useWasm) {
    ...
    return new Promise((resolve) => {
      ...
      try {
        ...
        if (opts.strictpropertyinitialization !== undefined) args.push(opts.strictpropertyinitialization ? '--strictPropertyInitialization' : '--strictPropertyInitialization=false');
        ...
        if (opts.noimplicitthis !== undefined) args.push(opts.noimplicitthis ? '--noImplicitThis' : '--noImplicitThis=false');
        if (opts.useunknownincatchvariables !== undefined) args.push(opts.useunknownincatchvariables ? '--useUnknownInCatchVariables' : '--useUnknownInCatchVariables=false');
