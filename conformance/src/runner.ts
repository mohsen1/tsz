...
function logProgress(current: number, total: number, extra = ''): void {
 ...
 */
async function runTsc(testCase: ParsedTestCase, libSource: string): Promise<TestResult> {
...
async function runWasm(
  ...
  try {
    const wasm = wasmModule as {
      ThinParser: new (name: string, code: string) => ThinParserInstance;
      WasmProgram: new () => WasmProgramInstance;
    };
    ...
    if (testCase.isMultiFile || testCase.files.length > 1) {
      ...
      return {
        diagnostics: Array.from(codes as number[]).map((code: number) => ({
          code,
          message: '',
          category: 'Error',
        })),
      ...
      const diagnostics = [
        ...parseDiags.map((d: { code: number; message: string }) => ({
          code: d.code,
          message: d.message,
          category: 'Error',
        })),
        ...(checkResult.diagnostics || []).map((d: { code: number; message_text: string; category: string }) => ({
          code: d.code,
...
// Type definitions for WASM instances
interface ThinParserInstance {
 ...
 */
function printReport(stats: TestStats, verbose: boolean): void {
