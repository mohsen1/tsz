#!/usr/bin/env node
/**
 * Parallel Conformance Test Runner - Uses worker threads for ~14x speedup
 */
...
async function runWorker(testFiles, workerId) {
  return new Promise((resolve, reject) => {
    const worker = new Worker(join(__dirname, 'conformance-worker.mjs'), {
      workerData: {
        wasmPkgPath: CONFIG.wasmPkgPath,
        conformanceDir: CONFIG.conformanceDir,
        testFiles,
      },
    });
...
async function main() {
  ...
  log('Parallel Conformance Test Runner', colors.bold);
  log('═'.repeat(60), colors.dim);
  log(`  Workers: ${numWorkers}`, colors.cyan);
  ...
  log(`  Distributing across ${chunks.length} workers (~${Math.ceil(testFiles.length / numWorkers)} tests each)`, colors.dim);

  log(`\nRunning tests in parallel...`, colors.cyan);
