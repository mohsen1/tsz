...
- **#1145 CLOSED** (2026-04-25) — Phase 2 step 5: `global_file_locals_index` migration introduced a CONFORMANCE REGRESSION (6 shards failing on retrigger — not flaky). Likely cause: `binder.file_locals` semantics differ from other migrated indexes — possibly name shadowing or SymbolId-binder coupling that the skeleton projection doesn't capture. Branch preserved at `phase2-step5-file-locals-index` for root-cause investigation. **DO NOT REOPEN without finding the regression's root cause first.**
