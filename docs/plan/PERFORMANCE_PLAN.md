...
| Typed cross-file query migration | **Promoted — highest Tier 2 priority** | 2026-05-13 post-#5863 attribution confirmed `delegate.cache_hits_cross_file = 0` was load-bearing because the hot `DelegateCrossArenaSymbol` path did not reach the canonical readers. On monorepo-006, `DelegateCrossArenaSymbol` constructed 924 child checkers; all 924 misses came from `symbol_arenas`, with 883 targeting source files. **Current PR in flight:** route proven single-arena source-file `symbol_arenas` targets through the canonical `SymbolType` bucket. An unguarded prototype moved `delegate.cache_hits_cross_file` to 632 and dropped `DelegateCrossArenaSymbol` construction to 292 on monorepo-006, but conformance showed the cache must exclude merged/augmented symbols. Re-measure the guarded version after CI. Decision record: [`perf-runs/2026-05-13-post-5863-attribution.md`](perf-runs/2026-05-13-post-5863-attribution.md). |
...
routes proven single-arena declarations through the canonical `SymbolType`
bucket. A broader prototype moved `delegate.cache_hits_cross_file` from 0
to 632 and reduced `DelegateCrossArenaSymbol` child checkers from 924 to
292 on monorepo-006, but it regressed conformance by caching merged or
augmented symbols. The guarded version should be re-measured before using
those counters as merge evidence.
