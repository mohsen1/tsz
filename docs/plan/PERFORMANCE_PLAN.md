...
**2026-05-13 utility-alias follow-up:** after preserving
`lib_delegation_cache` type-parameter metadata, the conformance-safe alias slice
admits only the measured actual-lib `FlatArray` alias with the indexed
type-literal plus conditional-index body shape. On monorepo-006 it drops
`DelegateCrossArenaSymbol` from 28 to 27,
`checker.with_parent_cache_constructed` from 31 to 30, and `delegate.misses`
from 30 to 29 with unchanged diagnostic count. Shape-only routing, including
`IteratorResult` and `Record`, stays on fallback after hosted conformance
regressed the broader exploratory branch. The remaining rows show that the
bigger solution should be a dedicated
actual-lib type-alias body query or canonical `DefinitionStore` prepopulation,
not a broader name gate. Decision record:
[`perf-runs/2026-05-13-delegate-lib-utility-aliases.md`](perf-runs/2026-05-13-delegate-lib-utility-aliases.md).
