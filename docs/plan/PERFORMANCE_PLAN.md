...
**2026-05-14 simple-object missing-interface lib attribution:** the follow-up
classifies the conformance-safe non-iterator `reject_missing_interface_decl`
rows as actual/cloned lib-backed in attribution when perf counters are enabled,
while keeping semantic type computation on the existing full merge path and
leaving normal checker execution on the existing control flow. On regenerated
monorepo-006, diagnostics remain `10,198`, simple-object hits and successes
remain `24,762`, `reject_missing_interface_decl` drops from `7` to `4`, and the
remaining declaration/provenance residue is the iterator-family
...
**2026-05-14 parallel default-on follow-up:** the parallel no-emit lane now
defaults to chunked file-session reuse with the same global opt-out
(`TSZ_DISABLE_FILE_SESSION_REUSE=1`). On synthetic no-wildcard fixtures,
diagnostics remain identical between default and disabled modes (`40` and
`400` respectively), while constructor counters shift strongly toward reuse
(`state_constructed 41 -> 6` on 40 files; `401 -> 51` on 400 files). Timing is
mixed on the small fixture and favorable on the larger fixture in single runs,
so treat this as constructor/counter reduction evidence rather than a universal
single-run timing claim. Decision record:
[`perf-runs/2026-05-14-parallel-file-session-reuse-default.md`](perf-runs/2026-05-14-parallel-file-session-reuse-default.md).
Claim:
[`claims/perf-parallel-file-session-reuse-default-2026-05-14.md`](claims/perf-parallel-file-session-reuse-default-2026-05-14.md).
