...
**2026-05-13 utility-alias update:** the narrow `FlatArray` /
`IteratorResult` / `Record` actual-lib alias slice removes one child-checker
construction per alias on monorepo-006, dropping `DelegateCrossArenaSymbol`
from 30 to 27, `checker.with_parent_cache_constructed` from 39 to 36, and
`delegate.misses` from 38 to 35 with unchanged diagnostic count. The remaining
one-per-name rows show that the bigger solution should be a dedicated
