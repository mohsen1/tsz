...
| Fresh phase split | Done (refreshed 2026-05-13) | See `docs/plan/perf-runs/2026-05-13-post-6111-attribution.md`. Post-#6111 attribution keeps the cliff checker-dominated (monorepo-003..006: check ≈ 97-98 % in attribution mode). `large-ts-repo` remains deferred (previous OOM / stack-overflow blocker); re-measure after the first T2.2/T2.1.D code PR removes a measured child-checker path. |
...
| Typed cross-file query migration | **Promoted — highest Tier 2 priority** | #6111 landed the first `DelegateCrossArenaSymbol` source-file symbol-arena gateway path. The post-merge attribution refresh shows the final requester-scoped key makes those misses observable as `cross_file_cache_miss_causes.bucket_empty` (343 on monorepo-006), but does not produce batch `delegate.cache_hits_cross_file` hits. `TypeEnvironmentCore` remains the dominant child-checker reason (5,259 on monorepo-006 vs. 924 `DelegateCrossArenaSymbol`), so the next active lane is the TypeEnvironmentCore arena-direct slice (#6144). Decision record: [`perf-runs/2026-05-13-post-6111-attribution.md`](perf-runs/2026-05-13-post-6111-attribution.md). |
