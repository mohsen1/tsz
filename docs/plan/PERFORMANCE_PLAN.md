...
admits only actual-lib aliases with the indexed type-literal plus
conditional-index body shape. On monorepo-006 it drops
`DelegateCrossArenaSymbol` from 28 to 27,
`checker.with_parent_cache_constructed` from 31 to 30, and `delegate.misses`
from 30 to 29 with unchanged diagnostic count. `IteratorResult` and `Record`
stay on fallback after hosted conformance regressed the broader exploratory
branch. The remaining rows show that the bigger solution should be a dedicated
