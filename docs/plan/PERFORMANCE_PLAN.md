...
proven interface slice of the remaining declaration-file residue through the
existing lib resolver instead of constructing a child checker. On monorepo-006
this drops `DelegateCrossArenaSymbol` from 41 to 28,
`checker.with_parent_cache_constructed` from 56 to 37, and `delegate.misses`
from 55 to 36 with unchanged diagnostic count. The next implementation target
is the 28 remaining declaration-file misses: 16 type aliases plus 12 interfaces
that need namespace-qualified or merged-lib proof before admission.
