...
**2026-05-13 core value-interface allowlist update:** on top of the merged
declaration actual-lib direct path (`8427e11638` baseline), admitting a small
value-bearing interface set (`Function`, `Iterator`, `Locale`, `Object`,
`RegExp`) drops monorepo-006 `DelegateCrossArenaSymbol` from 26 to 22,
`checker.with_parent_cache_constructed` from 26 to 22, and `delegate.misses`
from 26 to 22 with unchanged diagnostics (`10,198`). The declaration-file
residue is now 22 misses: 15 type aliases plus 7 interfaces. Decision record:
