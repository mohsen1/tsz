...
routes through the canonical `SymbolType` bucket. Local monorepo-006
attribution moved `delegate.cache_hits_cross_file` from 0 to 632 and
reduced `DelegateCrossArenaSymbol` child checkers from 924 to 292. The
remaining source-file misses now report as `bucket_empty`, so the
miss-cause table is actionable for the next PR.
