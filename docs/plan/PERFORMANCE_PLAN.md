...
**2026-05-14 Iterator proof-bypass follow-up (main-based):** a narrow
iterator-family follow-up now routes `Iterator`/`IteratorObject` through the
existing parameterized direct-lib path, adds an `Iterator` fallback to
`resolve_lib_interface_type_by_symbol`, and permits a bounded declaration-proof
bypass for `Iterator` under existing actual-lib provenance checks. On
monorepo-006 (latest `main` baseline) this drops
`DelegateCrossArenaSymbol` children from 22 to 19, `delegate.misses` from 24
to 19, and `checker.with_parent_cache_constructed` from 25 to 19 with unchanged
diagnostics (`10,198`). Declaration-file residue rows removed:
`Iterator`, `IteratorObject`, `Symbol`. Decision record:
[`perf-runs/2026-05-14-delegate-actual-lib-iterator-proof-bypass-main.md`](perf-runs/2026-05-14-delegate-actual-lib-iterator-proof-bypass-main.md).
Claim:
[`claims/perf-delegate-actual-lib-iterator-proof-bypass-main-2026-05-14.md`](claims/perf-delegate-actual-lib-iterator-proof-bypass-main-2026-05-14.md).
