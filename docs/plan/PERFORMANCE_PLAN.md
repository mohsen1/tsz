...
| Fresh phase split | Done (refreshed 2026-05-13) | See `docs/plan/perf-runs/2026-05-13-post-alias-mapped-attribution.md`. Post-#6449 attribution across monorepo-001..006 still shows checker-dominant cliff behavior (check ≈ 97-98 % on monorepo-003..006), but with `delegate.misses = 0` and `with_parent_cache_constructed = 0` on all fixtures. `large-ts-repo` remains deferred (previous OOM / stack-overflow blocker). |
...
| Checker lifetime split | **Promoted** | T0.4 measured `with_parent_cache_constructed = 1.28 × files` on monorepo-006. The 2026-05-11 attribution run, post-#5090 (`reset_for_next_file` boundary), measures 1.22 × files — a ~5 % drop from the same fixture. **T2.1.A** scaffolding (inventory + shells + reset boundary) is on `perf/master`. **T2.1.B** sequential session-reuse path behind `TSZ_FILE_SESSION_REUSE` shipped at `32d1c20bfe`; the `CheckerContext::switch_to_file` boundary in `crates/tsz-checker/src/context/file_session_reset.rs` clears file-local state while preserving the shared `QueryCache` and program-stable caches. **T2.1.C** parallel session reuse (#5842, merged `ee20f50f0e`) extends the same boundary to the rayon-chunked parallel driver path. The post-#6449 attribution refresh (`perf-runs/2026-05-13-post-alias-mapped-attribution.md`) shows `with_parent_cache_constructed = 0` and `DelegateCrossArenaSymbol = 0` across monorepo-001..006, so the child-checker-specific T2.1.D target is satisfied on this fixture set. A follow-up timing-mode refresh now confirms checker dominance remains at the cliff (median check share ~96.7-98.0% on monorepo-003..006). Next action is non-child-checker hotspot work, starting with `compute_type_of_symbol` call-volume reduction. Decision records: [`perf-runs/2026-05-11-attribution-lock-wait.md`](perf-runs/2026-05-11-attribution-lock-wait.md), [`perf-runs/2026-05-13-post-alias-mapped-timing-refresh.md`](perf-runs/2026-05-13-post-alias-mapped-timing-refresh.md). |
| Typed cross-file query migration | **Done (scale-cliff monorepo-001..006)** | #6111 landed the first `DelegateCrossArenaSymbol` source-file symbol-arena gateway path. #6144 then removes the dominant `TypeEnvironmentCore` arena-only type-param child-checker path. #6191 converts 96 stable source-file symbol-arena bucket-empty misses into cross-file cache hits on monorepo-006, dropping `DelegateCrossArenaSymbol` from 924 to 828. #6203 classifies the residue: 247 stable source-file keys are cold first reads, 540 are source-file variable symbols outside the current stability proof, and 41 are declaration-file targets. #6212 proves and admits the annotated single-declaration variable slice, dropping the variable-driven `not_class_or_interface` outcome from 540 to 0 and `DelegateCrossArenaSymbol` from 828 to 539 on monorepo-006. #6231 adds a direct source-file interface query for scope-independent stable interfaces, dropping `DelegateCrossArenaSymbol` from 539 to 292 on monorepo-006. #6243 adds a direct source-file variable annotation query for scope-independent annotations and same-file direct interfaces, dropping `DelegateCrossArenaSymbol` from 292 to 41 on monorepo-006. #6260/#6286 route a conservative actual bundled-lib option/registry interface slice through the existing lib resolver, dropping `DelegateCrossArenaSymbol` from 41 to 40 and `checker.with_parent_cache_constructed` from 56 to 55 on monorepo-006. #6314 broadens the proven non-DOM/non-webworker interface slice, dropping `DelegateCrossArenaSymbol` from 40 to 31 and `checker.with_parent_cache_constructed` from 55 to 40 while keeping aliases and value-merged symbols on fallback paths. #6302 adds the first namespace-qualified actual-lib slice for `Intl.CollatorOptions`; its isolated pre-#6314 run dropped `DelegateCrossArenaSymbol` from 40 to 39 and `checker.with_parent_cache_constructed` from 55 to 54, and on the current broader path it should remove that single interface from the #6314 residue. A core value-interface allowlist follow-up then drops `DelegateCrossArenaSymbol` from 30 to 28 and `checker.with_parent_cache_constructed` from 39 to 37 (aliases unchanged at 16; interfaces 14 -> 12). A targeted `resolve_lib_type_with_params` fallback for unresolved actual-lib interface names then drops `delegate.misses` from 36 to 27, `DelegateCrossArenaSymbol` from 28 to 25, and `checker.with_parent_cache_constructed` from 37 to 28 (aliases unchanged at 16; interfaces 12 -> 9). Expanding `Intl.*` fallback coverage plus targeted `Symbol`/`IteratorObject` admissions then drops `delegate.misses` from 36 to 20, `DelegateCrossArenaSymbol` from 28 to 20, and `checker.with_parent_cache_constructed` from 37 to 20 (aliases 16 -> 17; interfaces 12 -> 3). A targeted `NumberFormatOptionsSignDisplayRegistry` admission plus namespace-symbol fallback gating then drops `delegate.misses` from 20 to 19, `DelegateCrossArenaSymbol` from 20 to 19, and `checker.with_parent_cache_constructed` from 20 to 19 (aliases unchanged at 17; interfaces 3 -> 2). A narrow `Intl.Locale` heritage admission in namespace-direct lowering then drops `delegate.misses` from 19 to 18, `DelegateCrossArenaSymbol` from 19 to 18, and `checker.with_parent_cache_constructed` from 19 to 18 (aliases unchanged at 17; interfaces 2 -> 1). A targeted `Iterator` admission for merged builtin-lib symbols with unmapped declaration indices then drops `delegate.misses` from 18 to 17, `DelegateCrossArenaSymbol` from 18 to 17, and `checker.with_parent_cache_constructed` from 18 to 17 (aliases unchanged at 17; interfaces 1 -> 0). A narrow `Intl` option-alias allowlist plus namespace-symbol alias lowering then drops `delegate.misses` from 17 to 13, `DelegateCrossArenaSymbol` from 17 to 13, and `checker.with_parent_cache_constructed` from 17 to 13 (aliases 17 -> 13; interfaces unchanged at 0). A follow-up admitting two additional `Intl` alias exports (`LocalesArgument` and `UnicodeBCP47LocaleIdentifier`) via the same namespace-symbol alias-lowering path then drops `delegate.misses` from 13 to 11, `DelegateCrossArenaSymbol` from 13 to 11, and `checker.with_parent_cache_constructed` from 13 to 11 (aliases 13 -> 11; interfaces unchanged at 0). A follow-up alias mapped-declaration gate for allowlisted actual-lib aliases (including `Record`, `Readonly`, `Partial`, `IteratorResult`, and `FlatArray`) then drops `delegate.misses` from 11 to 0, `DelegateCrossArenaSymbol` from 11 to 0, and `checker.with_parent_cache_constructed` from 11 to 0 while keeping diagnostics unchanged (`10,198`). The declaration-file residue is now 0 on monorepo-006. Decision records: [`perf-runs/2026-05-13-delegate-bucket-empty-attribution.md`](perf-runs/2026-05-13-delegate-bucket-empty-attribution.md), [`perf-runs/2026-05-13-delegate-residue-classification.md`](perf-runs/2026-05-13-delegate-residue-classification.md), [`perf-runs/2026-05-13-delegate-variable-symbol-cache.md`](perf-runs/2026-05-13-delegate-variable-symbol-cache.md), [`perf-runs/2026-05-13-delegate-source-file-direct-interface.md`](perf-runs/2026-05-13-delegate-source-file-direct-interface.md), [`perf-runs/2026-05-13-delegate-source-file-variable-direct.md`](perf-runs/2026-05-13-delegate-source-file-variable-direct.md), [`perf-runs/2026-05-13-delegate-actual-lib-direct.md`](perf-runs/2026-05-13-delegate-actual-lib-direct.md), [`perf-runs/2026-05-13-delegate-intl-lib-direct.md`](perf-runs/2026-05-13-delegate-intl-lib-direct.md), [`perf-runs/2026-05-13-delegate-actual-lib-core-value-allowlist.md`](perf-runs/2026-05-13-delegate-actual-lib-core-value-allowlist.md), [`perf-runs/2026-05-13-delegate-actual-lib-with-params-fallback.md`](perf-runs/2026-05-13-delegate-actual-lib-with-params-fallback.md), [`perf-runs/2026-05-13-delegate-actual-lib-intl-fallback-expansion.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-fallback-expansion.md), [`perf-runs/2026-05-13-delegate-actual-lib-intl-sign-display-registry.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-sign-display-registry.md), [`perf-runs/2026-05-13-delegate-actual-lib-intl-locale-heritage.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-locale-heritage.md), [`perf-runs/2026-05-13-delegate-actual-lib-iterator-unmapped-decls.md`](perf-runs/2026-05-13-delegate-actual-lib-iterator-unmapped-decls.md), [`perf-runs/2026-05-13-delegate-actual-lib-intl-alias-keyof.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-alias-keyof.md), [`perf-runs/2026-05-13-delegate-actual-lib-intl-alias-locales-unicode.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-alias-locales-unicode.md), [`perf-runs/2026-05-13-delegate-actual-lib-alias-mapped-decls.md`](perf-runs/2026-05-13-delegate-actual-lib-alias-mapped-decls.md), [`perf-runs/2026-05-13-post-alias-mapped-attribution.md`](perf-runs/2026-05-13-post-alias-mapped-attribution.md). |
...
**2026-05-13 core value-interface allowlist update:** on top of the rebased
actual-lib direct path (`e38a96cf19` baseline), admitting a small value-bearing
interface set (`Function`, `Iterator`, `Locale`, `Object`, `RegExp`) drops
monorepo-006 `DelegateCrossArenaSymbol` from 30 to 28,
`checker.with_parent_cache_constructed` from 39 to 37, and `delegate.misses`
from 38 to 36 with unchanged diagnostics (`10,198`). The declaration-file
residue is now 28 misses: 16 type aliases plus 12 interfaces. Decision record:
[`perf-runs/2026-05-13-delegate-actual-lib-core-value-allowlist.md`](perf-runs/2026-05-13-delegate-actual-lib-core-value-allowlist.md).

**2026-05-13 with-params fallback update:** on top of the same rebased path,
the direct actual-lib slice now tries `resolve_lib_type_with_params` first for
a targeted unresolved interface list (e.g., `ArrayIterator`,
`DateTimeFormatOptions`, `NumberFormatOptions*`, `Object`,
`RegExpStringIterator`, `StringIterator`) before falling back to the existing
`resolve_lib_type_by_name` + `Intl.CollatorOptions` route. On monorepo-006 this
drops `delegate.misses` from 36 to 27, `DelegateCrossArenaSymbol` from 28 to
25, and `checker.with_parent_cache_constructed` from 37 to 28 with unchanged
diagnostics (`10,198`). The declaration-file residue is now 25 misses: 16 type
aliases plus 9 interfaces. Decision record:
[`perf-runs/2026-05-13-delegate-actual-lib-with-params-fallback.md`](perf-runs/2026-05-13-delegate-actual-lib-with-params-fallback.md).

**2026-05-13 Intl fallback expansion update:** on top of #6359, expanding the
direct-path `Intl.*` fallback allowlist (beyond `CollatorOptions`) and adding
targeted `Symbol`/`IteratorObject` admissions drops monorepo-006
`delegate.misses` from 36 to 20, `DelegateCrossArenaSymbol` from 28 to 20, and
`checker.with_parent_cache_constructed` from 37 to 20 with unchanged
diagnostics (`10,198`). The declaration-file residue is now 20 misses: 17 type
aliases plus 3 interfaces. Decision record:
[`perf-runs/2026-05-13-delegate-actual-lib-intl-fallback-expansion.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-fallback-expansion.md).

**2026-05-13 Intl SignDisplay registry follow-up:** on top of #6369, adding
`NumberFormatOptionsSignDisplayRegistry` to the targeted direct-path
`Intl.*`/with-params set and allowing namespace-export symbol fallback when the
export symbol still passes the direct actual-lib declaration gate drops
monorepo-006 `delegate.misses` from 20 to 19, `DelegateCrossArenaSymbol` from
20 to 19, and `checker.with_parent_cache_constructed` from 20 to 19 with
unchanged diagnostics (`10,198`). The declaration-file residue is now 19
misses: 17 type aliases plus 2 interfaces. Decision record:
...
**2026-05-13 Intl Locale heritage follow-up:** on top of #6395, allowing
namespace-direct lowering of `Intl.Locale` despite heritage clauses (while
keeping the existing heritage guard for all other interfaces) drops
monorepo-006 `delegate.misses` from 19 to 18, `DelegateCrossArenaSymbol` from
19 to 18, and `checker.with_parent_cache_constructed` from 19 to 18 with
unchanged diagnostics (`10,198`). The declaration-file residue is now 18
misses: 17 type aliases plus 1 interface. Decision record:
[`perf-runs/2026-05-13-delegate-actual-lib-intl-locale-heritage.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-locale-heritage.md).

**2026-05-13 Iterator unmapped-decl follow-up:** on top of #6398, adding a
targeted `Iterator` admission for merged builtin-lib symbols that carry
unmapped declaration indices (while keeping the strict default declaration gate
for other symbols) drops monorepo-006 `delegate.misses` from 18 to 17,
`DelegateCrossArenaSymbol` from 18 to 17, and
`checker.with_parent_cache_constructed` from 18 to 17 with unchanged
diagnostics (`10,198`). The declaration-file residue is now 17 misses, all type
aliases. Decision record:
[`perf-runs/2026-05-13-delegate-actual-lib-iterator-unmapped-decls.md`](perf-runs/2026-05-13-delegate-actual-lib-iterator-unmapped-decls.md).

**2026-05-13 Intl alias-keyof follow-up:** on top of #6417, admitting a narrow
`Intl` option-alias set (`NumberFormatOptionsCurrencyDisplay`,
`NumberFormatOptionsSignDisplay`, `NumberFormatOptionsStyle`,
`NumberFormatOptionsUseGrouping`) through namespace-symbol alias lowering drops
monorepo-006 `delegate.misses` from 17 to 13, `DelegateCrossArenaSymbol` from
17 to 13, and `checker.with_parent_cache_constructed` from 17 to 13 with
unchanged diagnostics (`10,198`). The declaration-file residue is now 13
misses, all type aliases. Decision record:
[`perf-runs/2026-05-13-delegate-actual-lib-intl-alias-keyof.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-alias-keyof.md).

**2026-05-13 Intl Locales/Unicode alias follow-up:** on top of #6430, adding
`LocalesArgument` and `UnicodeBCP47LocaleIdentifier` to the same narrow `Intl`
alias allowlist drops monorepo-006 `delegate.misses` from 13 to 11,
`DelegateCrossArenaSymbol` from 13 to 11, and
`checker.with_parent_cache_constructed` from 13 to 11 with unchanged
diagnostics (`10,198`). The declaration-file residue is now 11 misses, all
type aliases. Decision record:
[`perf-runs/2026-05-13-delegate-actual-lib-intl-alias-locales-unicode.md`](perf-runs/2026-05-13-delegate-actual-lib-intl-alias-locales-unicode.md).

**2026-05-13 mapped-declaration alias follow-up:** on top of #6440, extending
the allowlist to the remaining actual-lib aliases (`Record`, `Readonly`,
`Partial`, `IteratorResult`, `FlatArray`, `PropertyKey`,
`DecoratorMetadataObject`, `DecoratorMetadata`) and admitting allowlisted
aliases when merged symbols include unmapped declaration indices drops
monorepo-006 `delegate.misses` from 11 to 0, `DelegateCrossArenaSymbol` from
11 to 0, and `checker.with_parent_cache_constructed` from 11 to 0 with
unchanged diagnostics (`10,198`). Decision record:
[`perf-runs/2026-05-13-delegate-actual-lib-alias-mapped-decls.md`](perf-runs/2026-05-13-delegate-actual-lib-alias-mapped-decls.md).

**2026-05-13 post-#6449 attribution refresh:** running monorepo-001..006 after
the mapped-declaration alias slice shows `delegate.misses = 0`,
`with_parent_cache_constructed = 0`, and `DelegateCrossArenaSymbol = 0` across
all six fixtures, while check-phase dominance remains (~97-98% on
monorepo-003..006) and diagnostics stay unchanged. Decision record:
[`perf-runs/2026-05-13-post-alias-mapped-attribution.md`](perf-runs/2026-05-13-post-alias-mapped-attribution.md).

**2026-05-13 post-#6454 timing refresh:** timing-mode runs (median over two
runs on monorepo-003..006) still show checker-dominant cliff behavior
(`check ~= 96.7-98.0%`) with unchanged diagnostics. With child-checker
residue already at zero on this fixture set, the next checker-internal lane is
`compute_type_of_symbol` call-volume reduction, guided by call-site
attribution. Decision record:
[`perf-runs/2026-05-13-post-alias-mapped-timing-refresh.md`](perf-runs/2026-05-13-post-alias-mapped-timing-refresh.md).
