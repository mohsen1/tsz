...
- **2026-04-26** · branch `chore/parser-flags-test-coverage` · **Test coverage WIP** · Lock the bitfield invariants and helper functions in `crates/tsz-parser/src/parser/flags.rs` (138 LOC, currently 4 spot-check assertions in `crates/tsz-parser/tests/tests.rs`) behind a comprehensive unit-test file. New `crates/tsz-parser/tests/flags_tests.rs` mounted via `parser/mod.rs` covers: every `node_flags::*` constant value (33 constants), every `modifier_flags::*` constant value (17 constants), every `transform_flags::*` constant value (29 constants), `node_flags::is_await_using` / `is_let_or_const` / `is_block_scoped` truth tables (NONE, single-bit, AWAIT_USING decomposition, irrelevant high bits, all-three combos), the `AWAIT_USING == CONST | USING` derived-value invariant, and disjointness of single-bit constants from `_HAS_*` aggregate flags. Pure additive — no production change. Future flag renumbering or helper edits will fail the new tests instead of slipping through.
