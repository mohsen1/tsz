...
- **2026-04-26** · branch `chore/wasm-target-module-option-helper` · **DRY workstream-4 active claim** · Workstream 4 ("WASM API surface, options, and diagnostics DTOs — Centralize option DTO conversion"): introduce `target_kind_from_u8` and `module_kind_from_u8` helpers in `crates/tsz-wasm/src/wasm_api/` and migrate the 5 duplicated `ScriptTarget::from_ts_numeric(u32::from(self.target.unwrap_or(1))).unwrap_or(ScriptTarget::ES5)` / `ModuleKind::from_ts_numeric(u32::from(...unwrap_or(0))).unwrap_or(ModuleKind::None)` sites in `wasm_api/program.rs` (resolve_module, emit_json, emit_file) and `wasm_api/emit.rs` (to_printer_options, transpile). Pure refactor, behavior-preserving.
