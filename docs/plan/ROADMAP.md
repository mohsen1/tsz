...
- **2026-04-26** · branch `chore/lowering-test-fixture` · **DRY workstream-8.9 active claim (lowering)** · workstream 8 item 9 ("Create parser/scanner/binder/lowering fixtures") for the lowering crate. Eight sibling `parse_X` helpers in `crates/tsz-lowering/tests/lower_tests.rs` each repeated the same 7-line `ParserState::new("test.ts", …) ; parse_source_file ; assert no parse-diagnostics ; std::mem::take(&mut parser.arena)` prelude. Extract that prelude into a single `parse_and_take_arena(source) -> NodeArena` helper; reduce each per-kind helper to its node-search loop only. Pure refactor — no production change. 114 lowering tests pass. Net 49 lines removed. Draft PR title: `[WIP] chore(lowering/tests): extract parse_and_take_arena helper`.
