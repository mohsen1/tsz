...
6. With perf counters enabled, the simple local-interface object shortcut now
   classifies the conformance-safe non-iterator missing-interface
   declaration/provenance rows as actual/cloned lib-backed in attribution while
   keeping normal checker execution and semantic type computation on the
   existing full merge path:
