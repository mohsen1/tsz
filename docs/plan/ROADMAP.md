...
- **2026-04-25** · branch `chore/span-conversion-test-matrix` · **DRY workstream-8.5 claim** · workstream 8 item 5 ("AST traversal and span semantics: Add span conversion tests for ASCII, multi-byte UTF-8, and surrogate-pair UTF-16"): the existing `crates/tsz-common/tests/position_tests.rs` covers basic UTF-16 supplementary planes but lacks 2-byte UTF-8 (Latin-1 supplement), 3-byte UTF-8 (BMP non-ASCII), 4-byte UTF-8 (supplementary planes / surrogate pairs in UTF-16), mixed-width content with empty lines, very long ASCII line, and partial position-into-surrogate. Add a parameterized matrix that locks each conversion direction (`offset_to_position`, `position_to_offset`, `line_start`) on a representative source covering each width class. Pure test additions — no runtime change. Draft PR: `[WIP] test(common/position): add UTF-8 / UTF-16 span conversion matrix`.
