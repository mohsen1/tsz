...
- **2026-04-26** · branch `chore/lsp-project-file-context` · **DRY workstream-2 active claim** · workstream 2 item 1 ("Add `ProjectFileContext` / `with_file_context` helpers to centralize: file name, arena, binder, source text, line map"): the binder-tier provider construction pattern `Provider::new(file.arena(), file.binder(), file.line_map(), file.file_name().to_string(), file.source_text())` repeats ~17 times in `crates/tsz-lsp/src/project/features.rs` and `operations.rs` (each call expanding to 6-7 lines). Add a `LspProviderContext<'a>` view struct + `ProjectFile::provider_context()` accessor in `crates/tsz-lsp/src/project/`, extend the `define_lsp_provider!(binder ...)` macro arm with a `from_context` constructor, and migrate the binder-tier sites in `features.rs`. Pure refactor; behavior preserved. The minimal-tier (`(arena, line_map, source_text)`) and full-tier sites stay unchanged in this slice — they can land in subsequent PRs.
