...
- **2026-04-26** · branch `chore/snapshot-rollback-count-guard` · **Workstream-4 / architecture-health-metric-5 claim** · Operating Principle 8 + Architecture Health Metric 5 ("Broad cache-clear and snapshot-restore call sites"): pin the **snapshot-rollback** half of metric 5 (the cache-clear half is bounded today and can be pinned separately if it grows). Add `SNAPSHOT_ROLLBACK_FILE_COUNT_CHECKS` + `scan_snapshot_rollback_file_count` in `scripts/arch/arch_guard.py` mirroring metric 3 (PR #1237). Counts non-test files under `crates/tsz-checker/src/` that call any of the speculation-rollback APIs — `CheckerContext::rollback_full` / `rollback_diagnostics(_filtered)?` / `rollback_and_replace_diagnostics` / `rollback_return_type` / `rollback_filtered`, the `restore_ts2454_state` / `restore_implicit_any_closures` snapshot restorers, or a `*guard.rollback(` SpeculationGuard call — outside `crates/tsz-checker/src/context/speculation.rs` (the canonical home). Live count: 15 files. Cap pinned at 15. New `arch_guard.py` unit tests lock the regex semantics. Draft PR: `[WIP] chore(arch): pin snapshot-rollback call site count (architecture health metric 5)`.
