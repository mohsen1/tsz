...
- **2026-04-25** · branch `fix/checker-ts1238-generic-decorator-call` · **KPI-2 partial-land + regression-suspect** · Targeted `decoratorCallGeneric.ts` (TS1238). Smoke confirmed dist-fast CLI emits 0 at origin/main HEAD; the checker test harness emits TS1238 correctly. Unit-test PR #1220 is landing as a behavior lock. **Deeper finding**: a CLI binary built from local `main` (`fe3e457d3f`, behind origin/main by ~20 commits) DOES emit TS1238 for the same source. A CLI binary built from origin/main HEAD (`f0e5fa9a60`) emits 0. **Regression introduced somewhere in the range `fe3e457d3f..f0e5fa9a60`**. Suspected culprits (ordered by likelihood given decorator/generic-constraint surface): `f4f9ee7d3d perf: Arc-share per-file semantic_defs`, `876319e9c2 Revert "respect constrained type parameter overlap"`, `c7ab1272ab fix: respect constrained type parameter overlap`, `0f7afc9974 fix(solver/diagnostics): preserve optional params in assignability`. Future-iteration target: `git bisect` `fe3e457d3f..f0e5fa9a60` rebuilding and re-running `./scripts/conformance/conformance.sh run --filter decoratorCallGeneric --verbose` at each step to identify the regressing commit, then fix the root cause.
