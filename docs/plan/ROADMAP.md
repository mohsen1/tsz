...
- **2026-05-12 00:00:00** · branch `claude/busy-knuth-wMzjB` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 232 → 233 for `merged_value_types`, an `FxHashMap<SymbolId, TypeId>` that pre-caches the VALUE-side type for merged `const X + type X` symbols before type-alias body lowering, preventing false TS2456 circularity errors. Also added 4 files to the >2000 LOC exclusion set (`context/mod.rs`, `query_boundaries/common.rs`, `type_alias_variable_alias.rs`, `type_node_advanced.rs`) that crossed the threshold as part of the merged-symbol fix.
