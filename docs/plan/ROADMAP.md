...
- **2026-04-26** · branch `chore/common-script-target-module-helper-tests` · **Coverage ready** · Test coverage for previously-uncovered helpers in `crates/tsz-common/src/common/mod.rs`: `ScriptTarget::{ts_numeric_value, as_ts_str, supports_es2017, supports_es2018, supports_es2021, supports_es2022, supports_es2024}`, `ModuleKind::{is_node16_or_node18, as_ts_str, is_commonjs (negative cases)}`, plus `normalize_ts_option` edge cases (whitespace, dashes, underscores, empty, multi-char prefix matches that should not). These APIs are used in production by `tsz-cli`, `tsz-checker`, `tsz-emitter`, and `tsz-core` (e.g. `target.supports_es2022()` is a downleveling decision in 6+ checker/emitter sites; `module.is_node16_or_node18()` gates TS1479 emission in import declarations). Pure additive test additions in `crates/tsz-common/tests/common.rs`; no production change. PR #1285.
