...
- **2026-04-26** · branch `chore/emitter-import-usage-tests` · **DRY workstream-8 active claim** · workstream 8 item 4 / Architecture Health Metric: lock the text-based import-usage heuristics in `crates/tsz-emitter/src/import_usage.rs` (528 LOC, 0 unit tests). The three exported functions (`contains_identifier_occurrence`, `strip_type_only_content`, `strip_type_declaration_lines`) are heavily relied on by `emitter::module_emission::imports` and `lowering::core` for value-vs-type-only import classification, and currently have no behavior locks. Add a colocated `#[cfg(test)] mod tests` (or sibling integration test) covering identifier word-boundary detection, namespace-alias-import preservation, `import type` / `export type` / `export *` stripping, multi-line `interface`/`type` block handling, `as`/`satisfies` removal, `implements` clause stripping, generic-call `<T>` skipping, `const x: T = val` annotation stripping, and string-literal preservation. Pure additive; behavior preserving.
