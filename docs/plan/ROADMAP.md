...
- **2026-04-26** · branch `chore/scripts-conformance-query-helpers` · **DRY workstream-10 active claim** · workstream 10 item 3 ("Keep conformance/fourslash snapshot query helpers single-source"): `query-conformance.py` and `classify-render-corpus.py` each define their own `load_detail()` for `conformance-detail.json`; `basename(path)` is duplicated as a function in three scripts (`query-conformance.py`, `classify-render-corpus.py`, `analyze-conformance.py`) plus inlined as `path.rsplit("/", 1)[-1] if "/" in path else path` six more times in `query-conformance.py` alone. `is_fingerprint_only` and `is_same_code_count_drift` are also defined in `query-conformance.py` and rebuilt implicitly inside `classify-render-corpus.py:failure_category`. Extract a shared `scripts/lib/conformance_query.py` that wraps `lib.query_snapshot.load_snapshot()` with the conformance default path, plus `basename`, `code_counts`, `is_fingerprint_only`, `is_same_code_count_drift`. Migrate three scripts to import from it; add `scripts/lib/test_conformance_query.py` to lock the helper invariants. Pure refactor; verified by byte-identical before/after diffs of dashboard, overview, campaigns, code, fingerprint-only, false-positive, and classify-render-corpus invocations.
