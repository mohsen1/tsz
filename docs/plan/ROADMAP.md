...
- **2026-04-25** · branch `fix/parser-incremental-interner-coherence` · **Workstream 7 — ready for review** · Fix incremental parse interner identity drift: `ParserState::parse_source_file_statements_from_offset` did not refresh `arena.interner` after parsing, so atoms newly interned by the scanner during incremental parse resolved to `""` through `arena.resolve_identifier_text`. Mirrors the `parse_source_file` interner sync at the end of the incremental entry point. Adds two parser regression tests covering `resolve_identifier_text` and `get_identifier_text`. PR #1205. Verification: full parser/scanner/binder/LSP unit tests pass; conformance IMPROVED +11 (12140 vs 12129 baseline); emit IMPROVED JS +1, DTS +37; fourslash 50/50 100%.
...
- **2026-04-25** · branch `chore/diagnostic-speculation-snapshot-rename` · **Workstream-4 claim** · Operating principle 6 + architecture health metric 6 ("Speculation APIs with surprising non-RAII behavior"): `DiagnosticSpeculationGuard` is named "Guard" but explicitly does NOT roll back on drop — dropping is an implicit commit (per its own header comment). Per the roadmap rule "Avoid names that imply rollback safety when dropping preserves speculative state", rename to `DiagnosticSpeculationSnapshot`. Pure rename across the type definition + 6 call sites in `checkers/jsx/overloads.rs`, `types/function_type.rs`, `types/class_type/js_class_properties.rs`, `types/computation/object_literal/computation.rs`. No behavior change. Draft PR: `[WIP] chore(checker): rename DiagnosticSpeculationGuard → DiagnosticSpeculationSnapshot to match drop-implicit-commit semantics`.
