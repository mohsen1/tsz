...
- **2026-04-26** · branch `chore/conformance-extract-process-rss` · **DRY active claim** · DRY cleanup in `crates/conformance/src/`: extract the duplicated `get_process_rss(pid: u32) -> Option<usize>` helper (byte-identical Linux+macOS impl in `batch_pool.rs` lines 297-326 and `server_pool.rs` lines 313-340) into a new `crates/conformance/src/process_rss.rs` module. Move the existing `get_process_rss_reports_current_process_memory_usage` test there. Net -28 LOC duplication. Behavior-preserving. Draft title: `[WIP] chore(conformance): extract shared get_process_rss helper`.
