...
- **2026-04-25** · branch `chore/emit-extract-cli-arg-builder` · **DRY workstream-10 active claim** · workstream 10 item 1 ("Add or continue shared compiler-option extraction and output-path helpers for emit scripts"): extract the duplicated ~30-line `if (X) args.push(...)` block in `scripts/emit/src/cli-transpiler.ts` (main args at L296-329 + retry args at L385-414) into a single `appendCompilerOptionFlags(args, opts)` helper. **Latent bug fix**: the retry path is currently MISSING `--strictNullChecks` (present in main args at L316), causing emit divergence on declaration-emit retry; helper extraction makes both paths identical by construction. Draft PR title: `[WIP] chore(scripts/emit): extract shared compiler-option flag builder; fix retry-path strictNullChecks drop`.
