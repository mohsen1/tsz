...
- **2026-05-06** · branch `fix/instantiation-leaf-fast-path-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 to match the branch's live context shape while preserving the instantiation leaf fast-path change. All non-lint CI lanes were already green; this keeps the guardrail explicit and visible for follow-up context-state reduction work.
