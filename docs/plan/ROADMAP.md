...
- **2026-04-25** · branch `chore/emit-runner-cache-sha256` · **DRY workstream-10 active claim** · workstream 10 item 2 ("Use stable JSON plus `crypto.createHash(\"sha256\")` for cache hashing"): replace 32-bit polynomial `hashString` in `scripts/emit/src/runner.ts` with `crypto.createHash("sha256")`; switch `getCacheKey` input from colon-joined string to stable JSON; preserve current cache file format (string keys, string hashes — invalidates existing cache on first run, then re-fills). Draft PR title: `[WIP] chore(scripts/emit): replace 32-bit hashString with sha256 + stable JSON for cache keys`.
