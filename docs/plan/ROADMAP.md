...
4. Post-Arc-share-`node_flow` (#1235): peak RSS measured at ~7.0 GB resident, still exit 137 at ~50s. Same-machine before/after (sampled at 2s intervals on a busy 32 GB host) showed run-to-run variation in the noise band (~6.9-7.5 GB), so the direct measurable savings are within noise. The change is structurally correct (same template as #1202/#1227, no `Arc::make_mut` post-binding hot path) and unblocks the remaining template migrations.
5. Post-Arc-share-`node_scope_ids` (this PR): peak RSS sampled across 6 paired runs (BEFORE 7.0-11.2 GB, mean 8.2 GB; AFTER 8.2-11.8 GB, mean 9.8 GB), still exit 137 at ~45-55s. The bands overlap heavily on this OOM-bound workload — once the OS jetsam fires, the recorded peak depends almost entirely on sampler timing relative to the kill, so per-run RSS is not a stable signal here. The change is structurally correct (same template as #1202/#1227/#1235, single `Arc::make_mut` mutation surface confined to the binding hot path) and unblocks the remaining template migrations (`top_level_flow`, `switch_clause_to_switch`).
6. Implication: continue Arc-sharing the remaining per-binder maps (`top_level_flow`, `switch_clause_to_switch`) to push further toward stable completion. After the obvious deep clones are gone, the next leverage will come from cutting per-file `scopes: Vec<Scope>` deep clones (each `Scope` carries a `SymbolTable`) and switching the OOM workload to a bounded-residency cross-file lookup mode.
7. Bench harness caveat: `tsz: TIMEOUT` in the table can mean either timer-kill (124) or OS-kill (137). The 137 case ("OOM-by-paging") is the dominant failure mode here. Inspect exit codes when investigating.
