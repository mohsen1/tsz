...
- **2026-04-25** · branch `chore/binder-test-fixture` · **DRY workstream-8.9 active claim (binder)** · workstream 8 item 9 ("Create parser/scanner/binder/lowering fixtures") for the binder. The existing `parse_and_bind(source) -> (BinderState, ParserState)` helper at `crates/tsz-binder/src/state/tests.rs:734` was already in place but underused. Migrate the 29 inline 4-line `ParserState::new("test.ts", …) ; parse_source_file ; BinderState::new ; bind_source_file` blocks that share the `"test.ts"` file name onto the existing helper, with per-site `mut`/`_` binding adjusted to whether each test actually mutates or references `binder` and `parser` afterward. Net 93 lines removed. Sites that use other file names (e.g. `lib.d.ts`, `b.js`, `ambient.d.ts`), `BinderOptions::with_options`, or empty-binder construction are intentionally deferred. 245 binder lib tests pass. Draft PR title: `[WIP] chore(binder/tests): migrate canonical 4-line setup to existing parse_and_bind helper`.
- **2026-04-26** · branch `chore/binder-test-fixture-named-files` · **DRY workstream-8.9 active claim (binder follow-up, stacked on #1230)** · Follow-up to #1230: add `parse_and_bind_named(file_name, source) -> (BinderState, ParserState)` alongside the existing `parse_and_bind(source)` (which now delegates to the named version with `"test.ts"`), and migrate the 8 deferred non-`"test.ts"` canonical 4-line setups onto it (`a.js`, `b.js`, `ambient.d.ts`, `ambient2.d.ts`, `foo.d.ts`, `point.d.ts`, `react.d.ts`, `test.d.ts`). Net 23 lines removed. Multi-parser lib-merge sites (`lib_parser`/`lib1_parser`/`lib2_parser`), `BinderOptions::with_options`, and empty-binder construction remain deferred. 245 binder lib tests pass. Draft PR title: `[WIP] chore(binder/tests): add parse_and_bind_named; migrate 8 non-test.ts canonical sites`.
