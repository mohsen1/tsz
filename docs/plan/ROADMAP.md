...
- **2026-04-26** · branch `fix/checker-jsdoc-template-in-body-scope` · **Workstream-1 claim** · Workstream 1 (Diagnostic Conformance — TS2304 false positive in JSDoc-typed JS code). Conformance target `jsdocTemplateConstructorFunction.ts`: tsz emits an extra TS2304 ("Cannot find name 'T'") at the `@type {T}` annotation inside a JS function whose JSDoc declared `@template T`. Root cause: `function_type.rs` pushes the JSDoc `@template T` names into `type_parameter_scope` only for signature construction and pops them before the body walk; by the time `check_function_body` runs, `T` is no longer in scope, so `resolve_jsdoc_reference("T")` falls through `type_parameter_scope.get` and produces TS2304. Fix: re-push the function's `@template T` JSDoc-derived `TypeParamInfo` entries at the top of `check_function_body` (mirroring the signature-builder shape — `intern_string` → `TypeParamInfo` → `factory.type_param` → `type_parameter_scope.insert`) and pop them at the end, gated on `is_js_file()`, empty syntactic `func.type_parameters`, a JSDoc string, and a `contains_key` guard so a name already pushed by an enclosing scope is not duplicated. 3 unit tests in `crates/tsz-checker/tests/jsdoc_template_in_body_scope_tests.rs` lock the body-scope `@type {T}` resolution, the inline-cast variant, and the no-unrelated-diagnostics sanity check. Draft PR: `[WIP] fix(checker): push @template T into type_parameter_scope around JS function body check`.
