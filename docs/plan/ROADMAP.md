...
- **2026-04-26** · branch `chore/common-diagnostics-helper-tests` · **DRY workstream-8 active claim** · Lock untested helpers in `crates/tsz-common/src/diagnostics/mod.rs`: `is_parser_grammar_diagnostic` (1000-1999 range), `is_js_grammar_diagnostic` (8000-8999 range), `Diagnostic::error()` constructor, and `format_message` template-placeholder normalization (`${ ... }` whitespace stripping). Pure additive tests; no production change. Public APIs called from `tsz-cli`, `tsz-lsp`, `tsz-checker`, `tsz-core`. WIP.
