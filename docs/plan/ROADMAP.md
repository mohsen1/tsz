...
- **2026-04-26** · branch `chore/conformance-options-convert-edge-tests` · **DRY active claim** · Workstream 1 / harness-coverage support: lock untested behavior of `crates/conformance/src/options_convert.rs` with edge-case unit tests covering (a) `noLib` directive blocking target-driven default-lib injection, (b) comma-separated `target` selecting libs from the first token, (c) unrecognized `target` skipping lib injection, (d) bool `false` mapped to `Value::Bool(false)`, (e) mixed-case directive keys normalized via `to_lowercase`, (f) `nofallthrough` alias mapping to `noFallthroughCasesInSwitch`, (g) empty-directives map producing empty options, (h) explicit `noLib: true` preventing lib injection. The conformance harness routes every test directive through this module; locking these branches prevents silent drift when adding new directive support. Pure test additions in `crates/conformance/src/options_convert.rs` test module; no production change.
