...
- **2026-04-26** · branch `chore/decl-emit-semantic-rederive-guard` · **Workstream-2 / architecture-health-metric-8 ready** · Operating Principle 8 + Architecture Health Metric 8 ("Declaration emit paths that rederive semantic facts instead of consuming summaries"). Adds `DECLARATION_EMIT_REDERIVE_CHECKS` + `scan_declaration_emit_rederive` to `scripts/arch/arch_guard.py`. The check walks `crates/tsz-emitter/src/declaration_emitter/` and counts non-test files that import `tsz_checker`, `tsz_solver`, or `tsz_binder` directly — each one being a place where declaration emit reaches into upstream semantic computation rather than reading from a stable semantic summary (workstream 2 "Declaration emit priorities"). A file is counted at most once even when it imports multiple semantic crates. Cap pinned at 19 (live count: 19 of 23 production files). Future rederive additions force a cap bump in the same diff and a ROADMAP entry; consolidation onto a documented declaration-emit semantic view shows up as a cap reduction. 7 new unit tests in `scripts/arch/test_arch_guard.py` lock the detection semantics (all import forms flagged, non-semantic crates not flagged, multi-crate imports counted once, test/bench files excluded, comment-only lines ignored, cap matches live, check registered). Draft PR title: `[WIP] chore(arch): pin declaration emit semantic-rederive count`.
