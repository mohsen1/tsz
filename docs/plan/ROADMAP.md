...
- **2026-04-25** · branch `fix/class-property-init-preserves-declared-type` · **Workstream 1 (Diagnostic Conformance) claim — ABANDONED** · Class-property initializer type widening: `class C { D = DEFAULT }` where `DEFAULT: AB = 'A'` widens `D` to `string` instead of preserving `AB`. Tractable narrow fix (only widen fresh literals, mirroring `let`/`var`) was net-zero on conformance: tsz reads `DEFAULT` via flow-narrowing as `'A'` instead of `AB`, so even after suppressing the unconditional widen the property type lands at `'A'` (still wrong, just differently). The proper fix needs class-property inference to use the *declared* (symbol) type of the initializer source, not the flow-narrowed type — a deeper change in `get_type_of_node`-vs-symbol-type interaction. Draft PR #1201 closed.
