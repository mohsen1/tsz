...
- **2026-04-26** · branch `chore/emitter-transforms-helpers-tests` · **DRY workstream-1 active claim** · workstream 1 ("Test harness consolidation: continue locking pure helpers in tsz-emitter behind unit tests"): `crates/tsz-emitter/src/transforms/helpers.rs` (710 LOC) defines the `HelpersNeeded` flag struct, `any_needed()`, `needed_names()`, and the `emit_helpers()` priority-ordering function plus ~30 runtime-helper string constants (`__extends`, `__assign`, `__awaiter`, `__generator`, etc.). The file currently has zero direct unit tests; `emit_helpers` priority order and `needed_names` ordering are only exercised through full-emit integration tests, so a regression in priority sort or a missing `tslib` import-name would only surface as a downstream emit diff. Add inline `#[cfg(test)] mod tests` at the bottom of `helpers.rs` covering: (1) `HelpersNeeded::default().any_needed() == false`, (2) `any_needed()` flips for each individual flag, (3) `needed_names()` order matches the documented `compareEmitHelpers` priority, (4) `emit_helpers()` priority sort puts `__extends` before `__assign` before `__decorate` before `__metadata` before `__param` before `__awaiter` before `__generator` before unprioritized helpers, (5) `class_private_field_set_before_get` flips Set/Get ordering, (6) `import_star` triggers both `__setModuleDefault` and `__importStar` emission, (7) the no-priority block (`__rest`, `__values`, `__read`, `__spreadArray`, `__importDefault`, disposable helpers, `__asyncValues`) emits last, (8) every public `pub const HELPER` constant is non-empty and starts with `var __<name>`. Pure additive — no production change.
