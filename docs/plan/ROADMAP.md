...
- **2026-05-06** · branch `fix/large-ts-repo-nested-namespace-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 225 → 226 for `nested_namespace_candidates_cache`, a per-checker cache that avoids repeated nested namespace candidate scans in large repositories. Follow-up work should still extract related namespace caches into grouped state rather than continuing to grow `CheckerContext`.
