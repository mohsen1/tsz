...
- **2026-04-26** · branch `chore/emitter-integration-test-support` · **DRY workstream-8 active claim** · workstream 8 item 4 ("Add `emit_test_support` with parser/print helpers and table-case support"): the 5 emitter integration test files (`comment_tests.rs`, `variable_declaration_emit_tests.rs`, `optional_chaining_tests.rs`, `computed_property_es5_tests.rs`, `jsx_spread_tests.rs`) repeat the `let mut parser = ParserState::new(...); let root = parser.parse_source_file();` boilerplate ~14 times, plus the parse + `Printer::new(arena, opts)` + `set_source_text` + `print` + `finish` chain ~9 times in `comment_tests.rs` alone. Add `crates/tsz-emitter/tests/test_support.rs` with `parse_source`, `parse_and_print`, and `parse_and_print_with_opts` helpers; `#[path]`-mount it in each integration test file (Cargo `[[test]]`-registered binaries cannot share modules cross-binary, hence `#[path]`). Migrate the duplicate sites in the same PR. Pure refactor; behavior preserved.
- **2026-04-26** · branch `chore/frontend-internals-import-guard` · **Workstream-3 / architecture-health-metric-2 ready** · Operating Principle 8 + Architecture Health Metric 2 ("Frontend crates directly depending on parser/binder/checker/solver/emitter internals"). Adds `FRONTEND_PIPELINE_IMPORT_CHECKS` + `scan_frontend_pipeline_imports` to `scripts/arch/arch_guard.py`. The check walks `crates/tsz-cli/src`, `crates/tsz-core/src`, `crates/tsz-lsp/src`, `crates/tsz-wasm/src` and counts non-test files that import any of `tsz_parser`, `tsz_binder`, `tsz_checker`, `tsz_emitter`, `tsz_lowering` (each file counted once even when it imports multiple). Cap pinned at 73 (live count: 3 in cli, 4 in core, 65 in lsp, 1 in wasm). Complementary to metric 3 (#1237) which pins `tsz_solver` imports specifically — the two metrics are disjoint by design. 7 new unit tests in `scripts/arch/test_arch_guard.py` lock the detection semantics. Future cross-boundary imports must bump the cap in the same diff. Draft PR title: `[WIP] chore(arch): pin frontend parser/binder/checker/emitter/lowering import count`.
