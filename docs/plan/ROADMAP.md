...
- **2026-04-25** · branch `chore/scanner-test-fixture-regex-migration` · **DRY workstream-8.9 active claim (stacked on #1222)** · Follow-up to the scanner fixture: migrate the 39 `ScannerState::new(source, true)` sites in `regex_unicode_tests.rs` to `make_scanner(&str)`. Three sites that intentionally pass `skip_trivia = false` are kept inline (covered by the fixture's docstring). Pure test consolidation — no production change. 332 scanner tests pass. Draft PR title: `[WIP] chore(scanner/tests): migrate regex_unicode_tests to make_scanner`.
- **2026-04-25** · branch `chore/scanner-test-fixture-comprehensive-migration` · **DRY workstream-8.9 active claim (stacked on #1225)** · Final scanner-fixture migration: migrate the 171 default-options `ScannerState::new(source, true)` sites in `scanner_comprehensive_tests.rs` to `make_scanner(&str)`. Ten sites that intentionally pass `skip_trivia = false` (whitespace/comment-trivia tests) and the two top-of-file helpers (`scan_all_tokens`, `scan_single`) are migrated to the helper as well — they each took a `&str` parameter that flows directly into `make_scanner(s)`. Pure test consolidation — no production change. 208 tests in this binary pass; 332 scanner tests pass overall. Draft PR title: `[WIP] chore(scanner/tests): migrate scanner_comprehensive_tests to make_scanner`.
