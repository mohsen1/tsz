...
- **2026-04-25** · branch `fix/class-property-init-preserves-declared-type` · **Workstream 1 (Diagnostic Conformance) claim** · Class-property initializer type widening: `class C { D = DEFAULT }` where `DEFAULT: AB = 'A'` widens `D` to `string` instead of preserving `AB`, producing a TS2345 false positive at `expectAB(c.D)`. Fix routes through symbol-type inference (not display) — preserve the initializer expression's declared (annotated) type when a class property has no explicit type annotation. Draft PR title: `[WIP] fix(checker): preserve declared initializer type for class properties without annotation`. Targets `classPropertyInferenceFromBroaderTypeConst.ts`.
