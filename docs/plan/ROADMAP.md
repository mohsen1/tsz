...
- **2026-04-26** · branch `fix/checker-ts2862-keyof-receiver-self` · **Workstream-1 WIP** · Workstream 1 ("Diagnostic Conformance And Fingerprints"): fix false-positive TS2862 ("Type 'T' is generic and can only be indexed for reading") on writes like `obj[k]` where the receiver is a type parameter `T` and the index is bounded by the receiver's own keys (`keyof T` literally, or `K extends keyof T`). Root cause: `is_broad_index_type` evaluates `keyof T` to its constraint's key space (e.g. `string`), bypassing the existing keyof-of-type-parameter guard. Fix narrowed to `is_generic_indexed_write`: when the object is a type parameter, exclude indices whose keyof inner is the same type parameter. Targets `conformance/types/keyof/keyofAndIndexedAccessErrors.ts` extra TS2862; ships solver-style unit-test lock. Net-zero conformance regressions targeted.
