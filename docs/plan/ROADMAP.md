...
2. **Conformance maintenance (Workstream 1)** — second-class priority. The
   ...
   held until the slowdown is offset.
...
To avoid duplicate work, roadmap-adjacent implementation, including DRY cleanup, must be visible before coding starts. Use GitHub issues and draft PRs as the coordination surface.

Workflow:

1. Pull latest `main` and inspect open issues and PRs for overlap.
2. Create a branch for the intended work.
3. Create or update a GitHub issue for the scope and mark it `WIP`.
...
1. A WIP PR is for coordination, not a merge candidate.
2. Never merge a branch while its PR is draft, labeled `WIP`, titled with `[WIP]`, or otherwise described as WIP.
3. Before marking ready, update the issue/PR, remove the `WIP` label, remove the `[WIP]` title prefix, and mark the PR ready.
4. If implementation is abandoned, close the draft PR and update the issue.
5. DRY cleanup uses this same coordination flow. Keep each DRY slice small and reviewable.

### Merging Policy

When a PR's CI is otherwise green and the only remaining merge blocker is a `docs/plan/ROADMAP.md` rebase conflict:

1. Resolve the ROADMAP conflict locally.
...
6. The simple local-interface object shortcut now has attribution for its
   remaining declaration/provenance guard residue: regenerated monorepo-006
   shows `24,762` shortcut hits with diagnostics unchanged at `10,198`, and
   names `13` fallback rows split across `reject_out_of_arena_decl` and
   `reject_missing_interface_decl`. Next behavior work should prove one row
   family structurally before relaxing either guard.
...
2. Check current repo and GitHub state before starting work:
   - `git fetch origin`
   - `git status --short`
...
4. Open or update a WIP issue, open a draft PR with the `WIP` label, then begin implementation.
...
2. Run targeted crate tests for the touched helper when useful, then rely on CI before marking ready.
...
1. **Preserve 100% conformance.** Treat any conformance drift as a regression and fix it through the owning architecture layer.
2. **Emit pass-rate fixes are the next public pass-rate priority.** Bucket failures by transform family; pick the bucket with the highest test count and fix one transform end-to-end.
