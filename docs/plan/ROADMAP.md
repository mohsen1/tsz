...
- **2026-05-09 00:30:00** · branch `claude/nice-darwin-0LFgG` · **Conformance — JS/JSDoc import diagnostic parity (#3508)** · Runtime ES `import { Foo }` paired with JSDoc `@import { Foo }` in a checked JS file currently produces TS18042 instead of two TS2300s. Binder fix: `bind_jsdoc_import_tags` skips the JSDoc declaration when the local name already has an alias in the current scope, so the runtime symbol keeps ownership of the binding and its `is_type_only` is not corrupted. Checker fix: `check_jsdoc_duplicate_imports` also walks the arena for runtime named/default/namespace import local-binding positions and emits TS2300 at every position when at least one occurrence is JSDoc. Six new regression tests in `crates/tsz-checker/tests/issue_3508_jsdoc_runtime_duplicate_tests.rs`. Closes #3508.
