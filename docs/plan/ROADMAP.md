...
- **2026-04-25** · branch `chore/scripts-remove-unused-readme-updater` · **DRY workstream-10 active claim** · workstream 10 item 4 ("Pick one README metrics updater"): delete `scripts/update-readme-from-ci-metrics.mjs` (dead code; workflows `.github/workflows/ci.yml` and `.github/workflows/update-readme.yml` that called it were removed; only `.github/workflows/gh-pages.yml` remains and does not invoke it; `scripts/refresh-readme.py` remains the single documented updater per `scripts/README.md`). Draft PR title: `[WIP] chore(scripts): drop unused update-readme-from-ci-metrics.mjs`.
