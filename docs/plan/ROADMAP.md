...
- **2026-05-10 11:05:00** · branch `perf/t2.2-typeparam-memo-2026-05-10` · **Workstream-4 / architecture-health-metric-1 cap bump + T2.2 first migration** · `CheckerContext` field count cap 231 → 232 for `cross_file_type_params_cache`, the program-wide `Arc<DashMap<(file_idx, NodeIndex), Vec<TypeParamInfo>>>` introduced by the first Tier 2.2 typed-cross-file-query migration (`PERFORMANCE_PLAN.md` §7). Replaces both `with_parent_cache_attributed(..., TypeEnvironmentCore)` call sites with a memoized fast path; the slow path still runs on cache miss. Follow-up work should still group cross-file query state into a typed-query coordinator rather than continuing to grow `CheckerContext` field-by-field.
- **2026-05-10 09:55:00** · branch `perf/t0.4-attribution-decision-record-2026-05-10` · **PERFORMANCE_PLAN T0.4 — Tier 0 exit decision** · First checked-in attribution-mode phase split using the freshly-merged T0.2 (#4945) and T0.3 (#4948) JSON surfaces. Run is `monorepo-001..006` from `scripts/bench/scale-cliff/generate-fixtures.sh`. Cliff sits between monorepo-002 (1k files / 0.73 s) and monorepo-003 (5k files / 11.3 s). Phase split on cliff: `check ≈ 85 %`, `parse_bind ≈ 12.5 %`, `io_read ≈ 2-3 %`. Counter signal: `delegate.cache_hits_cross_file = 0` over ~1100 calls, `with_parent_cache_constructed = 1.28 × files`. **Decision: promote T2.2 typed cross-file queries (highest priority) and T2.1 lifetime split; defer T2.0 resolver and T2.4 interner redesign.** `large-ts-repo` deferred this round (OOM on flat config, stack overflow on `recovery-console`). Decision record: `docs/plan/perf-runs/2026-05-10-scale-cliff-summary.md`. Raw JSON: `docs/plan/perf-runs/raw/`.
- **2026-05-05 06:17:27** · branch `fix/emit-define-fields-typed-only` · **Emit pass-rate fix** · With `useDefineForClassFields: true` and target < ES2022, typed-only class fields like `prop: number;` (no initializer) were dropped from the constructor emit, leaving the runtime property unmaterialized. tsc lowers them to `Object.defineProperty(this, "prop", { value: void 0 })`. The lowering pass at `crates/tsz-emitter/src/emitter/declarations/class/emit_es6.rs:923` was skipping any property without an initializer; now it materializes them when `use_define_for_class_fields` is set, and the synthesized-constructor + `emit_constructor_prologue` paths emit `void 0` for the no-initializer case. Fixes `emitStatementsBeforeSuperCallWithDefineFields` and `override18(target=es2015)` plus a flake-cleanup elsewhere. Net +3 JS emit pass.
- **2026-04-26 12:31:44** · branch `fix/checker-lib-symbol-canonicalization-stable-key` · **Robustness audit item #4 (visibility step)** · `canonical_lib_sym_id` (`crates/tsz-checker/src/context/def_mapping.rs:299-346`) falls back to a "largest `SymbolId` wins" heuristic when neither the main binder nor the global index resolves a name. The heuristic depends on allocator behavior, not semantics. This PR doesn't replace the heuristic yet — instead it makes drift visible: clearer doc-comment explicitly labeling it a HEURISTIC (not identity), plus a `tracing::trace!` event every time it fires AND chooses a non-input `SymbolId`. A future PR can swap to a stable-key canonicalization once we have data on how often the heuristic fires. See `docs/architecture/ROBUSTNESS_AUDIT_2026-04-26.md` item #4.
- **2026-04-26 11:36:39** · branch `docs/robustness-audit-2026-04-26` · **Robustness audit (umbrella) ready** · Authored `docs/architecture/ROBUSTNESS_AUDIT_2026-04-26.md` — a 17-item catalog of silent-repair / patch-over-it anti-patterns across checker, solver, parser, LSP, and tsserver, with per-item file:line anchors, suggested small-PR fixes, and 5-wave sequencing. Each subsequent wave-1 PR (#A through #E in the audit's "Top 5") will land separately with its own claim line linking back. Pure documentation; net-zero conformance.
- **2026-04-26 11:39:09** · branch `fix/checker-speculation-doc-coherence` · **Robustness audit item #E** · Align `crates/tsz-checker/src/context/speculation.rs` module preamble with the `Snapshot`-family explicit-action semantics already documented on the type. The preamble's "Rollback-on-drop (default)" line contradicts the type's own docstring (which correctly explains the implicit-commit behavior). Pure docs change in source comments — no behavior change. See `docs/architecture/ROBUSTNESS_AUDIT_2026-04-26.md` item #5.
...
- **2026-04-26** · branch `fix/checker-ts2862-keyof-tparam-suppress` · **KPI-3 false-positive fix** · `keyofAndIndexedAccessErrors.ts` emits an extra TS2862 ("Type 'T' is generic and can only be indexed for reading") at `this.testy[key] += 1` where `key: keyof T` and `T extends Record<string, number>`. Root cause: `is_broad_index_type` (`crates/tsz-checker/src/state/state_checking/readonly.rs:681`) eagerly recurses through `evaluate_type_with_env` BEFORE the keyof-inner-type guard at line 708. For `keyof T`, the evaluation resolves the keyof through T's constraint to a primitive (`string`), then recursive `is_broad_index_type(string) = true` — bypassing the explicit "keep generic `keyof T` out of this path" intent in lines 705-707. Plan: guard the `evaluate_type_with_env` recursion at line 692 by skipping it when `type_id` contains type parameters, so generic `keyof T` reaches the keyof-inner-type check and is correctly classified as non-broad. Draft PR title: `[WIP] fix(checker): suppress TS2862 for generic keyof T index writes`.
...
- **2026-04-28 18:42:24** · branch `perf/gate-owner-symbol-result-cache` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 222 → 223 for `share_owner_symbol_type_results`, a batch-check-only gate that keeps shared owner symbol result caching out of persistent LSP/fourslash project contexts while preserving the large-repo ProjectEnv path. Follow-up to #1689 in PR #1691.
- **2026-05-05** · branch `fix/destructuring-tuple-conformance` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 223 → 224 for `no_overload_call_nodes`, a speculation-scoped marker set used to reject outer overload candidates when callback bodies contain nested TS2769 failures that are later rolled back or suppressed for recovery. The state is snapshotted and restored by `context/speculation.rs` alongside diagnostic dedup state.
- **2026-05-05** · branch `fix/checker-unknown-control-flow-fingerprints` · **Workstream-4 / architecture guard baseline** · Re-pinned the live `CheckerContext` field count cap 224 → 225 and added `types/computation/access.rs` to the current >2000 LOC exclusion set so the pre-commit architecture guard reflects `origin/main` before the unknown-control-flow fingerprint fix. Follow-up work should still reduce both metrics rather than grow them.
- **2026-05-06** · branch `fix/large-ts-repo-namespace-exports-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 225 → 226 for `namespace_exports_cache`, a per-checker cache that avoids repeated cross-file namespace export scans in large repositories. Follow-up work should still extract ambient checker caches into grouped state rather than continuing to grow `CheckerContext`.
- **2026-05-06** · branch `fix/large-ts-repo-namespace-exports-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for `nested_namespace_candidates_cache`, a per-checker cache that avoids repeated nested namespace candidate scans while resolving many members from the same nested namespace.
- **2026-05-06** · branch `fix/large-ts-repo-namespace-exports-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 229 after rebasing the namespace export cache lane across the current checker-cache baseline. Follow-up work should still group checker cache state instead of continuing to grow `CheckerContext`.
- **2026-05-06** · branch `fix/large-ts-repo-namespace-exports-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 229 → 230 for the merged namespace export cache branch state. Follow-up work should still group checker cache state instead of continuing to grow `CheckerContext`.
- **2026-05-08 00:06:48** · branch `fix/jsdoc-enum-circular-stack-overflow-3767` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 230 → 231 for `jsdoc_enum_resolution_set`, an `FxHashSet<SymbolId>` that breaks the `@enum {E}` → name-resolution → `jsdoc_enum_annotation_type_for_symbol_decl` recursion that previously stack-overflowed (#3767). Follow-up work should still consolidate checker context state.
- **2026-05-06** · branch `fix/large-ts-repo-nested-namespace-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for `nested_namespace_candidates_cache`, a per-checker cache that avoids repeated nested namespace candidate scans in large repositories. Follow-up work should still extract related namespace caches into grouped state rather than continuing to grow `CheckerContext`.

- **2026-05-05** · branch `fix/checker-destructuring-tuple-diagnostics` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 224 → 225 for `preserve_destructuring_initializer_overload_diagnostics`, a scoped flag that keeps nested overload diagnostics visible while destructuring iterability diagnostics recompute non-literal initializers.
- **2026-05-06** · branch `codex/follow-up-ci-conformance-20260506` · **Workstream-4 / architecture guard baseline** · Re-pinned the `CheckerContext` field count cap 225 → 226 and the emitter file-size ratchet largest-file baseline 7531 → 7932 after the open-PR main-merge sweep. These bumps keep the guardrails honest for the aggregate cleanup lane; follow-up work should reduce the metrics rather than grow them further.
- **2026-05-06** · branch `fix-dts-next-011599` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for PR #4227's declaration emit support state. This makes the drift explicit while follow-up work continues to reduce `CheckerContext` coordination state.
- **2026-05-06** · branch `fix/export-equals-stack-growth-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 229 for the current branch state after the export-equals resolver stack growth work. This is an intentional guard bump for the CI lint gate; follow-up work should reduce the metric rather than grow it further.
- **2026-05-06** · branch `fix/js-emit-next-122` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 228 for the current branch state after the recovered module-helper emitter work. This is an intentional guard bump for the CI lint gate; follow-up work should reduce the metric rather than grow it further.
- **2026-05-06** · branch `fix/export-equals-miss-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 to match the current checker context field baseline while keeping the export-equals miss-cache PR CI-visible.
- **2026-05-06** · branch `fix/export-equals-miss-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 after the PR branch merge-base advanced and brought in one additional checker context field; this keeps the guard aligned while the export-equals miss cache fix runs CI.
- **2026-05-06** · branch `fix-dts-next-011597` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for the reused structural call return declaration fix in PR #4211. This is an intentional lint-guard bump for the live branch state; follow-up work should still split `CheckerContext` state by lifetime rather than keep growing the coordination object.
- **2026-05-06** · branch `fix/js-emit-next-135` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for the parser arrow-recovery branch's current lint baseline. This is an intentional guard bump for the live branch state; follow-up work should still split `CheckerContext` state by lifetime rather than keep growing the coordination object.
- **2026-05-06** · branch `fix/symbol-name-candidate-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for `symbol_name_candidates_cache`, a per-checker cache that avoids repeated same-name symbol scans across the current binder and cross-file binders.
- **2026-05-06** · branch `fix/js-emit-next-135` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for the parser recovery context carried by the arrow recovery close-brace branch.
- **2026-05-06** · branch `fix/issue-4095-symbol-method-types` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for the symbol primitive method typing lane. This is an intentional guard bump for PR #4230; follow-up work should still consolidate checker context state rather than grow the struct further.
- **2026-05-06** · branch `fix/issue-4095-symbol-method-types` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 after the current-base #4230 integration. This keeps the architecture guard aligned with the live branch while the symbol primitive method typing fix proceeds through CI; follow-up work should still consolidate checker context state rather than grow the struct further.
- **2026-05-06** · branch `fix/namespace-exports-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 230 for PR #4220's namespace export cache state. This keeps the perf cache drift explicit while follow-up work continues to split cross-file module/export state out of `CheckerContext`.
- **2026-05-06** · branch `fix-dts-next-011592` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for the current branch state after the constrained string lookup key emitter work. This is an intentional guard bump for the CI lint gate; follow-up work should reduce the metric rather than grow it further.
- **2026-05-06** · branch `chore/refresh-readme-stats-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for the positional no-emit file check fix after CI lint reported the branch-local field count. Follow-up work should group checker state rather than continuing to grow `CheckerContext`.
- **2026-05-06** · branch `fix-dts-next-011579` · **Workstream-4 / architecture guard baseline** · Re-pinned the emitter file-size ratchet largest-file baseline 8109 → 8295 after PR #3979 intentionally added const-assertion declaration inference logic to `declaration_emitter/helpers/type_inference.rs`. Follow-up emitter cleanup should split helpers out of this file and ratchet the ceiling back down.
- **2026-05-06** · branch `fix-20260506-194427` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for the current branch state after the assignment-rest lowering emitter work. This is an intentional guard bump for the CI lint gate; follow-up work should reduce the metric rather than grow it further.
- **2026-05-06** · branch `fix-dts-next-011600` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for current checker context state after the bare-let declaration emit fix branch merged current checker-side cache work.
- **2026-05-06** · branch `fix/issue-3749-cross-file-references` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for the cross-file references lane. This is an intentional guard bump for PR #4240; follow-up work should still consolidate checker context state rather than grow the struct further.

- **2026-05-06** · branch `fix/symbol-name-candidate-cache-20260506` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 226 → 227 for `symbol_name_candidates_cache`, a per-checker cache that avoids repeated same-name symbol scans across the current binder and cross-file binders.
- **2026-05-06** · branch `fix-dts-next-011599` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for the unique symbol conditional return preservation fix after CI lint reported the branch-local field count. Follow-up work should group checker state rather than continuing to grow `CheckerContext`.
- **2026-05-06** · branch `fix/issue-4094-inherited-root-out-dir` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for inherited root/outDir option tracking carried by this branch.
- **2026-05-06** · branch `fix-dts-next-011597` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for the reused structural call return formatting fix after CI lint reported the branch-local field count. Follow-up work should group checker state rather than continuing to grow `CheckerContext`.
- **2026-05-06** · branch `fix/js-emit-next-123` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for the object rest assignment helper fix after CI lint reported the branch-local field count. Follow-up work should group checker state rather than continuing to grow `CheckerContext`.
- **2026-05-06** · branch `fix/js-emit-next-139` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for deferred enum export folding state carried by this branch.
- **2026-05-06** · branch `fix/issue-4135-public-break-recovery` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for public modifier break recovery parser/checker state carried by this branch.
- **2026-05-06** · branch `fix-dts-next-011592` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for the constrained string lookup key preservation fix after CI lint reported the branch-local field count. Follow-up work should group checker state rather than continuing to grow `CheckerContext`.
- **2026-05-06** · branch `fix/emit-namespace-baseline-4079-agent` · **Workstream-4 / architecture guard baseline** · Re-pinned the live `CheckerContext` field count cap 227 → 229 after CI reported the current merged checker context at 229 fields while isolating the namespace emit baseline revert. Follow-up work should reduce this metric rather than grow it.
- **2026-05-06** · branch `fix/conformance-20260506-201456` · **Workstream-4 / architecture guard baseline** · Re-pinned the live `CheckerContext` field count cap 227 → 228 after the current `origin/main` baseline exceeded the guard before this checked-JS JSDoc conformance slice. This PR does not add a `CheckerContext` field; follow-up work should still reduce the metric rather than grow it.
- **2026-05-06** · branch `fix/issue-4092-positional-noemit-checks` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap is preserved at the live 229 baseline for the positional noEmit checking lane after rebasing onto the current context-cache baseline. This is an intentional guard update for PR #4225; follow-up work should still consolidate checker context state rather than grow the struct further.
- **2026-05-06** · branch `fix/issue-4128-reexport-augmentation-members` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for the nested augmentation self-reference fix after CI lint reported the branch-local field count. Follow-up work should group checker state rather than continuing to grow `CheckerContext`.
- **2026-05-06** · branch `claude/fix-module-resolution-diagnostics-pp5UK` · **Workstream-4 / architecture guard baseline** · Re-pinned the live `CheckerContext` field count cap at 229 after merging the current context-cache baseline into the module-resolution diagnostics fix. This PR does not add a new `CheckerContext` field; follow-up work should still reduce the metric rather than grow it further.
- **2026-05-06** · branch `fix/issue-3913-showconfig-relative-paths` · **Workstream-4 / architecture guard baseline** · Re-pinned the live `CheckerContext` field count cap at 229 after merging the current context-cache baseline into the showConfig relative-path fix. This PR does not add a new `CheckerContext` field; follow-up work should still reduce the metric rather than grow it further.
- **2026-05-06** · branch `fix/issue-4091-compose-inference` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 227 → 228 for the nested call diagnostic preservation fix after CI lint reported the branch-local field count. Follow-up work should group checker state rather than continuing to grow `CheckerContext`.
...
- **2026-05-06** · branch `fix/conformance-20260506-184728` · **Workstream-4 / architecture guard baseline** · Re-pinned the live `CheckerContext` field count cap 227 → 229 after the current `origin/main` baseline exceeded the guard before this class-static-block TS1210 conformance slice. This PR does not add a `CheckerContext` field; follow-up work should still reduce the metric rather than grow it.

- **2026-05-06** · branch `fix/zod-number-isinteger-context` · **Workstream-4 / architecture guard baseline** · Re-pinned the live `CheckerContext` field count cap at 229 after merging the current context-cache baseline into the Zod number predicate fix. This PR does not add a new `CheckerContext` field; follow-up work should still reduce the metric rather than grow it further.
- **2026-05-06** · branch `fix/issue-4065-jsdoc-this-arrow` · **Workstream-4 / architecture guard baseline** · Re-pinned the live `CheckerContext` field count cap 227 → 229 after the latest main updates added checker context caches without moving the guard baseline. This PR does not add a `CheckerContext` field; follow-up work should still reduce the metric rather than grow it further.
...
- **2026-04-27** · Architecture-health-metric-3 cap bump 36 → 37: PR #1574 added `use tsz_solver::recursion::RecursionResult` to `crates/tsz-emitter/src/declaration_emitter/helpers/function_analysis.rs` (RecursionGuard-threaded identity-call traversal) without bumping the cap. Bumped in `scripts/arch/arch_guard.py` SOLVER_IMPORT_COUNT_CHECKS as part of the follow-up guard-threading fix (PR for the mutual-recursion cycle). No new boundary violations introduced.
- **2026-05-06** · Architecture-health-metric-3 cap bump 37 → 38: PR #3470 added one direct `tsz_solver` import from declaration emit helpers while preserving generic call literals in declarations. Bumped `SOLVER_IMPORT_COUNT_CHECKS` in `scripts/arch/arch_guard.py` with this PR so the intentional boundary drift remains visible for the workstream-3 cleanup queue.
...
Source: `README.md` on 2026-05-02.

| Surface | Current |
| --- | ---: |
| Diagnostic conformance | `98.1%` (`12,344 / 12,582`) |
...
6. Current `main` sample after #2204/#2211/#2209 (local 2026-05-02, 16 GB host, 75% physical-footprint guard): the same guarded command plateaued for more than a minute below the 12.29 GB guard and was manually stopped with exit 143. Peak sampled physical footprint was ~11.58 GB. The fixture still does not finish quickly enough on this host, but the current failure mode remains bounded runtime rather than runaway RSS or the prior sort panic.
7. Implication: the first-pass Arc-sharing of the known binder maps (`node_symbols`, `node_flow`, `node_scope_ids`, `top_level_flow`, `switch_clause_to_switch`) has landed; continue measuring remaining large-repo work as targeted residency/runtime slices, and prefer stable-identity/skeleton consumers over more per-file binder state.
8. Bench harness caveat: `tsz: TIMEOUT` in the table can mean either timer-kill (124), manual stop (143), or OS-kill (137). The 137 case ("OOM-by-paging") was the dominant historical failure mode. Inspect exit codes when investigating.
