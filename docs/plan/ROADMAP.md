...
- **2026-04-25** · branch `fix/checker-ts2322-elaboration-chain` · **Workstream 1 active claim** · Wire TS2322 multi-line elaboration chain in the generic `render_type_mismatch` path. `crates/tsz-checker/src/error_reporter/render_failure/type_mismatch.rs::render_type_mismatch` currently emits a single-line `Type 'X' is not assignable to type 'Y'.` and discards its `_reason: &SubtypeFailureReason` argument. tsc's baseline for TS2322 routinely includes nested `Type 'X' is not assignable to type 'Y'.` lines via diagnostic related-information (e.g. booleanAssignment, inDoesNotOperateOnPrimitiveTypes, intersectionsAndOptionalProperties, recursiveFunctionTypes). PropertyTypeMismatch and ReturnTypeMismatch in `render_failure.rs` already implement the nested-reason walk; the generic path lags. Plan: walk nested `SubtypeFailureReason`s and push `Diagnostic::related_information` entries with appropriate indentation, mirroring the existing pattern. Estimated impact: 60–80 of 144 fingerprint-only TS2322 tests. Draft PR title: `[WIP] fix(checker): emit TS2322 elaboration chain via related_information`.
