...
- **2026-04-26** · branch `chore/checker-tests-migrate-value-usage` · **DRY workstream-1 claim** · workstream 1 ("Test harness consolidation"): migrate `crates/tsz-checker/tests/value_usage_tests.rs` (1384 LOC) — replace 33 inline 13-line `ParserState::new` → `BinderState::new` → `CheckerState::new` boilerplate blocks (one per test) with single-line calls to `crate::test_utils::check_source_diagnostics(source)` (or `check_source(source, "test.js", { check_js: true, ..default })` for the one JS-specific test). 31 of 33 sites transformed via a Python regex over the canonical block; 2 manually touched (one JS-options test, one missing-blank-line variant). 41 multi-line `checker.ctx.diagnostics` chained accesses also rewritten to `diags`. Net -685 LOC; drops `tsz_binder::BinderState`, `tsz_parser::parser::ParserState`, `tsz_solver::TypeInterner`, and `crate::state::CheckerState` imports. Distinct from prior migration PRs and the still-WIP `claude/modest-archimedes-QBaXl` claim. Draft PR: `[WIP] chore(checker/tests): migrate value_usage_tests to test_utils`.
