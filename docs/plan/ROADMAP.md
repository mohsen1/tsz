...
- **2026-04-25** · branch `chore/checker-context-field-count-guard` · **Workstream-4 / architecture-health-metric-1 claim** · Operating Principle 8 ("New architecture metrics should be cheap to count repeatedly so drift is visible") + Architecture Health Metric 1 ("`CheckerContext` field count"): the current count is 211 fields (regex against `crates/tsz-checker/src/context/mod.rs`'s `pub struct CheckerContext` body). Add a `STRUCT_FIELD_COUNT_CHECKS` list + `scan_struct_field_count` function in `scripts/arch/arch_guard.py` that pins the count, mirroring the existing `FILE_LINE_LIMIT_CHECKS` convention (e.g. `tsz-core/lib.rs` is capped at 2420 LOC). Future field additions require an explicit cap bump in the same PR, making drift visible in the diff. Draft PR: `[WIP] chore(arch): pin CheckerContext field count in arch_guard`.
