...
- **2026-04-25** · branch `chore/parser-test-fixture` · **DRY workstream-8.9 claim** · workstream 8 item 9 ("Create parser/scanner/binder/lowering fixtures"): 6 parser test files (`decorator_tests.rs`, `parser_unit_tests.rs`, `state_expression_tests.rs`, `state_statement_tests.rs`, `state_declaration_tests.rs`, `state_type_tests.rs`) define byte-for-byte identical `parse_source(&str) -> (ParserState, NodeIndex)` helpers. `state_type_tests.rs` also defines a `parse_source_named` variant. Extract into `crates/tsz-parser/tests/test_fixture.rs`, wire as a `#[path]` mod under `parser/mod.rs`, migrate the 6 duplicate sites. Pure refactor — preserve test behavior. Draft PR: `[WIP] chore(parser/tests): extract shared parse_source fixture; remove 6 duplicate helpers`.
