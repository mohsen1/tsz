...
- **2026-04-26** · branch `chore/binder-validation-tests` · **WIP — Test coverage** · Lock untested behavior of `crates/tsz-binder/src/binding/validation.rs` (329 LOC, previously 0 unit tests) covering the post-binding validation, lib-symbol diagnostics, and resolution-statistics helpers. Plan: add a colocated tests file mounted via the existing `#[cfg(test)] #[path = "../../tests/binding_validation.rs"] mod tests;` pattern (matches `lib_loader.rs`, `module_resolution_debug.rs`). Coverage targets: `validate_symbol_table` (clean state, BrokenSymbolLink, OrphanedSymbol, InvalidValueDeclaration, multi-error merging), `is_symbol_table_valid` (true/false complement), `validate_global_symbols` (empty binder reports all expected missing; lib-merged binder reports none missing; partial merge reports exact missing list), `get_lib_symbol_report` (header text, present/missing counts, lib-binder contribution lines), `verify_lib_symbol_merge` (accessible via file_locals, accessible via lib_binders, inaccessible empty lib, all empty lib edge case), `get_resolution_stats` + `get_resolution_summary` (zero-state, multi-tier counts). Pure additive — no production change.
