...
- **2026-04-26** · branch `chore/checker-tests-migrate-js-grammar-span` · **DRY workstream-1 claim** · workstream 1 ("Test harness consolidation"): migrate `crates/tsz-checker/tests/js_grammar_span_tests.rs` (160 LOC) — replace local 24-line `check_source(source, file_name, options) -> Vec<Diagnostic>` wrapper that is byte-identical to `tsz_checker::test_utils::check_source` (same `set_lib_contexts(Vec::new())` step + same 3-arg signature) with a direct import. 4 call sites; pure rename via import. Distinct from PR #1253 (5 other files), PR #1267 (`type_arg_count_mismatch_tests.rs`), PR #1270 (`spread_rest_tests.rs`), and the still-WIP `claude/modest-archimedes-QBaXl` claim. Draft PR: `[WIP] chore(checker/tests): migrate js_grammar_span_tests to test_utils::check_source`.
