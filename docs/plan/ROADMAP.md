...
- **2026-04-25** · branch `chore/orchestration-path-count-guard` · **Workstream-3 / architecture-health-metric-4 claim** · Operating Principle 8 + Architecture Health Metric 4 ("Independent parse-bind-check orchestration paths"): there are currently 4 files in `crates/tsz-cli/`, `crates/tsz-core/`, `crates/tsz-lsp/`, `crates/tsz-wasm/` (excluding tests) that construct all three of `ParserState::new`, `BinderState::new`, `CheckerState::new` — i.e., independent end-to-end pipelines: `tsz_server/check.rs`, `tsz-core/parallel/core.rs`, `tsz-lsp/signature_help.rs`, `tsz-lsp/project/core.rs`. Workstream 3 ("Compiler Service Front Door") exit criterion is "There is one blessed parse-bind-check path." Pin the count at 4 in `arch_guard.py` so any *new* independent pipeline shows up as guard failure (and any consolidation shows up as a cap reduction in the same PR). Draft PR: `[WIP] chore(arch): pin independent parse-bind-check orchestration path count`.
