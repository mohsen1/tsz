...
- **2026-04-26** · branch `chore/solver-tests-judge-setup-helper` · **DRY active claim** · Solver tests DRY: extract a `JudgeSetup` fixture/helper in `crates/tsz-solver/tests/common/mod.rs` that bundles `(TypeInterner, TypeEnvironment, DefaultJudge)` and migrate the ~65 duplicated 3-line "interner / env / judge::with_defaults" stanzas in `crates/tsz-solver/tests/judge_tests.rs`. Pure refactor; behavior preserving; verified via `cargo nextest run -p tsz-solver`.
