...
- **2026-04-26** · branch `chore/common-linemap-span-range-helpers` · **DRY workstream-5 active claim** · workstream 8 item 5 ("AST traversal and span semantics: Standardize byte span, UTF-16 LSP range, and diagnostic start/length conversions"): `LineMap` exposes `offset_to_position` / `position_to_offset` but no canonical `Span -> Range` / `Range -> Span` pair. `tsz-core::SourceFile::span_to_range` and `range_to_span` open-code the start+end pair build, and 30+ caller sites in tsz-cli/tsz-lsp repeat the `let start_pos = line_map.offset_to_position(start, source); let end_pos = line_map.offset_to_position(end, source); Range::new(start_pos, end_pos)` triple inline. Add `LineMap::span_to_range(&self, span: Span, source: &str) -> Range` and `LineMap::range_to_span(&self, range: Range, source: &str) -> Option<Span>` to `tsz-common::position`, with matrix tests covering ASCII, multi-byte UTF-8, surrogate pairs, dummy spans, and start>=end edge cases. Pure additive contract. Caller migration in tsz-cli / tsz-lsp / tsz-core happens in follow-up PRs (those crates are blocked by other in-flight work this round).
