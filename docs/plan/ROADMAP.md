...
- **2026-04-26** · branch `chore/symbol-primary-declaration-migration` · **DRY workstream-8.6 claim** · workstream 8 item 6 ("Recount and continue Symbol::primary_declaration() migration across checker, LSP, CLI, and binder"): two checker sites manually re-implement `Symbol::primary_declaration()` semantics (`crates/tsz-checker/src/types/property_access_helpers/access_semantics.rs:1095-1103` and `crates/tsz-checker/src/types/queries/callable_truthiness.rs:309-318`). Both fall back from `value_declaration` to a `declarations.iter().find(|idx| idx.is_some())` lookup — the exact body of the existing `Symbol::primary_declaration()`. Migrate the two sites to call `primary_declaration()?`, saving ~17 lines of duplicated logic and concentrating the value-vs-type declaration semantics. Pure refactor. Draft PR: `[WIP] chore(checker): migrate two manual primary_declaration patterns to Symbol::primary_declaration()`.
