...
- **2026-04-26** · branch `chore/checker-tests-migrate-type-arg-count-mismatch` · **DRY workstream-1 claim** · workstream 1 ("Test harness consolidation"): migrate `crates/tsz-checker/tests/type_arg_count_mismatch_tests.rs` from a local 21-line `diagnostic_codes` wrapper (defining the canonical `ParserState::new` → `BinderState::new` → `CheckerState::new` boilerplate) to `tsz_checker::test_utils::check_source_codes`. 7 call sites; same `Vec<u32>` return shape so call sites change only via import. Distinct from PR #1253 (which migrated 5 other test files) and the still-WIP `claude/modest-archimedes-QBaXl` claim. Draft PR: `[WIP] chore(checker/tests): migrate type_arg_count_mismatch_tests to test_utils::check_source_codes`.
