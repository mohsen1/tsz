...
- **2026-04-25** · branch `fix/parser-incremental-interner-coherence` · **Workstream 7 — ready for review** · Fix incremental parse interner identity drift in `ParserState::parse_source_file_statements_from_offset`. PR #1205.
- **2026-04-25** · branch `fix/checker-ts2322-elaboration-chain` · **Workstream 1 — research/pivot, abandoned** · Initial scope (multi-line elaboration in `render_type_mismatch`) was invalidated by smoke-testing `booleanAssignment.ts`: tsc's `DiagnosticFingerprint.message_key` only captures the first line of a TS2322 diagnostic, so missing `related_information` lines do **not** explain fingerprint-only failures. The real gap surfaced by the smoke is **wrong source-type display**: tsz emits `Type 'Boolean' is not assignable to type 'Boolean'.` and `Type 'Symbol' is not assignable to type 'Boolean'.` where tsc expects `Type 'number' is not assignable to type 'Boolean'.` and `Type 'string' is not assignable to type 'Boolean'.`. This is a bug in `format_assignment_source_type_for_diagnostic` (`crates/tsz-checker/src/error_reporter/core/diagnostic_source.rs:1077`) or one of its branches, where the source type display is being mis-derived for primitive-to-wrapper-class assignments. Lesson for future agents: don't trust offline-only research that infers fingerprint deltas from baseline elaboration text — verify with `./scripts/conformance/conformance.sh run --filter <name> --verbose` before scoping printer fixes. Closing PR #1214 without merging; next iteration picks a different concrete target (KPI 2 crashes are well-bounded).
