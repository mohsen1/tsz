...
- **2026-04-26** · branch `chore/cli-locale-edge-tests` · **Coverage active claim** · Add edge-case unit tests for the `crates/tsz-cli/src/localization/locale.rs` private helpers and locale loader. Existing `crates/tsz-cli/src/locale_tests.rs` covers happy paths but skips: `extract_quoted_strings` corner cases (empty input, no quotes, unterminated quote, empty quotes `''`, multibyte content inside quotes, leading-quote in input), `substitute_params_from_english` edge cases (out-of-order placeholders `{1}` before `{0}`, repeated `{0}` in template, more english params than template needs, empty english), `parse_locale_json` shape edge cases (empty object, null/numeric/array values mixed, key without trailing code, key shaped like a code-only number), `extract_code_from_key` (empty, no underscore, trailing underscore, code-only `12345`), `normalize_locale` (empty, whitespace, unsupported `xx`, mixed-case region tag), and `LocaleMessages::load` (unknown locale returns `None`). Pure additive tests; behavior locks; no production code changes.
