...
- **2026-04-26** · branch `chore/solver-deep-reduce-display-tests` · **Test coverage WIP** · Coverage claim for `tsz_solver::deep_reduce_for_display` (`crates/tsz-solver/src/diagnostics/reduce.rs`, 150 LOC, 0 unit tests today; consumed by `tsz_checker::query_boundaries::common::deep_reduce_for_display` and `class_abstract_checker.rs` for heritage-display reduction). Plan: add `crates/tsz-solver/tests/deep_reduce_tests.rs` mounted via `#[cfg(test)] #[path = "../tests/deep_reduce_tests.rs"]` from `diagnostics/reduce.rs`, locking the documented contract: intrinsics return identity, `Application`/`Conditional` leaves that fully reduce are replaced (with `NoopResolver` they don't, so they stay verbatim — guarding against accidental over-reduction), structural composites (`Union`, `Intersection`, `Object`, `ObjectWithIndex`) recurse and only re-intern when a child changes, recursion respects the `visited` cycle guard, and `TypeData::Lazy` post-reduction is treated as non-reducible (the `match db.lookup(reduced)` arm). Pure additive — no production change.
