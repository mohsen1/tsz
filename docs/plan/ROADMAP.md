...
- **2026-04-26** · branch `chore/emitter-comments-core-tests` · **Test coverage active claim** · Add unit tests for `crates/tsz-emitter/src/emitter/comments/core.rs` (231 LOC, currently 0 inline tests, 0 integration tests for the two pure public functions `get_leading_comment_ranges` and `get_trailing_comment_ranges` — `tests/comment_tests.rs` only exercises Printer-level integration paths). Following the model of PR #1257 (`import_usage.rs` test coverage). Pure additive: no production-code change. Target 20-30 unit tests covering: empty input, no-comment positions, single/multi-line comments, leading vs trailing semantics, shebang skip, multi-comment sequences, `\r\n` line ending, multi-byte UTF-8 (BMP + surrogate-pair) inside comment bodies, position-out-of-bounds clamps, unterminated multi-line comment EOF behavior, the `has_trailing_newline` flag on both leading and trailing variants, and `CommentKind` discrimination.
