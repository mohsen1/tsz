...
- **2026-04-26** · branch `chore/parser-defensive-identifier-text-fallback` · **Workstream-7 claim** · Workstream 7 ("Incremental Parser/Interner Coherence") deliverable 3: "Add a defensive identifier text resolution path only if it is consistent with the parser identity model." `NodeArena::resolve_identifier_text` currently returns `interner.resolve(atom)` directly — if the interner is stale (the bug fixed in PR #1205), this returns `""` instead of falling back to `IdentifierData.escaped_text`. Fall back to `escaped_text` when the interner returns empty for a non-`NONE` atom. Defends against future interner-coherence regressions without changing happy-path behavior (interner returns the canonical text). Add a unit test that constructs an arena with an interner missing the atom and asserts the fallback. Draft PR: `[WIP] fix(parser): fall back to escaped_text when interner returns empty for a set atom`.
