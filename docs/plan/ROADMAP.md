...
- **2026-04-25** · branch `fix/incremental-parser-interner-coherence` · **Workstream-7 claim** · Incremental parser/interner coherence (workstream 7 deliverables 1 and 2): `parse_source_file_statements_from_offset` parses a suffix into the existing arena via `scanner.set_text(...)` but never propagates the scanner's updated interner back to `arena.interner()`. New identifiers introduced after offset 0 land in the scanner interner only; arena lookups via `resolve_identifier_text` fall through to `escaped_text` instead of the canonical interner string. Fix: mirror the `set_interner` call from `parse_source_file` at the end of the offset variant. Add a regression test where the initial parse interns no identifiers and the incremental suffix introduces a fresh one. Draft PR title: `[WIP] fix(parser): propagate scanner interner to arena after incremental parse`.
