...
- **2026-04-26** · branch `chore/binder-export-surface-tests` · **Coverage WIP** · Workstream-8 ("Lock untested public-API helpers behind unit tests"): `crates/tsz-binder/src/state/export_surface.rs` (404 LOC, 1 inline test) exposes `ExportSurface` query methods (`is_exported`, `is_type_only_export`, `exported_names`, `has_overloads`, `symbol_for_export`, `public_api_size`) consumed by 18+ callers across tsz-checker / tsz-cli / tsz-core / tsz-lsp. Plan: add ~20 inline tests in the existing `mod tests` block covering each query method's empty-state, single-population, both-populations, dedup, sort, and ambiguous-overlap behavior (including `public_api_size` not double-counting names present in both `module_exports` and `file_exported_locals`). Pure additive; no production change.
