...
- **2026-04-25** · branch `fix/checker-ts1238-generic-decorator-call` · **KPI-2 partial-land** · Targeted `decoratorCallGeneric.ts` (TS1238). Smoke confirmed CLI emits 0; `check_class_decorator_call_signature` is wired (`crates/tsz-checker/src/state/state_checking/class.rs:1447`). **Surprising finding**: a unit test that runs `check_source_codes_experimental_decorators` on the same source (with and without `target=ES2015`) PASSES — i.e. the checker DOES emit TS1238 when invoked through the test harness. The dist-fast CLI binary, however, emits zero diagnostics on the same source even with explicit `--experimentalDecorators --target es2015 --noEmit` flags. The discrepancy is somewhere in the CLI's per-file checker invocation flow under `crates/tsz-cli/src/driver/`. Landing the regression test as a checker behavior lock (passes today; serves as a tight reproducer for the CLI-side investigation). Future-iteration target: trace `args.experimental_decorators -> options.checker.experimental_decorators -> per-file CheckerState` in the CLI driver and find why the check is silently elided in production.
