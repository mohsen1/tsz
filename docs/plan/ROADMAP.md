...
- **2026-04-25** · branch `fix/emitter-cjs-module-temp-name-collision` · **DRY §8 bug-shaped finding active claim** · Fix `tsz-emitter` `next_commonjs_module_var` so it skips suffixes that collide with `file_identifiers` or `generated_temp_names`. Without this, an `import …` whose synthetic temp would be `foo_1` was emitted as-is even when the source has a user-declared `foo_1`, producing invalid JS (`var foo_1` followed by `const foo_1 = require(…)` is a redeclaration error). Regression test in `crates/tsz-emitter/tests/printer.rs` covers the collision case. Draft PR title: `[WIP] fix(emitter): skip colliding suffixes in CJS module temp naming`.
