...
- **2026-04-25** · branch `fix/checker-ts1238-generic-decorator-call` · **KPI-2 partial-land + bisect inconclusive** · Targeted `decoratorCallGeneric.ts` (TS1238). Smoke confirmed dist-fast CLI emits 0 at origin/main HEAD; the checker test harness emits TS1238 correctly. Unit-test PR #1220 is landing as a behavior lock. **Bisect attempt**: rebuilt and tested at `a3b1c44efc`, `f0e5fa9a60`, `3bee494013`, `eea5ec3c7c` — all emit 0. The bug has been present across 4+ recent origin/main checkpoints. Earlier hypothesis (regression in last ~20 commits) was based on an outlier `.cache/tsz-target/dist-fast/tsz` binary at 21:18 that emits TS1238 but cannot be reproduced from any tested commit. The outlier binary was likely built from a different worktree state (possibly fresh-session with my parser fix on top, OR with uncommitted local solver changes in the main repo) — its provenance is no longer traceable. **Working hypothesis revised**: TS1238 for `decoratorCallGeneric.ts` is a long-standing CLI gap in tsz, not a recent regression. The unit-test path emits it correctly because it exercises `check_class_decorator_call_signature` directly; the CLI path either reaches the function with different state (e.g. evaluated decorator type degenerates to ANY/ERROR) OR doesn't reach it at all (e.g. `experimental_decorators` reads false through some option-flow nuance). Next-iteration target: instrument the CLI's per-file `CheckerState` construction to log `experimental_decorators` and the early-return short-circuits inside `check_class_decorator_call_signature` (ERROR/ANY/UNKNOWN guards), build, and observe which gate the CLI invocation hits.
