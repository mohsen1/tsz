...
- **2026-04-25** · branch `fix/checker-ts1238-generic-decorator-call` · **KPI-2 partial-land + ROOT CAUSE PINPOINTED** · Targeted `decoratorCallGeneric.ts` (TS1238). Unit-test PR #1220 ships as behavior lock. **Instrumentation revealed the precise gap**: in `check_class_decorator_call_signature` (`crates/tsz-checker/src/state/state_checking/class.rs:1447`), every early-return guard passes for both unit-test and CLI invocations. `decorator_type` is non-trivial in both. `has_call_signatures` is true. `class_constructor_type` is non-error. They diverge at the final step — `resolve_call_with_checker_adapter`. Captured DBG output: \n\n  - **Unit test**: `resolve_call_result: ArgumentTypeMismatch { index: 0, expected: TypeId(153), actual: TypeId(121), fallback_return: TypeId(5) }` → emits TS1238 ✓\n  - **CLI**: `resolve_call_result: Success(TypeId(5))` → no emit ✗\n\n  Same source. TypeIds differ (CLI: 1171/2543; unit test: 160/121) — CLI's much-larger type universe (lib loading) suggests the `dec` function type is being computed/instantiated differently along the way. Most likely the CLI path's `compute_type_of_node(decorator.expression)` returns a partly- or fully-instantiated form (e.g. `(c: I<unknown>) => void`) whereas the unit-test path returns the polymorphic generic that `resolve_call` then re-infers. With T already resolved to `unknown`/`any`, `typeof C` IS structurally assignable to `I<unknown>` — bypassing the constraint failure. **Concrete fix candidates**: (a) in `check_class_decorator_call_signature`, ensure `decorator_type` is the un-instantiated generic before passing to `resolve_call`; (b) in solver's `resolve_call`, re-validate generic constraints even when the function type appears already-instantiated. Path (a) is more local; (b) is more correct. Future iteration: dump TypeData for both TypeId universes to confirm the instantiation hypothesis, then implement the fix and verify with both unit test and conformance smoke.
