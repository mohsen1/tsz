...
- **2026-05-05** · branch `fix/main-arch-guard-drift` · **Workstream-4 / architecture-health-metric-1 cap bump** · `CheckerContext` field count cap 224 → 225 to record the current checker context size after the conformance PR batch. No behavior change; this keeps the architecture guard green while preserving visible metric drift for follow-up extraction work.
