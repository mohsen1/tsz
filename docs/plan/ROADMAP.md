...
- **2026-04-26** · branch `chore/lsp-minimal-provider-context` · **DRY workstream-2 follow-up to #1244** · Adds `LspMinimalProviderContext<'a> { arena, line_map, source_text }` alongside the existing `LspProviderContext` (binder-tier). Extends `define_lsp_provider!(minimal …)` macro arm with a `from_context` constructor and adds `ProjectFile::minimal_provider_context()` accessor. Migrates the 5 minimal-tier construction sites in `project/features.rs` (`DocumentSymbolProvider`, `FoldingRangeProvider`, `SelectionRangeProvider`, `DocumentLinkProvider`, `LinkedEditingProvider`) from the 5-line `::new(file.arena(), file.line_map(), file.source_text())` shape to the single-line `::from_context(file.minimal_provider_context())` form. `DocumentColorProvider` is hand-rolled (not macro-defined) and intentionally deferred. 3715 LSP tests pass; pure refactor. Draft PR title: `[WIP] chore(lsp): add LspMinimalProviderContext + minimal-tier from_context constructor`.
