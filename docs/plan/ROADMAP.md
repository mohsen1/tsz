...
- **2026-04-26** · branch `chore/scripts-print-truncated-more-helper` · **DRY workstream-10 active claim** · workstream 10 item 3 ("Keep conformance/fourslash snapshot query helpers single-source"): `scripts/fourslash/query-fourslash.py` and `scripts/emit/query-emit.py` repeat the truncation-tail pattern `if len(items) > top: print(f"  ... and {len(items) - top} more")` 9 times (5 in `query-fourslash.py`, 4 in `query-emit.py`). Add `print_truncated_more(items, top, indent="  ")` to the existing `scripts/lib/query_snapshot.py` shared helper module and migrate all 9 sites. Add unit tests covering boundary, empty-tail, and custom-indent cases in `scripts/lib/test_query_snapshot.py`. Pure refactor; behavior preserved (byte-identical output).
