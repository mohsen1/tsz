...
>
> **Bind-merge update 2026-04-29**: compacting file skeleton heritage metadata and parallelizing skeleton extraction for large programs removes the sampled `extract_skeleton` foreground hotspot, but does not fix the full-large run. A 5s live sample at ~102s on the validated 1.29M LOC fixture was still entirely in `merge_bind_results_ref`, with 47.2GB physical footprint. Async dropping the retained `Vec<BindResult>` was tested and rejected because it let foreground work overlap old retained bind data and worsened peak memory. The next required slice is an ownership-consuming or streaming bind merge that moves only the needed fields into the merged program and releases per-file bind payloads incrementally.
