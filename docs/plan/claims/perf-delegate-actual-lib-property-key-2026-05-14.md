# Claim: Admit PropertyKey direct actual-lib alias body
...
one more declaration-file `DelegateCrossArenaSymbol` miss on monorepo-006 with
...
- `crates/tsz-checker/src/state/type_analysis/cross_file_direct.rs`
  ...
  - updates alias-body coverage so `PropertyKey` is expected to resolve through
    direct alias-body lowering.
- `docs/plan/perf-runs/2026-05-14-delegate-actual-lib-property-key.md`
  records monorepo-006 attribution evidence:
  - diagnostics unchanged (`10,198`)
  - `DelegateCrossArenaSymbol` children `14 -> 13`
  - `delegate.misses` `14 -> 13`
  - `checker.with_parent_cache_constructed` `14 -> 13`
...
- `TSZ_PERF_COUNTERS=1 .target/release/tsz --noEmit -p /private/tmp/tsz-bench-fixtures/monorepo-006/tsconfig.json --extendedDiagnostics --pretty false --diagnostics-json /tmp/tsz-perf-goal-next/propertykey-admit-after-diag.json --perf-counters-json /tmp/tsz-perf-goal-next/propertykey-admit-after-pc.json` (expected exit `2`)
