...
2. The driver's `is_parser_grammar_code` filter classified TS1029/1030/1040/1243 as "non-grammar parse errors", which then suppressed sibling grammar codes (e.g. TS1070) via the `hasParseDiagnostics`-style gate. tsc emits all of these via `grammarErrorOnNode` from the checker, so it does not self-suppress.

This PR (a) adds TS1275 emission at the `accessor` modifier-keyword position for class members that turn out to be a constructor / method / getter / setter, plus the top-level `accessor <decl>` case; (b) adds TS1276 emission at the `?` token of an optional auto-accessor property; (c) adds TS1029/1030/1040/1243/1275/1276 to `is_parser_grammar_code` so they coexist with siblings instead of suppressing them.
...
- `crates/tsz-cli/src/driver/check_utils.rs` (+6 codes in `is_parser_grammar_code`)
...
- Full conformance suite: net 12366 → 12369 (+3), no regressions.
- `./scripts/conformance/conformance.sh run --filter override` → 33/33 passed.
