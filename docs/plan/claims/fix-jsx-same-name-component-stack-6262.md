...
its name with its props interface. The slice adds crash regressions for the
reported TSX pattern and fixes the narrow recursion path in JSX diagnostic
display without changing normal JSX prop checking.
...
- Added a declaration recursion guard to the JSX prop annotation/declaration
  walkers. Recursive expansions of a type reference now skip declarations
  already on the diagnostic-walker path while still allowing the same merged
  symbol to fall through to the actual interface/type declaration.
- Added focused JSX checker regressions for a function component and props
  interface that share the same name and accept multiple JSX body children,
  including the reported module-shaped repro.
...
- `cargo test -p tsz-checker --test jsx_component_attribute_tests jsx_function_component_same_name_as_props_interface_does_not_recurse -- --nocapture` - passed
