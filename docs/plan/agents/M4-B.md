...
- Current open PRs owned by `agent:M4-B`:
  ...
  - `#9230` ready/off-auto; exact-head draft-light CI passed and M4-B promoted
    ...
    `project-compile-canary`, `conformance-1`, `conformance-4`, `fourslash-3`,
    and `fourslash-5` passed. `conformance-0` is in progress while the rest of
    the ready-review heavy matrix is queued after the earlier
