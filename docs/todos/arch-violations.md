...
27. **DRY violation: `has_modifier` duplicated in 9+ files**: The "scan modifiers NodeList for a SyntaxKind" pattern is copy-pasted across binder (`state_node_binding_names.rs:507`), checker (`context/mod.rs:1527`, `types/type_checking.rs:130`), and emitter (`emitter/helpers.rs:1029`, `declaration_emitter/helpers.rs:806`, `transforms/namespace_es5_ir_helpers.rs:112`, `transforms/class_es5_ir.rs:2008`, `transforms/module_commonjs.rs:456`, `transforms/module_commonjs_ir.rs:426`). Should be a shared utility in `tsz-parser`.
28. **DRY violation: `is_function_like` reimplemented in 4+ locations**: The 7-SyntaxKind "is function-like" check is duplicated in `tsz-lsp/src/call_hierarchy.rs:319`, `tsz-wasm/src/wasm_api/ast.rs:504`, and 4x inline in `tsz-checker/src/flow/flow_analysis_usage.rs:206,440,476,507` instead of using `Node::is_function_like()`.
29. **DRY violation: LSP hover/definition share 3 identical methods**: `find_symbol_query_node_at_or_before`, `is_comment_context`, `should_backtrack_to_previous_symbol` are byte-for-byte identical in `tsz-lsp/src/hover.rs:215-290` and `tsz-lsp/src/definition.rs:873-948`. Extract to shared LSP utility.
30. **DRY violation: type arg constraint-checking loop in `generic_checker.rs`**: `validate_call_type_arguments` (lines 122-178) and `validate_new_expression_type_arguments` (lines 375-431) contain ~50 identical lines for TS2344 constraint checking. Extract shared `validate_type_args_against_params` helper.
