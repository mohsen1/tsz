...
13. **Emit JS pass rate regression**: Commit 92ba86966 ("Fix auto-accessor emit ordering") caused JS emit pass rate to drop from 67.9% → 67.7% (23 fewer passing tests). The unit test breakage is fixed, but the rate regression needs investigation — likely related to `declarations.rs` or `declarations_class_members.rs` changes in that commit.
