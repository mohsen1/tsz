...
33. **DRY violation: `is_*` type predicates duplicated between `visitors/visitor_predicates.rs` and `type_queries/mod.rs`** — 15+ identical single-expression predicates (`is_union_type`, `is_array_type`, `is_tuple_type`, etc.) exist in both files with only the parameter name differing (`types` vs `db`). One module should delegate to the other.
34. **DRY violation: `export_decl_has_runtime_value` + `export_clause_is_type_only`** — near-identical 40+ line functions in `emitter/module_emission.rs` and `lowering_pass_helpers.rs`. Should be free functions in a shared module.
35. **DRY violation: `is_instantiated_module`** — near-identical 30-line function in `emitter/helpers.rs` and `transforms/module_commonjs.rs` (free function variant).
36. **Dead code: commented-out debug `println!`** in `solver/relations/subtype.rs` lines 528-529, 547. Should be removed.
