...
50. **Emitter near-threshold files**: ~~`emitter/declarations.rs` (1,967 LOC — 33 lines from limit)~~ ✅ Split — extracted class emission logic (~1,160 LOC of methods + ~387 LOC of tests) into `declarations_class.rs`, reducing `declarations.rs` to 414 LOC. ~~`lowering_pass.rs` (1,916 LOC)~~ ✅ Split (9834ab491) — extracted `visit_children` child-traversal dispatch (~530 LOC) into `lowering_pass_visit_children.rs`, reducing `lowering_pass.rs` to 1,385 LOC. ~~`declaration_emitter/mod.rs` (1,850 LOC)~~ ✅ Split — extracted interface declaration/member emission (~357 LOC) into `declaration_emitter/interfaces.rs`, reducing `mod.rs` to 1,493 LOC. ~~`declaration_emitter/helpers.rs` (1,794 LOC)~~ ✅ Split — extracted type syntax emission (`emit_type`, `emit_entity_name`, ~492 LOC) into `declaration_emitter/type_emission.rs`, reducing `helpers.rs` to 1,304 LOC. Remaining near-threshold files: `transforms/namespace_es5_ir.rs` (1,793 LOC), `transforms/async_es5_ir.rs` (1,783 LOC).
