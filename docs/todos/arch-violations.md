...
40. **DRY violation: thin modifier wrappers duplicated across emitter files** — `has_declare_modifier` (4 copies), `has_export_modifier` (5 copies), `has_default_modifier` (2 copies) are all 1-line wrappers over `NodeArena::has_modifier`. Files: `lowering_pass_helpers.rs`, `emitter/helpers.rs`, `declaration_emitter/helpers.rs`, `transforms/namespace_es5_ir_helpers.rs`, `transforms/es5.rs`.
