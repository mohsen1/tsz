...
20. **Solver `operations/mod.rs`** reduced from 1,811 → 1,085 LOC by extracting argument checking, parameter analysis, tuple rest handling, placeholder detection, and contextual sensitivity helpers (~726 LOC) into `operations/call_args.rs`. Next solver files to monitor: `type_queries/mod.rs` (1,750 LOC), `contextual.rs` (1,693 LOC), `diagnostics.rs` (1,690 LOC).
