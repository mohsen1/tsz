...
50. **Emitter near-threshold files**: ~~`emitter/declarations.rs` (1,967 LOC — 33 lines from limit)~~ ✅ Split — extracted class emission logic (~1,160 LOC of methods + ~387 LOC of tests) into `declarations_class.rs`, reducing `declarations.rs` to 414 LOC. Remaining near-threshold files: `lowering_pass.rs` (1,916 LOC), `declaration_emitter/mod.rs` (1,850 LOC), `declaration_emitter/helpers.rs` (1,794 LOC), `transforms/namespace_es5_ir.rs` (1,793 LOC), `transforms/async_es5_ir.rs` (1,783 LOC). Priority: `lowering_pass.rs` is closest to the 2000-line limit.
