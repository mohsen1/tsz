...
- [x] Migration action 205: continued solver-internal `TypeData` migration by renaming remaining `TypeKey` signature/type references in `crates/tsz-solver/src/instantiate.rs`, `crates/tsz-solver/src/infer.rs`, `crates/tsz-solver/src/evaluate.rs`, and `crates/tsz-solver/src/tracer.rs`.
- [x] Migration action 206: final roadmap closure audit confirmed no remaining unchecked roadmap items or in-progress status markers in `docs/architecture/CRITIQUE.md` and no remaining public-facing `TypeKey` signatures in non-test solver source.
- [x] Migration action 207: added explicit final closure note block in `docs/architecture/CRITIQUE.md` to document completion state and the key boundary outcomes (TypeKey privatization + guardrail enforcement).
- [x] Migration action 208: added `scripts/render_architecture_report.py` and wired `scripts/check-checker-boundaries.sh` to emit `artifacts/architecture/arch_guard_report.md` alongside JSON for auto-generated architecture tables (largest files + guard summary).
- [x] Migration action 209: expanded `scripts/render_architecture_report.py` markdown output with a structured failure-group summary table before detailed hit listings to improve auto-generated architecture table readability.
- [x] Migration action 210: added `.github/pull_request_template.md` enforcing the architecture contribution checklist (`WHAT/WHERE`, solver boundary usage, parity impact, TS2322 path checks) and linked this completion in `docs/architecture/CRITIQUE.md`.
