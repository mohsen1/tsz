...
| `Object`, `Function` global values | `is_builtin_global_reference`, `known_global_value_has_local_shadow`, `identifier_resolves_to_unshadowed_global`, `sym_id_is_lib_function`, `resolve_lib_type_by_name`, and `TypeId::{OBJECT,FUNCTION}` | Object/Function constructor and callable-constraint behavior depends on the built-in global and must not trigger for a locally shadowed identifier. | `crates/tsz-checker/src/flow/control_flow/type_guards.rs`, `crates/tsz-checker/src/checkers/generic_checker/callable_constraint_helpers.rs`, `crates/tsz-checker/src/context/lib_queries.rs`, `crates/tsz-checker/src/error_reporter/properties.rs`, `crates/tsz-checker/src/state/type_analysis/computed_commonjs/exports_detection.rs`, `crates/tsz-checker/src/state/type_analysis/computed_commonjs/exports_resolution.rs`, `crates/tsz-checker/src/state/variable_checking/variable_helpers/core.rs`, `crates/tsz-checker/src/types/type_checking/global.rs`, `crates/tsz-solver/src/diagnostics/format/mod.rs` |
