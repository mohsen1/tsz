...
- **2026-04-25** · branch `claude/modest-archimedes-WwJ1o` · **IN PROGRESS** · audit §Scripts `scripts/lib/query_snapshot.py` · extract shared snapshot-loading and top-counter mechanics from `scripts/emit/query-emit.py` and `scripts/fourslash/query-fourslash.py` into `scripts/lib/query_snapshot.py`; add `scripts/lib/__init__.py`; add unit tests in `scripts/lib/test_query_snapshot.py`

- **2026-04-25** · branch `claude/modest-archimedes-WwJ1o` · **IN PROGRESS** · audit §tsz-emitter `tests/statements.rs` · add test-local `parse_and_print(source) -> String` helper; migrate all 57 `ParserState::new`/`Printer::new`/`printer.finish().code` setup blocks (1293-line file) off hand-rolled 6-line boilerplate
