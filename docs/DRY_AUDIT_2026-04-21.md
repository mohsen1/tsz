...
- **2026-04-25** · branch `claude/modest-archimedes-fD8XR` · **SHIPPED** (partially) · §P1 Conformance Runner Backend Consolidation (scripts) · `lib/results.py` + `lib/test_results.py` added; `build-snapshot-detail.py`, `extract-baseline.py`, `analyze-conformance-areas.py` migrated — `analyze-conformance.py` inline parser still duplicates `parse_runner_output`
- **2026-04-25** · branch `claude/modest-archimedes-hKp2R` · §P1 Conformance Runner Backend Consolidation (scripts) — migrate `analyze-conformance.py` inline FAIL/XFAIL parser + wrong-code diff to `lib.results.parse_runner_output` / `lib.results.compute_diff` · planned files: `scripts/conformance/analyze-conformance.py`, `scripts/conformance/lib/test_results.py` · draft PR title: `[do not merge] chore(scripts/conformance): migrate analyze-conformance.py to lib.results`
