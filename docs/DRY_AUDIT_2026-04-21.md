...
- **2026-05-08 08:43:47** · branch `dry-loop-test-utils-load-lib-batch5-20260508-084347` · audit §P0 Test-Harness Consolidation (tsz-checker) — `load_lib_files_for_test` migration batch 5 · `ts2353_tests.rs` (37 tests; -72 lines): collapse single-file `lib.es5.d.ts` loader plus the manual parse/bind/check pipeline onto `check_source_with_libs(source, "test.ts", default, &load_lib_files(&["es5.d.ts"]))`. Verified the lib-name change from `lib.es5.d.ts` to `es5.d.ts` is benign for TS2353 — the `source.file_name.starts_with("lib.")` gate at `crates/tsz-checker/src/types/queries/lib_resolution.rs:983` only affects declaration-file value-lowering paths these tests don't exercise. Draft PR title: `chore(checker-tests): batch 5 of load_lib_files_for_test migration`.
