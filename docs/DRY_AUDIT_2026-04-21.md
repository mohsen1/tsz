...
- **2026-05-08 10:35:41** · branch `dry-loop-diagnostic-span-helper-20260508-103541` · audit §tsz-common — "Span exists, but common diagnostics store start plus length, forcing LSP/core callers to reconstruct spans. Recommendation: Add diagnostic constructors and accessors that use Span so callers stop reconstructing start+length tuples." · add `Diagnostic::error_with_span`, `Diagnostic::from_code_with_span`, `Diagnostic::with_related_span`, and `Diagnostic::span()` accessor in `crates/tsz-common/src/diagnostics/mod.rs`. 4 self-tests pin equivalence with the start+length variants and the `Span::from_len` round-trip. No call-site migrations in this PR. Draft PR title: `feat(common): add Span-based Diagnostic constructors and accessor`.
