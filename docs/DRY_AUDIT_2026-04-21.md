...
- **2026-05-07 23:55:00** · branch `chore/checker-tests-load-lib-files-helper` · audit §P0 Test-Harness Consolidation (tsz-checker) — "First useful PR ... 10 `load_lib_files_for_test` variants" · add `load_lib_files(names: &[&str]) -> Vec<Arc<LibFile>>` plus shared lib-name constants (`LIB_NAMES_ES2015`, `LIB_NAMES_ES2015_DOM`, `LIB_NAMES_ES2015_CORE_ONLY`) in `crates/tsz-checker/src/test_utils.rs`; replace local `load_lib_files_for_test` clones in `crates/tsz-checker/tests/{jsdoc_type_tag_tests,jsdoc_param_return_ts2314_tests,ts2322_tests,jsx_spread_assignability_suppresses_ts2741,contextual_typing_tests,tuple_rest_array_canonicalization_tests}.rs` (and any other files using the same `lib-assets` / `lib-assets-stripped` / `TypeScript/src/lib` probing pattern). Self-tests pin the helper's no-roots, dedup, and ordering contracts. Draft PR title: `[do not merge] chore(checker-tests): consolidate load_lib_files_for_test variants in test_utils`.
