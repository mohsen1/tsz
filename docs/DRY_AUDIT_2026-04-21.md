...
Pick one small DRY refactor from the remaining audit items, verify that it has not already landed or been opened as a PR, create a WIP GitHub issue and a draft PR titled with the `[do not merge]` prefix, implement it with focused tests, and then update that PR for review after CI passes.
...
Every successful iteration must push a draft PR and use CI for broad build, lint, unit, conformance, emit, fourslash, and WASM verification. Fix any failures before marking the PR ready. If verification cannot pass, keep the PR as draft with the `[do not merge]` prefix and document the blocker.
