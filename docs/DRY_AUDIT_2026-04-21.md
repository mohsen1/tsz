...
Use this document as the backlog and operating prompt for repeated DRY-cleanup iterations. Each iteration should produce one manageable, reviewable PR. Do not batch unrelated audit items together just because they are listed near each other. Use a WIP GitHub issue and draft PR before implementation so concurrent loop iterations can see it and avoid repeating the same slice.

### Mission

Pick one small DRY refactor from the remaining audit items, verify that it has not already landed or been opened as a PR, create a WIP GitHub issue and a draft PR titled with the `[WIP]` prefix, implement it with focused tests, and then update that PR for review after CI passes.
...
### Coordinate Work Before Implementation

After choosing the slice, create or update a GitHub issue for the scope, mark it `WIP`, and record the planned files/symbols there before touching production code. The issue and draft PR are the coordination surface.
...
## Summary
- Implements a DRY cleanup slice from `docs/DRY_AUDIT_2026-04-21.md`.
...
Do not begin implementation until the WIP issue and draft PR exist. If another loop starts while this one is in progress, it should see either the issue or the draft PR and pick different work.
...
Every successful iteration must push a draft PR and use CI for broad build,
lint, unit, conformance, emit, fourslash, and WASM verification. Fix any
failures before marking the PR ready. If verification cannot pass, keep the PR
as draft with the `[WIP]` prefix and document the blocker.

After verification passes, update the existing draft PR, remove the `[WIP]` title prefix, remove the `WIP` label, and mark it ready for review:
...
## Verification
- Draft PR light CI, then ready-for-review full CI
