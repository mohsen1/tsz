...
- **2026-04-25** · branch `claude/modest-archimedes-1aZ7C` · audit P0 §2 Shared Test Harnesses — remove local pipeline boilerplate in 6 checker test files, replace with `test_utils::check_source` · planned files/symbols: `crates/tsz-checker/tests/for_in_narrowing_tests.rs` (local `check_strict`), `crates/tsz-checker/tests/class_index_signature_compat_tests.rs` (local `compile_and_get_diagnostics`), `crates/tsz-checker/tests/ts2323_tests.rs` (local `get_error_codes`), `crates/tsz-checker/tests/ts1501_tests.rs` (local `has_ts1501`), `crates/tsz-checker/tests/ts1254_ambient_const_tests.rs` (local `get_codes`), `crates/tsz-checker/tests/ts1210_class_arguments_tests.rs` (local `diagnostic_codes_for_js`) · draft PR title: `[do not merge] chore(checker-tests): remove local pipeline boilerplate in 6 test files`
