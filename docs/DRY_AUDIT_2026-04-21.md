...
- **2026-05-08 07:33:10** · branch `dry-loop-test-utils-load-lib-batch3-20260508-073310` · audit §P0 Test-Harness Consolidation (tsz-checker) — `load_lib_files_for_test` migration batch 3 · collapse local `load_lib_files_for_test` bodies in `crates/tsz-checker/tests/conformance_issues/core/helpers.rs` (used by 4 sub-tests via re-export) and `crates/tsz-checker/tests/conformance_issues/types/enum.rs` to a single-line delegate to `tsz_checker::test_utils::load_default_lib_files`; migrate `jsdoc_type_tag_tests.rs` (3 inline pipelines collapsed onto `check_source_with_libs` / `check_source`). Net −243 lines across 3 files. Draft PR title: `chore(checker-tests): batch 3 of load_lib_files_for_test migration`.
- **2026-05-08 07:47:33** · branch `dry-loop-test-utils-load-lib-batch4-20260508-074733` · **SHIPPED** → PR #4607 · `contextual_typing_tests.rs` (71 tests; -64 lines): 3 wrappers collapsed onto `check_source` + `check_source_with_libs(LIB_NAMES)`.
