...
- **2026-05-08 10:14:37** · branch `dry-loop-test-utils-compiled-lib-helper-20260508-101437` · audit §P0 Test-Harness Consolidation (tsz-checker) — extend `tsz_checker::test_utils` with `load_compiled_lib_files(names)` that probes `TypeScript/lib/`, `scripts/{conformance,emit,top-level}/node_modules/typescript/lib/` for files like `lib.es5.d.ts` and stores them with the `lib.` prefix preserved. Unblocks future migration of test files that depend on the `source.file_name.starts_with("lib.")` gate at `crates/tsz-checker/src/types/queries/lib_resolution.rs:983` (verified by ts2300's `duplicate_identifier_with_default_lib_symbol_reports_lib_locations` test, which fails when LibFile names lose the `lib.` prefix). 1 self-test pins the naming and dedup contract. No call-site migrations in this PR. Draft PR title: `chore(checker-tests): add load_compiled_lib_files helper for lib.X.d.ts naming`.
