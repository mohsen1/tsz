...
- 2026-04-25 · branch `claude/modest-archimedes-Ycq3h` · §tsz-solver test fixture consolidation
  - Item: tsz-solver test fixture — add `SolverFixture` builder to `tests/common/mod.rs`; migrate `tests/subtype_cache_tests.rs` and `tests/constraint_tests.rs` off per-test `TypeInterner::new()` / `QueryCache::new()` / `InferenceContext::new()` boilerplate.
  - Files: `crates/tsz-solver/tests/common/mod.rs`, `crates/tsz-solver/tests/subtype_cache_tests.rs`, `crates/tsz-solver/tests/constraint_tests.rs`
  - Draft PR title: `[do not merge] refactor(solver/tests): add SolverFixture builder; migrate subtype-cache and constraint tests`
