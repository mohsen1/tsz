...
- **2026-05-08 10:57:11** · branch `dry-loop-load-compiled-lib-primary-fallback-20260508-105711` · audit §P0 Test-Harness Consolidation (tsz-checker) — `load_compiled_lib_files` walk-up fallback · the original probe paths (`../../scripts/node_modules/...`) are relative to the worktree's `CARGO_MANIFEST_DIR`, but `npm install` only populates the primary checkout's `scripts/`. Worktrees end up with empty roots and the helper returned nothing. This adds a parent-walk fallback (up to 8 levels) that locates the first ancestor with `scripts/node_modules/typescript/lib/` and includes its `scripts/{,conformance,emit}/node_modules/typescript/lib/` dirs as additional roots. New self-test pins the worktree-friendly behavior. Draft PR title: `chore(checker-tests): walk-up to primary checkout in load_compiled_lib_files`.
