...
///
/// The rule: "When a mapped type's template is a Conditional whose `check_type` or
/// `extends_type` is or contains an Application of a Lazy alias, the instantiator
/// must defer evaluation to the outer resolver-backed evaluator."
#[test]
fn test_mapped_template_conditional_check_type_lazy_app_defers() {
    // Simulates: type IsStr<T> = T extends string ? true : false;
    //            type StrKeys<T> = { [K in keyof T]: IsStr<T[K]> extends true ? K : never }[keyof T]
    //
    // When instantiating `{ [K in keyof T]: App(IsStr, [T[K]]) extends true ? K : never }`
    // with T = Foo = { a: string, b: number }, the template has a Conditional whose
    // check_type is Application(Lazy(IsStr), [Foo[K]]). This must be deferred.
    let interner = TypeInterner::new();
    ...
    // IsStr alias as a Lazy reference (any DefId stands in for a real alias)
    let is_str_def = DefId(42);
    let lazy_is_str = interner.intern(TypeData::Lazy(is_str_def));

    // Concrete object type Foo = { a: string, b: number }
    let foo = interner.object(vec![
        PropertyInfo::new(interner.intern_string("a"), TypeId::STRING),
        PropertyInfo::new(interner.intern_string("b"), TypeId::NUMBER),
    ]);
    let keyof_foo = interner.keyof(foo);
    // Evaluate keyof Foo → "a" | "b"
    let keyof_foo_eval = crate::evaluation::evaluate::evaluate_type(&interner, keyof_foo);

    // Foo[K] = IndexAccess(Foo, K)
    let foo_index_k = interner.index_access(foo, k_type);

    // App(IsStr, [Foo[K]])
    let is_str_app = interner.application(lazy_is_str, vec![foo_index_k]);

    // Template: App(IsStr, [Foo[K]]) extends true ? K : never
    let template = interner.conditional(crate::types::ConditionalType {
        check_type: is_str_app,
    ...
    // Mapped type: { [K in "a"|"b"]: template }
    let mapped = MappedType {
        type_param: k_param,
        constraint: keyof_foo_eval,
        name_type: None,
        template,
        optional_modifier: None,
        readonly_modifier: None,
    ...
    // Instantiate with empty substitution (T already substituted, K shadowed)
    let subst = TypeSubstitution::new();
    let result = instantiate_type(&interner, mapped_id, &subst);

    // The result must be a Mapped type (deferred), not an Object with wrong values.
    // If evaluation were attempted with NoopResolver, App(IsStr, ...) would stay
    // unresolved and the conditional would always yield `never`, making every
    // property `never`. Deferral preserves the Application for the outer evaluator.
    match interner.lookup(result) {
        Some(TypeData::Mapped(_)) => {
            // Expected: evaluation deferred so outer evaluator can resolve IsStr
        }
        other => panic!(
            "Expected deferred Mapped type when template conditional check_type is \
             Application(Lazy), got {other:?}"
        ),
    }
}

/// Same rule with a renamed iteration variable (`P` instead of `K`): the fix must
/// not be sensitive to the spelling of the mapped type's type parameter name.
#[test]
fn test_mapped_template_conditional_check_type_lazy_app_defers_rename_p() {
    let interner = TypeInterner::new();

    // Use "P" instead of "K" to prove the fix is name-independent
    let p_name = interner.intern_string("P");
    ...
    let check_alias_def = DefId(99);
    let lazy_check = interner.intern(TypeData::Lazy(check_alias_def));

    let foo = interner.object(vec![
        PropertyInfo::new(interner.intern_string("x"), TypeId::STRING),
        PropertyInfo::new(interner.intern_string("y"), TypeId::NUMBER),
    ]);
    let keyof_foo = crate::evaluation::evaluate::evaluate_type(&interner, interner.keyof(foo));

    let foo_index_p = interner.index_access(foo, p_type);
    let app = interner.application(lazy_check, vec![foo_index_p]);
    ...
    let mapped_id = interner.mapped(mapped);
    let result = instantiate_type(&interner, mapped_id, &TypeSubstitution::new());

    assert!(
        matches!(interner.lookup(result), Some(TypeData::Mapped(_))),
        "Deferred regardless of iteration variable spelling; got {:?}",
        interner.lookup(result)
    );
}

/// When the `extends_type` of a Conditional is a Lazy Application, the instantiator
/// must also defer, because the `NoopResolver` cannot evaluate the extends check.
///
/// Example pattern: `{ [K in keyof T]: T[K] extends SomeLazyAlias<U> ? K : never }`
/// where `SomeLazyAlias` is an unresolved type alias.
#[test]
fn test_mapped_template_conditional_extends_type_lazy_app_defers() {
    let interner = TypeInterner::new();
    ...
    let alias_def = DefId(7);
    let lazy_alias = interner.intern(TypeData::Lazy(alias_def));
    // extends_type: SomeLazyAlias<number>
    let extends_app = interner.application(lazy_alias, vec![TypeId::NUMBER]);
    ...
    // Template: Foo[K] extends SomeLazyAlias<number> ? K : never
    let template = interner.conditional(crate::types::ConditionalType {
        check_type: foo_index_k,
        extends_type: extends_app,
        true_type: k_type,
        false_type: TypeId::NEVER,
        is_distributive: false,
    });
    ...
    assert!(
        matches!(interner.lookup(result), Some(TypeData::Mapped(_))),
        "Deferred when extends_type is Application(Lazy); got {:?}",
        interner.lookup(result)
    );
}
