...
#[test]
fn test_definition_store_semantic_def_overlays_keep_same_raw_symbol_in_different_files() {
    let interner = create_test_interner();
    let mut base = rustc_hash::FxHashMap::default();
    base.insert(
        tsz_binder::SymbolId(2),
        semantic_def_entry("BaseLib", 1, tsz_binder::SemanticDefKind::Interface),
    );

    let mut overlay = rustc_hash::FxHashMap::default();
    overlay.insert(
        tsz_binder::SymbolId(2),
        semantic_def_entry("OverlayLib", 2, tsz_binder::SemanticDefKind::Class),
    );
    ...
    let base_def = store
        ...
        .and_then(|def_id| store.get(def_id))
        .expect("base semantic def should remain registered for its file");
    assert_eq!(base_def.name, interner.intern_string("BaseLib"));
    assert_eq!(base_def.kind, DefKind::Interface);

    let overlay_def = store
        .lookup_by_symbol(2, 2)
        .and_then(|def_id| store.get(def_id))
        .expect("overlay semantic def should register for its file");
    assert_eq!(overlay_def.name, interner.intern_string("OverlayLib"));
    assert_eq!(overlay_def.kind, DefKind::Class);
}
