...
fn test_flow_facts_basic_operations() {
    ...
    facts.add_narrowing("y".to_string(), crate::TypeId::STRING);
    assert_eq!(facts.get_narrowed_type("y"), Some(crate::TypeId::STRING));
...
fn test_flow_facts_merge_intersection_for_narrowings() {
    ...
    facts1.add_narrowing("x".to_string(), crate::TypeId::STRING);
    facts1.add_narrowing("y".to_string(), crate::TypeId::NUMBER);

    let mut facts2 = FlowFacts::new();
    facts2.add_narrowing("x".to_string(), crate::TypeId::STRING);
    facts2.add_narrowing("z".to_string(), crate::TypeId::BOOLEAN);

    let merged = facts1.merge(&facts2);

    // x should be present (same narrowing in both)
    assert_eq!(merged.get_narrowed_type("x"), Some(crate::TypeId::STRING));
...
fn test_flow_facts_merge_conflicting_narrowings() {
    ...
    let mut facts2 = FlowFacts::new();
    facts2.add_narrowing("x".to_string(), crate::TypeId::NUMBER);
...
fn test_flow_type_evaluator_compute_narrowed_type() {
    ...
    let mut facts = FlowFacts::new();
    facts.add_narrowing("x".to_string(), crate::TypeId::STRING);

    // Should return the narrowed type
    assert_eq!(
        evaluator.compute_narrowed_type(crate::TypeId::ANY, &facts, "x"),
        crate::TypeId::STRING
    );

    // Should return original type if no narrowing
    assert_eq!(
        evaluator.compute_narrowed_type(crate::TypeId::NUMBER, &facts, "y"),
        crate::TypeId::NUMBER
    );
}
...
fn test_flow_type_evaluator_facts_from_narrowings() {
    ...
    narrowings.insert("x".to_string(), crate::TypeId::STRING);
    narrowings.insert("y".to_string(), crate::TypeId::NUMBER);

    let facts = evaluator.facts_from_narrowings(narrowings);

    assert_eq!(facts.get_narrowed_type("x"), Some(crate::TypeId::STRING));
    assert_eq!(facts.get_narrowed_type("y"), Some(crate::TypeId::NUMBER));
...
fn test_flow_type_evaluator_narrow_by_typeof() {
    ...
    let string_or_number = interner.union2(crate::TypeId::STRING, crate::TypeId::NUMBER);
    let narrowed = evaluator.narrow_by_typeof(string_or_number, "string");

    // Should narrow to string
    assert_eq!(narrowed, crate::TypeId::STRING);
}
...
fn test_flow_type_evaluator_narrow_excluding_type() {
    ...
    let string_or_null = interner.union2(crate::TypeId::STRING, crate::TypeId::NULL);
    let narrowed = evaluator.narrow_excluding_type(string_or_null, crate::TypeId::NULL);
...
fn test_flow_facts_clone() {
    let mut facts1 = FlowFacts::new();
    facts1.mark_definitely_assigned("x".to_string());
    facts1.add_narrowing("y".to_string(), crate::TypeId::STRING);
    ...
    assert_eq!(facts2.get_narrowed_type("y"), Some(crate::TypeId::STRING));
