...
/// Expand a mapped type's keys into properties using a TypeResolver for deep evaluation.
///
/// This is the resolver-aware version of `expand_mapped_type_to_properties`. It uses
/// `TypeEvaluator::with_resolver` to evaluate instantiated templates and name remappings,
/// enabling resolution of Lazy(DefId) types through the provided resolver. This is the
/// preferred entry point when the caller has a resolver context (e.g., CheckerContext).
///
/// The no-resolver version (`expand_mapped_type_to_properties`) should only be used in
/// contexts where no resolver is available (e.g., solver-internal evaluation, tests).
pub fn expand_mapped_type_to_properties_with_resolver<
...
>(
    ...
    for &key_name in string_keys {
        ...
        // Handle name remapping with resolver-aware evaluation
        let remapped = if let Some(name_type) = mapped.name_type {
            subst.clear();
            subst.insert(mapped.type_param.name, key_literal);
            let instantiated_name = instantiate_type(db, name_type, &subst);
            evaluator.evaluate(instantiated_name)
        } else {
            key_literal
        };
        if remapped == TypeId::NEVER {
        ...
        // Instantiate template with this key, evaluate with resolver
        subst.clear();
        subst.insert(mapped.type_param.name, key_literal);
        let instantiated = instantiate_type(db, mapped.template, &subst);
        let mut property_type = evaluator.evaluate(instantiated);

        // Check if evaluation hit depth limits
        if property_type == TypeId::ERROR && evaluator.is_depth_exceeded() {
            return properties;
        }
        ...
        // source property's declared type to avoid double-encoding undefined from
        // indexed access. This applies regardless of whether `-?` is used.
        if is_homomorphic
            && source_optional
            && let Some((_, _, declared_type)) = source_info
        {
