...
/// such member. Also true when `type_id` is a generic application of an alias
/// whose declared body is a bare indexed access over the alias's own type
/// parameters (`type Pluck<TSrc, KSel extends keyof TSrc> = TSrc[KSel]` used
/// at `Pluck<TSrc, KSel>`), *and* the application's actual arguments in the
...
/// common `type Alias<A, B> = A[B]` shape) — checks whether the application's
/// actual arguments at those positions still mention a type parameter.
/// Declines (returns `false`) for any other body shape (including a `KeyOf`
/// or `Conditional` alias body) rather than risking a wrong substitution;
/// widen to those once a concrete `tsc`-verified witness motivates it.
