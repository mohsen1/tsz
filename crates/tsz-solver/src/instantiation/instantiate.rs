            ...
            TypeData::TypeParameter(info) => {
                ...
                if let Some(substituted) = self.substitution.get(info.name) {
                    ...
                    if !self.preserve_unsubstituted_type_params {
                        ...
                        //
                        // Pre-check: the substitution must actually reach some name inside
                        // the constraint. Without this gate, walking the constraint can
                        // re-intern a structurally-equivalent inner TypeParameter at a
                        // different `TypeId` (e.g. when an outer-scope T was first
                        // observed with a transient constraint and later re-canonicalised
                        // with constraint=None). The bare `instantiated_constraint != constraint`
                        // check would then fire even though nothing was actually substituted,
                        // collapsing an outer-scope type parameter into its constraint at the
                        // call boundary. Concretely: inside `function f2<T, E extends keyof T>(t: T, e: E)`,
                        // calling `f1<T_c, E_c extends keyof T_c>(p: E_c)` with `e`
                        // would replace caller's `E` with `keyof T_outer & string` and the
                        // post-resolution constraint check would then collapse the parameter
                        // type to `never` instead of `E_caller`.
                        if let Some(constraint) = info.constraint
                            && !self.substitution.is_empty()
                            && {
                                let mut visited = FxHashSet::default();
                                substitution_keys_appear_in(
                                    self.interner,
                                    constraint,
                                    self.substitution,
                                    &mut visited,
...
/// Does any `TypeParameter`/`Infer` name reachable from `type_id` match a key
/// in `substitution`?
///
/// This is the structural prerequisite for the "fall back to instantiated
/// constraint" path in `instantiate_key` (`TypeParameter` case). The fallback
/// must only fire when the substitution genuinely applies *somewhere* inside
/// the constraint — otherwise re-interning side effects (e.g. a deeper
/// `TypeParameter` happening to canonicalize to a different `TypeId`) would
/// spuriously replace a passing-through outer-scope type parameter with its
/// constraint, losing its identity at the call boundary.
///
/// Walks once with a small visited set to avoid revisits on cyclic alias
/// graphs. `BoundParameter` / `Lazy` / `Recursive` / `TypeQuery` /
/// `UnresolvedTypeName` / `ModuleNamespace` are treated as not carrying free
/// substitution-targeted parameters: any captured type arguments arrive via
/// an enclosing `Application` whose `args` are walked directly. This keeps
/// the gate tight for outer-scope type parameters that pass through a
/// generic call without `args` of their own.
fn substitution_keys_appear_in(
    interner: &dyn TypeDatabase,
    type_id: TypeId,
    substitution: &TypeSubstitution,
    visited: &mut FxHashSet<TypeId>,
    ...
    match key {
        ...
        TypeData::Union(members) | TypeData::Intersection(members) => {
            ...
            members
                .iter()
                .any(|&m| substitution_keys_appear_in(interner, m, substitution, visited))
        }
        TypeData::Array(elem) => substitution_keys_appear_in(interner, elem, substitution, visited),
        TypeData::Tuple(list_id) => {
            let elements = interner.tuple_list(list_id);
            elements.iter().any(|elem| {
                substitution_keys_appear_in(interner, elem.type_id, substitution, visited)
            })
        }
        TypeData::IndexAccess(obj, idx) => {
        ...
        TypeData::StringIntrinsic { type_arg, .. } => {
            substitution_keys_appear_in(interner, type_arg, substitution, visited)
        }
        TypeData::Conditional(cond_id) => {
            let cond = interner.conditional_type(cond_id);
            substitution_keys_appear_in(interner, cond.check_type, substitution, visited)
                || substitution_keys_appear_in(interner, cond.extends_type, substitution, visited)
                || substitution_keys_appear_in(interner, cond.true_type, substitution, visited)
                || substitution_keys_appear_in(interner, cond.false_type, substitution, visited)
        }
        TypeData::Mapped(mapped_id) => {
            let mapped = interner.mapped_type(mapped_id);
            // The mapped iteration variable is locally bound; it never matches
            // an outer substitution key (placeholder names are globally unique
            // via PLACEHOLDER_COUNTER). Walking constraint and template covers
            // the actual free positions.
            substitution_keys_appear_in(interner, mapped.constraint, substitution, visited)
                || substitution_keys_appear_in(interner, mapped.template, substitution, visited)
                || mapped.name_type.is_some_and(|name_type| {
                    substitution_keys_appear_in(interner, name_type, substitution, visited)
                })
        }
        TypeData::TemplateLiteral(list_id) => {
            let spans = interner.template_list(list_id);
            spans.iter().any(|span| match span {
                crate::types::TemplateSpan::Type(t) => {
                    substitution_keys_appear_in(interner, *t, substitution, visited)
                }
        ...
        TypeData::Application(app_id) => {
            let app = interner.type_application(app_id);
            substitution_keys_appear_in(interner, app.base, substitution, visited)
                || app
                    .args
                    .iter()
                    .any(|&a| substitution_keys_appear_in(interner, a, substitution, visited))
        }
        TypeData::Object(shape_id) | TypeData::ObjectWithIndex(shape_id) => {
            let shape = interner.object_shape(shape_id);
            let visit_index = |idx: &Option<crate::types::IndexSignature>,
                               visited: &mut FxHashSet<TypeId>|
             -> bool {
                idx.as_ref().is_some_and(|idx| {
                    substitution_keys_appear_in(interner, idx.value_type, substitution, visited)
                })
            };
            shape.properties.iter().any(|prop| {
                substitution_keys_appear_in(interner, prop.type_id, substitution, visited)
            }) || visit_index(&shape.string_index, visited)
                || visit_index(&shape.number_index, visited)
        }
        TypeData::Function(shape_id) => {
            let shape = interner.function_shape(shape_id);
            shape.params.iter().any(|param| {
                substitution_keys_appear_in(interner, param.type_id, substitution, visited)
            }) || substitution_keys_appear_in(interner, shape.return_type, substitution, visited)
