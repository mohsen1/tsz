...
/// Type hint for a parameter in an intrinsic method signature (display use only).
...
/// Describes one parameter of an intrinsic primitive method for LSP display.
/// Used solely for signature-help label generation in the no-lib fallback; does
/// not affect type-checking behaviour.
...
// --- Static parameter tables for common intrinsic method signatures ---
...
///
/// Used by the LSP signature-help path (no-lib fallback) to display accurate
/// parameter names and optionality instead of the generic `...args: any[]`
/// synthesized by `make_apparent_method_type`.  Returns `None` for methods whose
/// parameter shapes are too complex to represent here (e.g. `replace`, `split`).
pub fn apparent_primitive_method_params(
    kind: IntrinsicKind,
    method: &str,
    ...
    match kind {
        IntrinsicKind::String => Some(match method {
            ...
            // match/matchAll/replace/replaceAll/search/split take RegExp or complex
            // overloads — return None so the caller falls back gracefully.
            _ => return None,
        }),
