...
use crate::sound::{SoundDiagnostic, SoundDiagnosticCode, SoundLawyer, SoundModeConfig};
use crate::subtype::{SubtypeChecker, SubtypeResult, TypeEnvironment};
...
mod phase3_defid {
    ...
    fn test_def_id_allocation() {
        ...
        let id1 = store.register(DefinitionInfo::type_alias(
            tsz_common::interner::Atom(1),
            vec![],
            TypeId::NUMBER,
        ));

        let id2 = store.register(DefinitionInfo::type_alias(
            tsz_common::interner::Atom(2),
            vec![],
            TypeId::STRING,
        ));
                ...
                thread::spawn(move || {
                    for j in 0..50 {
                        let id = store.register(DefinitionInfo::type_alias(
                            tsz_common::interner::Atom(i * 1000 + j),
                            vec![],
                            TypeId::NUMBER,
                        ));
