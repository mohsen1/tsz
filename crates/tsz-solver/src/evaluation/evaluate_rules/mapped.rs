...
impl<'a, R: TypeResolver> TypeEvaluator<'a, R> {
    ...
    ///
    /// Re-entrant guard: the walk recurses through `resolve_lazy` bodies, and
    /// mutually-referential def bodies (`A`'s shared-store body referencing
    /// `Lazy(B)` while `B`'s references `Lazy(A)`) would otherwise recurse
    /// without progress until stack overflow. Resolution forms are not
    /// guaranteed acyclic — per-checker refinement and shared-store
    /// publication can pin forms that point at each other — so a same-`TypeId`
    /// re-entry returns `None` (defer), matching the existing "cannot extract
    /// keys" semantics.
