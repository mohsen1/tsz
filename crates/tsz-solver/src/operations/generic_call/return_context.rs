...
impl<'a, C: AssignabilityChecker> CallEvaluator<'a, C> {
    ...
    fn collect_return_context_substitution(
            ...
            crate::type_queries::get_union_members(self.interner.as_type_database(), target)
        {
            ...
            // Ambiguity accounting must also cover arms whose would-be binding
            // is BLOCKED rather than merely different. An arm that is an
            // aligned same-base application of the source (`RawBuilder<O>`
            // against the declared `RawBuilder<Tagged>`) "addresses" the
            // tracked parameter even when its binding is rejected — e.g. the
            // untracked-type-param guard blocks binding `Tagged := O` for an
            // outer-scope `O`. Counting only the arms that produced a value
            // makes `RawBuilder<O> | RawBuilder<number>` look unambiguous
            // (one value: `number`) and pins the parameter, overriding the
            // combined-union inference (`number | O`) that tsc uses and
            // silently accepting the return. Track, per parameter, how many
            // arms address it through an aligned application argument and how
            // many actually contributed a binding; a shortfall means a blocked
            // arm, so the union stays ambiguous and must not pin.
            let mut addressing_arms: FxHashMap<tsz_common::Atom, usize> = FxHashMap::default();
            let mut contributing_arms: FxHashMap<tsz_common::Atom, usize> = FxHashMap::default();
            let source_views = self.transparent_application_views(source);
            for member in target_members
                .into_iter()
                .filter(|member| *member != TypeId::NULL && *member != TypeId::UNDEFINED)
            {
                let member_views = self.transparent_application_views(member);
                let mut arm_addressed: FxHashSet<tsz_common::Atom> = FxHashSet::default();
                'align: for (source_base, source_args) in &source_views {
                    for (member_base, member_args) in &member_views {
                        if source_args.len() != member_args.len()
                            || !self.application_bases_share_declaration(*member_base, *source_base)
                        {
                            continue;
                        }
                        for &source_arg in source_args {
                            if let Some(TypeData::TypeParameter(tp)) =
                                self.interner.lookup(source_arg)
                                && substitution
                                    .domain_contains_type_parameter(&tp, tracked_type_params)
                            {
                                arm_addressed.insert(tp.name);
                            }
                        }
                        break 'align;
                    }
                }
                for name in arm_addressed {
                    *addressing_arms.entry(name).or_default() += 1;
                }
                ...
                for (&name, &member_ty) in member_substitution.map() {
                    *contributing_arms.entry(name).or_default() += 1;
            ...
            for name in param_order {
                ...
                let addressed = addressing_arms.get(&name).copied().unwrap_or(0);
                let contributed = contributing_arms.get(&name).copied().unwrap_or(0);
                if values.len() == 1 && contributed >= addressed {
