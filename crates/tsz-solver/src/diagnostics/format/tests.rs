...
// Issue #9727: array-of-`infer` and other low-precedence type forms must be
// parenthesized when rendered as the element of `T[]`. The rule is
// structural — non-`PrimaryType` element forms (`Union`, `Intersection`,
// `Function`, `Callable`, `Conditional`, `Infer`, `KeyOf`) always need
// parens because the postfix `[]` would otherwise bind tighter than the
// element form.
...
fn format_array_of_infer_is_parenthesized_with_renamed_binder() {
    // The rule is structural, not name-based: any infer var name must
    // produce the same parenthesized form.
    let db = TypeInterner::new();
    let mut fmt = TypeFormatter::new(&db);
...
fn format_array_of_keyof_is_parenthesized() {
    ...
    // keyof on an opaque type parameter — must stay as `keyof T` and be
    // wrapped when used as the element of `T[]`.
...
fn format_array_of_primitive_is_unparenthesized_control() {
    // Negative control: PrimaryType elements (here, a primitive intrinsic)
    // must NOT be wrapped — `number[]` not `(number)[]`.
    let db = TypeInterner::new();
    let mut fmt = TypeFormatter::new(&db);
...
fn format_readonly_array_of_infer_is_parenthesized() {
    // `ReadonlyArray<infer E>` -> `readonly (infer E)[]`. Same rule as the
    // mutable array path; the readonly shorthand must not lose the parens.
    let db = TypeInterner::new();
    let mut fmt = TypeFormatter::new(&db);
    ...
    // Build `ReadonlyArray<infer E>` via the Application shorthand path:
    // need a base type named `ReadonlyArray` and a single argument.
...
fn format_infer_with_constraint_includes_extends() {
    // `infer X extends C` must render its constraint; the bare `infer X`
    // form was the previous behavior even when `constraint` was set, which
    // dropped information from displayed conditional types.
    let db = TypeInterner::new();
    let mut fmt = TypeFormatter::new(&db);
