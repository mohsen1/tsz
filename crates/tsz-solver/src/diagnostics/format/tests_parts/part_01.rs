...
#[test]
fn preferred_application_alias_does_not_repaint_existing_mapped_alias() {
    let db = TypeInterner::new();
    let k = db.type_param(TypeParamInfo::simple(db.intern_string("K")));
    let mapped = db.mapped(MappedType {
        type_param: TypeParamInfo::simple(db.intern_string("P")),
        constraint: k,
        template: TypeId::NUMBER,
    ...
    let first_app = db.application(db.lazy(crate::def::DefId(7001)), vec![TypeId::NUMBER]);
    let later_app = db.application(db.lazy(crate::def::DefId(7002)), vec![TypeId::STRING]);
    ...
    assert_eq!(
        ...
        Some(first_app),
        "A later preferred application must not repaint an existing mapped display alias"
    );
}

#[test]
fn mapped_alias_application_expands_unrelated_argument_display_alias() {
    let db = TypeInterner::new();
    let def_store = crate::def::DefinitionStore::new();
    let t_param = TypeParamInfo::simple(db.intern_string("T"));
    ...
    let arg = db.object(vec![PropertyInfo::new(db.intern_string("c"), TypeId::NUMBER)]);
    let bar_def = def_store.register(crate::def::DefinitionInfo::type_alias(
        db.intern_string("Bar"),
        vec![],
        arg,
    ));
    let bar_app = db.application(db.lazy(bar_def), vec![]);
    db.store_display_alias(arg, bar_app);
    let id_app = db.application(db.lazy(id_def), vec![arg]);

    let mut fmt = TypeFormatter::new(&db).with_def_store(&def_store);
    let result = fmt.format(id_app);

    assert!(
        result.contains("Id<{ c: number; }>"),
        "mapped alias application args should not chase unrelated display aliases, got: {result}"
    );
    assert!(
        !result.contains("Bar"),
        "unrelated argument display alias should not leak into mapped alias application, got: {result}"
    );
}
