...
impl ModuleResolver {
    ...
    pub(super) fn resolve_bare_specifier(
        ...
        match self.try_self_reference_v2(
            ...
            SelfReferenceResultV2::ExportsFailed | SelfReferenceResultV2::NotSelfReference => {
    ...
    pub(super) fn resolve_package(
        ...
        //
        // For symlinked package roots with an explicit package.json, tsc expects an
        // explicit entry point (exports/main/types) and can report module-not-found
        // when none is configured; keep that behavior by skipping index fallback in
        // that specific case only.
        let is_symlinked_package_root = std::fs::symlink_metadata(package_dir)
            .map(|meta| meta.file_type().is_symlink())
            .unwrap_or(false);
        let has_package_json = package_dir.join("package.json").is_file();
