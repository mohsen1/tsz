    ...
    pub(super) fn resolve_bare_specifier(
        ...
        match self.try_self_reference_v2(
            ...
            SelfReferenceResultV2::ExportsFailed { .. } => {
                ...
                // TypeScript requires the project root to be determinable for self-references
                // with exports fields. When exports don't resolve, we should emit TS2209
                // to indicate the project root is ambiguous, regardless of whether rootDir
                // or outDir are set. This matches TypeScript's behavior where self-references
                // that fail export resolution should error rather than fall through to
                // node_modules or produce format mismatch errors.
                //
                // Find the package.json path for the error message
                let mut current = containing_dir.to_path_buf();
                let mut package_json_path = None;
                loop {
                    let candidate = current.join("package.json");
                    if candidate.is_file() {
                        if let Ok(pkg_json) = self.read_package_json(&candidate) {
                            if pkg_json.name.as_deref() == Some(&package_name) {
                                package_json_path = Some(candidate);
                                break;
                            }
                        }
                    }
                ...
                let subpath_key = match subpath.as_deref() {
                    Some(sp) => format!("./{sp}"),
                    None => ".".to_string(),
                };
