...
impl ModuleResolver {
    ...
    pub(super) fn resolve_bare_specifier(
        ...
        match self.try_self_reference_v2(
            ...
            SelfReferenceResultV2::AmbiguousRoot {
                ...
                // TS2209: Ambiguous project root for export map entry
                return Err(ResolutionFailure::AmbiguousProjectRoot {
                    export_map_entry,
                    package_json_path,
                    containing_file: containing_file.to_string(),
                    span: specifier_span,
    ...
    pub(super) fn resolve_package(
        ...
        if let Some(main) = &package_json.main {
            ...
            // For package.json "main", always try resolving unknown extensions.
            // The "main" field can legitimately point to any file type (.css, etc.)
            // and tsc resolves it regardless of allowArbitraryExtensions.
        ...
        // package.json is absent in node_modules. Skip the index fallback for symlinked
        // packages — these represent workspace-link directories where tsc requires an
        // explicit entry point (via exports/main/types) and emits TS2307 when none is
        // configured, even when an index file exists in the target directory.
