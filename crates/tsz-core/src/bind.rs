...
use crate::syntax::{
    AssignmentOperator, ClassDeclaration, ClassMember, ClassMemberKind,
    DeclarationHostBodyRepresentation, Expression, ExpressionKind, FunctionDeclaration,
    FunctionLikeBody, Literal, ParameterNameKind, PropertyNameKind, SourceUnit, Statement,
    StatementKind, SwitchClauseKind, TypeMember, TypeMemberKind, TypeMemberNameKind, TypeNode,
    TypeNodeKind, UnmodeledDeclarationHostFact, UnmodeledDeclarationHostKind, parse_number_literal,
};
...
use navigation::PendingReferenceFact;
...
/// Binder-owned identity for class member groups. Static and instance
/// declarations occupy distinct symbol spaces, matching TypeScript's class
/// constructor and instance sides.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub(crate) enum ClassMemberSymbol {
    Constructor,
    ReservedPrivateConstructor,
    ReservedStaticPrototype,
    Named {
        name: Vec<u16>,
        static_member: bool,
...
pub struct BoundFile {
    ...
    reference_facts: Vec<navigation::BoundReferenceFact>,
    this_type_references: FxHashMap<Span, DeclId>,
    /// Binder-owned value targets for local export specifiers, keyed by the
    /// authored reference span so downstream products never match user names.
    export_specifier_targets: FxHashMap<Span, DeclId>,
    pub type_member_groups: BTreeMap<(ScopeId, TypeMemberSymbol), Vec<DeclId>>,
    class_members: FxHashMap<NodeId, BoundClassMember>,
    class_member_groups: BTreeMap<(ScopeId, ClassMemberSymbol), ClassMemberGroupFacts>,
...
impl BoundFile {
    ...
    #[must_use]
    pub(crate) fn reference_facts(&self) -> &[navigation::BoundReferenceFact] {
        &self.reference_facts
    }
    #[must_use]
    pub(crate) fn reference_declaration(&self, span: Span) -> Option<DeclId> {
        self.reference_facts
            .iter()
            .find(|reference| reference.span == span)
            .and_then(|reference| reference.declaration(|_, _| None))
    }
    #[must_use]
    pub(crate) fn anonymous_type_literal(&self, span: Span) -> Option<DeclId> {
        self.declarations.iter().find_map(|declaration| {
            (declaration.kind == DeclarationKind::AnonymousType && declaration.name_span == span)
                .then_some(declaration.id)
        })
    }
    #[must_use]
    pub(crate) fn export_specifier_target(&self, span: Span) -> Option<&BoundDeclaration> {
        self.export_specifier_targets
            .get(&span)
    ...
    #[must_use]
    pub(crate) fn class_member_group(&self, node: NodeId) -> Option<&[DeclId]> {
        Some(&self.class_member_group_facts(node)?.declarations)
    }
    #[must_use]
    pub(crate) fn class_member_group_facts(&self, node: NodeId) -> Option<&ClassMemberGroupFacts> {
        let member = self.class_members.get(&node)?;
        self.class_member_groups
            .get(&(member.container, member.symbol.clone()))
    }
    #[must_use]
    pub(crate) fn class_member_symbol(&self, node: NodeId) -> Option<&ClassMemberSymbol> {
        self.class_members.get(&node).map(|member| &member.symbol)
    }
    #[must_use]
...
pub fn bind_source_with_kind(
    ...
    let mut binder = Binder {
        ...
        this_type_references: FxHashMap::default(),
        type_member_groups: BTreeMap::new(),
        class_members: FxHashMap::default(),
        class_member_groups: BTreeMap::new(),
    ...
    let mut bound = BoundFile {
        ...
        reference_facts: Vec::new(),
        this_type_references: binder.this_type_references,
        export_specifier_targets: FxHashMap::default(),
        type_member_groups: binder.type_member_groups,
        class_members: binder.class_members,
        class_member_groups: binder.class_member_groups,
    ...
    bound.reference_facts = navigation::finish(&bound, binder.reference_facts);
    for statement in &unit.statements {
        let StatementKind::Export(declaration) = &statement.kind else {
            continue;
        };
        if declaration.module_specifier.is_some() {
            continue;
        }
        ...
        {
            if let Some(target) = bound.resolve(ScopeId(0), &specifier.local, Meaning::Value) {
                bound
                    .export_specifier_targets
                    .insert(specifier.local_span, target);
            }
        }
    }
    bound
...
struct Binder {
    ...
    reference_facts: Vec<PendingReferenceFact>,
    this_type_references: FxHashMap<Span, DeclId>,
...
impl Binder {
    fn bind_statement(&mut self, statement: &Statement, scope: ScopeId, control: Option<NodeId>) {
        ...
        for host in &unmodeled_hosts {
            ...
            let meanings: &[Meaning] = match host.kind {
                UnmodeledDeclarationHostKind::Enum
                | UnmodeledDeclarationHostKind::Namespace
                | UnmodeledDeclarationHostKind::Module => &[Meaning::Value, Meaning::Type],
        ...
        if let Some(host) = unmodeled_hosts
            .iter()
            .find(|host| host.body == DeclarationHostBodyRepresentation::ParsedStatements)
        {
            let StatementKind::Block(statements) = &statement.kind else {
                debug_assert!(false, "parsed declaration-host body must retain statements");
                return;
            };
            let body_scope = match host.kind {
                UnmodeledDeclarationHostKind::Namespace
                | UnmodeledDeclarationHostKind::Module
                | UnmodeledDeclarationHostKind::ExternalModule => {
                    self.new_scope(scope, statement.id)
                }
                UnmodeledDeclarationHostKind::Global => scope,
                UnmodeledDeclarationHostKind::Enum | UnmodeledDeclarationHostKind::Using => {
                    debug_assert!(
                        false,
                        "opaque declaration host cannot own parsed statements"
                    );
                    return;
                }
            };
            for nested in statements {
                self.bind_statement(nested, body_scope, control);
            }
            return;
        }
        if self.unmodeled_declaration_hosts.iter().any(|host| {
            host.body == DeclarationHostBodyRepresentation::Omitted
        ...
        match &statement.kind {
            ...
            StatementKind::Variable(variable) => {
                for declarator in &variable.declarators {
                    let declared = if declarator.recovered_binding_names.is_empty() {
                        Some(self.declare(
                            scope,
                            statement.id,
                            &declarator.name,
                        ...
                        for binding in &declarator.recovered_binding_names {
                            self.declare(
                                scope,
                                statement.id,
                                &binding.name,
                    ...
                    if let (Some(declared), None, Some(initializer)) =
                        (declared, &declarator.annotation, &declarator.initializer)
                    ...
                    if let (Some(declaration), Some(initializer)) =
                        (declared, &declarator.initializer)
                    ...
                    if let Some(annotation) = &declarator.annotation {
                        self.bind_type_node_with_owner(annotation, scope, Some(statement.id));
                    }
                }
            }
            ...
            StatementKind::TypeAlias(declaration) => {
                ...
                let type_scope = self.new_scope(scope, statement.id);
                self.bind_type_parameters(&declaration.type_parameters, type_scope);
                self.bind_type_node(&declaration.ty, type_scope);
            }
            StatementKind::Interface(declaration) => {
                let interface = self.declare(
                    scope,
                    statement.id,
                    &declaration.name,
                    declaration.name_span,
                ...
                self.bind_type_parameters(&declaration.type_parameters, member_scope);
                for extended in &declaration.extends {
                    self.bind_type_node(extended, member_scope);
                }
                self.scopes[member_scope.0 as usize].this_type_owner = Some(interface);
            ...
            StatementKind::Export(declaration) => {
                for specifier in &declaration.specifiers {
                    ...
                    self.reference_facts.push(PendingReferenceFact::name(
                        &specifier.local,
    ...
    fn bind_type_member(&mut self, member: &TypeMember, container: ScopeId) {
        let named_symbol = |name: &crate::syntax::TypeMemberName| match &name.kind {
            TypeMemberNameKind::Identifier(name) => {
                Some(TypeMemberSymbol::Named(name.encode_utf16().collect()))
            }
            TypeMemberNameKind::StringLiteral(_, value) => value
                .as_ref()
                .map(|value| TypeMemberSymbol::Named(value.units().to_vec())),
            TypeMemberNameKind::NumericLiteral(name) => parse_number_literal(name)
                .map(|number| TypeMemberSymbol::Named(number.display.encode_utf16().collect())),
            TypeMemberNameKind::BigIntLiteral(_) | TypeMemberNameKind::Computed(_) => None,
        };
        ...
        let (name, name_span) = match &symbol {
            Some(symbol) => match symbol {
                TypeMemberSymbol::Named(name) => {
                    ...
                    (String::from_utf16_lossy(name), span)
                }
    ...
    fn bind_type_parameters(
        ...
        let owner = self.scopes[scope.0 as usize]
            .owner
            .expect("type-parameter scope must have an owner");
    ...
    fn bind_type_node(&mut self, node: &TypeNode, scope: ScopeId) {
        let owner = self.nearest_scope_owner(scope);
        self.bind_type_node_with_owner(node, scope, owner);
    }
    fn bind_type_node_with_owner(
        &mut self,
        node: &TypeNode,
        scope: ScopeId,
        owner: Option<NodeId>,
        ...
        match &node.kind {
            ...
            | TypeNodeKind::Parenthesized(element) => {
                self.bind_type_node_with_owner(element, scope, owner);
            }
            TypeNodeKind::Tuple(arguments)
            | TypeNodeKind::Union(arguments)
            | TypeNodeKind::Intersection(arguments) => {
                for argument in arguments {
                    self.bind_type_node_with_owner(argument, scope, owner);
                }
            }
            TypeNodeKind::This => {
                if let Some(declaration) = self.scopes[scope.0 as usize].this_type_owner {
                    self.this_type_references.insert(node.span, declaration);
                }
            }
            ...
            TypeNodeKind::TypeQuery {
                ...
                self.reference_facts
                    .push(PendingReferenceFact::type_query_root(
                        root, root_span, scope,
                    ));
            }
            TypeNodeKind::Object(members) => {
                self.declare_unscoped(
                    scope,
                    owner.expect("object type literal must have a declaration owner"),
                    String::from("__type"),
                    node.span,
                    DeclarationKind::AnonymousType,
                    Meaning::Type,
                );
                let member_scope = self.new_type_literal_scope(scope);
            ...
            TypeNodeKind::Infer { constraint, .. } => {
                if let Some(constraint) = constraint {
                    self.bind_type_node_with_owner(constraint, scope, owner);
                }
            }
            TypeNodeKind::Predicate { ty, .. } => {
                if let Some(ty) = ty {
                    self.bind_type_node_with_owner(ty, scope, owner);
                }
            }
            TypeNodeKind::Conditional {
                ...
                self.bind_type_node_with_owner(check_type, scope, owner);
                self.bind_type_node_with_owner(extends_type, scope, owner);
                self.bind_type_node_with_owner(true_type, scope, owner);
                self.bind_type_node_with_owner(false_type, scope, owner);
            }
            TypeNodeKind::Mapped {
                parameter,
                parameter_span,
                ...
                self.bind_type_node_with_owner(constraint, scope, owner);
                let mapped_scope = self.new_anonymous_scope(scope);
                let owner = owner.expect("mapped type must have a declaration owner");
                self.declare_parameter(
                    mapped_scope,
                    owner,
                    parameter,
                ...
                if let Some(name_type) = name_type {
                    self.bind_type_node_with_owner(name_type, mapped_scope, Some(owner));
                }
                self.bind_type_node_with_owner(value_type, mapped_scope, Some(owner));
                self.bind_type_members(members, mapped_scope);
            }
            TypeNodeKind::IndexedAccess { object, index } => {
                self.bind_type_node_with_owner(object, scope, owner);
                self.bind_type_node_with_owner(index, scope, owner);
            }
    ...
    fn bind_class(
        ...
        let class_scope = self.new_scope_with_context(parent, Some(owner), None, None);
        self.bind_type_parameters(&declaration.type_parameters, class_scope);
        if let Some(extended) = &declaration.extends {
            self.bind_type_node(extended, class_scope);
        }
        for implemented in &declaration.implements {
            self.bind_type_node(implemented, class_scope);
        }
        for member in &declaration.members {
            self.bind_class_member_symbol(member, class_scope);
            ...
            let member_this_type = (!member.modifiers.static_member).then_some(instance);
            let (
                type_parameters,
                parameters,
                return_type,
            ...
            self.bind_type_parameters(type_parameters, member_scope);
            let body_scope = self.new_anonymous_scope_with_this_type(member_scope, body_this_type);
            self.bind_signature_types(parameters, return_type, member_scope, member_scope);
    ...
    fn bind_class_member_symbol(&mut self, member: &ClassMember, container: ScopeId) {
        let symbol = match &member.kind {
            ClassMemberKind::Constructor { .. } => Some(ClassMemberSymbol::Constructor),
            ClassMemberKind::Method { .. } | ClassMemberKind::Property { .. } => {
                let name = match member.name_kind {
                    PropertyNameKind::Identifier => Some(member.name.encode_utf16().collect()),
                    PropertyNameKind::StringLiteral => member
                        .string_name_value
                        .as_ref()
                        .map(|name| name.units().to_vec()),
                    PropertyNameKind::NumericLiteral => parse_number_literal(&member.name)
                        .map(|number| number.display.encode_utf16().collect()),
                    PropertyNameKind::PrivateIdentifier => {
                        Some(member.name.encode_utf16().collect())
                    }
                    PropertyNameKind::Computed | PropertyNameKind::Unsupported => None,
                };
                name.map(|name| {
                    let private_identifier =
                        member.name_kind == PropertyNameKind::PrivateIdentifier;
                    if private_identifier && name.iter().copied().eq("#constructor".encode_utf16())
                    {
                        ClassMemberSymbol::ReservedPrivateConstructor
                    } else if member.modifiers.static_member
                        && !private_identifier
                        && name.iter().copied().eq("prototype".encode_utf16())
                    {
                        ClassMemberSymbol::ReservedStaticPrototype
                    } else {
                        ClassMemberSymbol::Named {
                            name,
                            static_member: member.modifiers.static_member,
                            private_identifier,
                        }
                    }
                })
        ...
        match &member.kind {
            ...
            | ClassMemberKind::Method {
                ...
                group.callables += 1;
                group.implementations += usize::from(*has_body);
                group.private_callable |= member.modifiers.private;
            }
            ...
            ClassMemberKind::Method {
                ...
                crate::syntax::AccessorKind::Get => group.getters += 1,
                crate::syntax::AccessorKind::Set => group.setters += 1,
            },
        }
        self.class_members
            .insert(member.id, BoundClassMember { container, symbol });
    }
    fn bind_expression(
        &mut self,
        ...
        self.bind_expression_with_demand(expression, scope, control, FlowDemandPath::root(), false);
    }
        ...
        match &expression.kind {
            ...
            ExpressionKind::Call {
                ...
                for type_argument in type_arguments.iter().flatten() {
                    self.bind_type_node_with_owner(type_argument, scope, Some(expression.id));
                }
                for argument in arguments {
            ...
            ExpressionKind::Parenthesized(object) | ExpressionKind::NonNull(object) => {
                self.bind_expression_with_demand(object, scope, control, demand, write);
            }
            ...
            ExpressionKind::As {
                ...
                self.bind_expression(object, scope, control);
                self.bind_type_node_with_owner(ty, scope, Some(expression.id));
            }
            ...
            ExpressionKind::FunctionLike(function) => {
                ...
                let (lexical_this, this_type_owner, kind) = ordinary.map_or_else(
                    || {
                        (
                            self.scopes[scope.0 as usize].lexical_this,
                            self.scopes[scope.0 as usize].this_type_owner,
                            FlowContainerKind::Creation,
                        )
                    },
                    |_| (None, None, FlowContainerKind::Ordinary),
                );
                let function_scope =
                    self.new_flow_scope(scope, expression.id, lexical_this, this_type_owner, kind);
            ...
            ExpressionKind::Conditional {
                ...
                self.bind_expression(condition, scope, control);
                self.bind_expression(when_true, scope, control);
                self.bind_expression(when_false, scope, control);
            }
            ExpressionKind::Assignment {
                ...
                if let Some(target) = flow_assignment_root(left) {
                    self.flow_facts.mutations.push(PendingFlowMutation {
                        target: target.id,
                        source: (*operator == AssignmentOperator::Assign)
                            .then(|| simple_assignment_target(left))
                            .flatten()
    ...
    fn bind_function(&mut self, owner: NodeId, declaration: &FunctionDeclaration, parent: ScopeId) {
        let scope = self.new_flow_scope(parent, owner, None, None, FlowContainerKind::Ordinary);
    ...
    fn new_scope(&mut self, parent: ScopeId, owner: NodeId) -> ScopeId {
        let context = &self.scopes[parent.0 as usize];
        self.new_scope_with_context(
            parent,
            Some(owner),
            context.lexical_this,
            context.this_type_owner,
        )
    }
    fn new_scope_with_context(
        &mut self,
        parent: ScopeId,
        owner: Option<NodeId>,
        lexical_this: Option<LexicalThisOwner>,
    ...
    fn new_flow_scope(
        ...
        let scope = self.new_scope_with_context(parent, Some(owner), lexical_this, this_type_owner);
    ...
    fn new_anonymous_scope(&mut self, parent: ScopeId) -> ScopeId {
        let this_type_owner = self.scopes[parent.0 as usize].this_type_owner;
        self.new_anonymous_scope_with_this_type(parent, this_type_owner)
    }
    fn new_anonymous_scope_with_this_type(
        &mut self,
        parent: ScopeId,
        this_type_owner: Option<DeclId>,
    ) -> ScopeId {
        let lexical_this = self.scopes[parent.0 as usize].lexical_this;
        self.new_scope_with_context(parent, None, lexical_this, this_type_owner)
    }
    fn new_type_literal_scope(&mut self, parent: ScopeId) -> ScopeId {
        self.new_anonymous_scope_with_this_type(parent, None)
    }
    fn nearest_scope_owner(&self, mut scope: ScopeId) -> Option<NodeId> {
        loop {
            let current = self.scopes.get(scope.0 as usize)?;
    ...
    fn declare_parameter(
        ...
        let duplicate = self.scopes[scope.0 as usize]
            ...
            .is_some_and(|ids| {
                ids.iter()
                    .any(|id| self.declarations[id.local as usize].kind == kind)
            });
        if duplicate {
