...
impl StandardLibraryEnvironment {
    ...
    pub fn resolve(&self, name: &str, meaning: Meaning) -> Option<DeclId> {
        match_value!(meaning; Meaning::Value => self.value_names.get(name), Meaning::Type => self.type_names.get(name)).copied()
    }
    #[must_use]
    ...
    pub(crate) fn call_member(
        ...
        let member = match receiver {
            LibraryReceiver::Array => {
                ...
                match_value!(name;
                    "indexOf" => LibraryCallMember::IndexOf, "lastIndexOf" => LibraryCallMember::LastIndexOf, "map" => LibraryCallMember::Map,
                    "push" => LibraryCallMember::Push, "slice" => LibraryCallMember::Slice, "splice" => LibraryCallMember::Splice,
            ...
            LibraryReceiver::Declaration(owner) if self.is_map_type(owner) => {
                ...
                match_value!(name; "get" => LibraryCallMember::MapGet, "set" => LibraryCallMember::MapSet, _ => return LibraryMemberLookup::Missing)
            }
...
fn default_library_for_target(target: &str) -> &'static str {
    match_value!(target.trim().to_ascii_lowercase().as_str();
        "es2015" | "es6" => "es6", "es2016" | "es7" => "es2016.full", "es2017" => "es2017.full", "es2018" => "es2018.full", "es2019" => "es2019.full", "es2020" => "es2020.full",
        "es2021" => "es2021.full", "es2022" => "es2022.full", "es2023" => "es2023.full", "es2024" => "es2024.full", "esnext" | "latest" => "esnext.full", _ => "es2025.full",
    )
}
fn explicit_library_name(name: &str) -> Option<&'static str> {
    ...
    let alias = match_value!(name.as_str();
        "es6" => "es2015", "es7" => "es2016", "esnext.asynciterable" => "es2018.asynciterable", "esnext.symbol" => "es2019.symbol", "esnext.bigint" => "es2020.bigint",
        "esnext.weakref" => "es2021.weakref", "esnext.object" => "es2024.object", "esnext.regexp" => "es2024.regexp", "esnext.string" => "es2024.string",
        "esnext.float16" => "es2025.float16", "esnext.iterator" => "es2025.iterator", "esnext.promise" => "es2025.promise", _ => name.as_str(),
    );
