...
#[test]
fn semantic_defs_heritage_survives_multifile_merge() {
    let files = vec![
        (
            "base.ts".to_string(),
        ...
        (
            "iface.ts".to_string(),
            "export interface Printable { print(): void }".to_string(),
        ),
        (
            "child.ts".to_string(),
            "class Child extends Base implements Printable { print() {} }".to_string(),
        ),
    ];
    let results = parse_and_bind_parallel(files);
    let program = merge_bind_results(results);

    let child_entry = program
        .semantic_defs
        .values()
        .find(|e| e.name == "Child")
        .expect("expected Child in merged semantic_defs");
    assert_eq!(
        child_entry.extends_names,
        vec!["Base"],
        "extends_names should survive merge"
    );
    assert_eq!(
        child_entry.implements_names,
        vec!["Printable"],
        "implements_names should survive merge"
    );
}

#[test]
fn semantic_defs_interface_heritage_survives_merge() {
    let files = vec![
        (
            "a.ts".to_string(),
            "export interface A { a: number }".to_string(),
        ),
        (
            "b.ts".to_string(),
            "interface B extends A { b: string }".to_string(),
        ),
    ];
    let results = parse_and_bind_parallel(files);
    let program = merge_bind_results(results);
    ...
    let b_entry = program
        ...
        .values()
        .find(|e| e.name == "B")
        .expect("expected B in merged semantic_defs");
    assert_eq!(
        b_entry.extends_names,
        vec!["A"],
        "interface extends_names should survive merge"
    );
}

#[test]
fn semantic_defs_enum_members_survive_merge() {
    let files = vec![(
        "enums.ts".to_string(),
        "export enum Direction { Up, Down, Left, Right }".to_string(),
    )];
    let results = parse_and_bind_parallel(files);
    let program = merge_bind_results(results);

    let dir_entry = program
        .semantic_defs
        .values()
        .find(|e| e.name == "Direction")
        .expect("expected Direction in merged semantic_defs");
    assert_eq!(
        dir_entry.enum_member_names,
        vec!["Up", "Down", "Left", "Right"],
        "enum member names should survive merge"
    );
}

#[test]
fn semantic_defs_heritage_stable_across_repeated_merge() {
    let files = vec![(
        "types.ts".to_string(),
        "
interface Serializable { serialize(): string }
class Base<T> { value: T }
class Derived extends Base<number> implements Serializable { serialize() { return ''; } }
"
        .to_string(),
    )];
    let results1 = parse_and_bind_parallel(files.clone());
    ...
    let entry1 = program1
        ...
        .expect("expected Derived in first merge");
    let entry2 = program2
        .semantic_defs
        .values()
        .find(|e| e.name == "Derived")
        .expect("expected Derived in second merge");

    assert_eq!(
        entry1.extends_names, entry2.extends_names,
        "extends_names should be stable across merges"
    );
    assert_eq!(
        entry1.implements_names, entry2.implements_names,
        "implements_names should be stable across merges"
    );
}
