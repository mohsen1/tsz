...
impl ParserState {
    ...
    ///
    /// Mirrors tsc's
    /// `nextTokenIsUsingKeywordThenBindingIdentifierOrStartOfDestructuringOnSameLine`,
    /// whose second half uses the *non-`DisallowOf`* variant. Because `of` is a
    /// contextual keyword, tsc's `isBindingIdentifier()` treats it as a binding
    /// identifier, so `for (await using of x)` still enters the declaration path.
    /// `parseVariableDeclarationList` then recognizes the contextual `of` and
    /// produces an *empty* declaration list (TS1123). Only `in` (a reserved word)
    /// keeps this out of the declaration path.
