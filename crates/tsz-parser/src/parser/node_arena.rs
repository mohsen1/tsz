...
impl NodeArena {
    ...
    ///
    /// Sums the allocated capacity of every typed pool plus the interner.
    /// This is a rough estimate for diagnostics reporting (not exact heap
    /// accounting), but tracks actual `Vec` capacity so it reflects real
    /// allocator pressure.
    #[must_use]
    pub fn estimated_size_bytes(&self) -> usize {
        ...
        // Interner heap estimate
        size += self.interner.estimated_size_bytes();
