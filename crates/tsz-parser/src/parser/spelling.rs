...
mod tests {
    ...
    fn levenshtein_case_difference_is_cheap() {
        let dist = levenshtein_with_max("Abc", "abc", 5.0).unwrap();
    ...
    fn levenshtein_single_char_substitution() {
        ...
        let dist = levenshtein_with_max("asynd", "async", 3.0).unwrap();
        assert!(
    ...
    fn levenshtein_insertion() {
        ...
        let dist = levenshtein_with_max("classs", "class", 3.0).unwrap();
        assert!(
    ...
    fn levenshtein_deletion() {
        ...
        let dist = levenshtein_with_max("clas", "class", 3.0).unwrap();
        assert!(
            (dist - 1.0).abs() < f64::EPSILON,
            "expected 1.0, got {dist}"
        );
