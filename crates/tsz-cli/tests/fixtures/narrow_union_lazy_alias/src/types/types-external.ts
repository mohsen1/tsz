...
export type WritableWrap<T> = T extends any[]
	? number extends T["length"]
		? IsPlainArray<T> extends true
			? Wrap<T[number]>[]
			: WritableNonArrayWrap<T>
		: WritableNonArrayWrap<T>
	: WritableNonArrayWrap<T>
...
export type Wrap<T> = T extends PrimitiveType
	? T
...
export type DeltaListener = (deltas: Delta[], inverseDeltas: Delta[]) => void
 ...
 * Utility types
 */
type DeltasTuple<T> = readonly [T, Delta[], Delta[]]
...
type ReturnTypeWithDeltasIfNeeded<
	State,
	UseDeltas extends boolean
> = UseDeltas extends true ? DeltasTuple<State> : State
...
type InferRecipeFromCurried<Curried> = Curried extends (
	...
	? ReturnType<Curried> extends State
		? (
				wrap: Wrap<State>,
				...rest: Args
			) => ValidRecipeReturnType<Wrap<State>>
		: never
	: never
...
type InferCurriedFromRecipe<
	Recipe,
	UseDeltas extends boolean
> = Recipe extends (wrap: infer WrapState, ...args: infer RestArgs) => any // verify return type
	? ReturnType<Recipe> extends ValidRecipeReturnType<WrapState>
		? (
				base: Immutable<WrapState>,
				...args: RestArgs
			) => ReturnTypeWithDeltasIfNeeded<WrapState, UseDeltas> // N.b. we return mutable wrapstate, in case the recipe's first arg isn't read only, and that isn't expected as output either
...
> = Recipe extends (
	...
	? (
			...
			...args: RestArgs
		) => ReturnTypeWithDeltasIfNeeded<State, UseDeltas>
...
export interface IProduce {
	...
	<State>(
		recipe: (state: Wrap<State>) => ValidRecipeReturnType<State>
	): (state: State) => State
	<State, Args extends any[]>(
		recipe: (state: Wrap<State>, ...args: Args) => ValidRecipeReturnType<State>
	): (state: State, ...args: Args) => State
	...
	/** Normal producer */
	<Base, D = Wrap<Base>>( // By using a default inferred D, rather than Wrap<Base> in the recipe, we can override it.
		base: Base,
		recipe: (wrap: D) => ValidRecipeReturnType<D>,
 ...
 * a tuple is returned with [nextState, deltas, inverseDeltas]
 *
 * Like produce, this function supports currying
 */
export interface IProduceWithDeltas {
	// Types copied from IProduce, wrapped with DeltasTuple
 ...
 */
export type Producer<T> = (wrap: Wrap<T>) => ValidRecipeReturnType<Wrap<T>>
