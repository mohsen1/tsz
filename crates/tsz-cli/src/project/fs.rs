...
pub fn discover_ts_files(options: &FileDiscoveryOptions) -> Result<Vec<PathBuf>> {
    ...
    // family, JSON last when `resolveJsonModule` is set — and only flattens the
    // buckets once the walk completes, so a `.ts` file always precedes a sibling
    // `.js` file in the program's root-file order regardless of alphabetical
    // filename order. A single alphabetically-sorted `BTreeSet` loses that
    // bucket priority; partition it into the same three groups, preserving each
    // group's alphabetical sub-order.
...
mod tests {
    ...
    fn test_discover_pattern_matched_files_bucket_ts_before_js_regardless_of_name() {
        ...
        // flattens the buckets after the walk, so a `.ts` file always precedes
        // a sibling `.js` file in program root-file order even when the `.js`
        // file's name sorts alphabetically first. This ordering is load-bearing
        // for cross-file declaration-merge diagnostics such as TS2403 (which
        // declaration is "first" vs "subsequent").
