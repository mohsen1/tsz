...
pub(crate) fn text_may_contain_module_specifiers(text: &str) -> bool {
...
#[derive(Debug)]
pub(crate) struct DiscoveryToken {
    pub(super) kind: SyntaxKind,
    pub(super) text: Option<String>,
}

pub(crate) fn collect_simple_module_requests_from_text(
    text: &str,
...
pub(crate) fn strip_scanned_string_literal(text: &str) -> Option<String> {
...
pub(crate) fn skip_ambient_module_body_without_dependencies(
...
pub(crate) fn collect_simple_import_request(
    tokens: &[DiscoveryToken],
    import_idx: usize,
) -> Option<(String, usize)> {
...
pub(crate) fn collect_simple_export_request(
    tokens: &[DiscoveryToken],
    export_idx: usize,
...
pub(crate) fn matching_brace(tokens: &[DiscoveryToken], open_brace: usize) -> Option<usize> {
    let mut depth = 1usize;
    let mut i = open_brace + 1;
    while i < tokens.len() {
        match tokens[i].kind {
...
pub(crate) fn reject_import_attributes_after_string(
    tokens: &[DiscoveryToken],
    after_string: usize,
) -> Option<()> {
...
pub(crate) fn parse_jsdoc_import_module_specifier(rest: &str) -> Option<String> {
    let rest = rest.trim();
...
pub(crate) fn collect_module_specifiers_for_source_discovery(
    arena: &NodeArena,
    source_file: NodeIndex,
) -> Vec<CollectedModuleSpecifier> {
    collect_module_specifiers_impl(
...
pub(crate) fn leftmost_import_type_call(
    arena: &NodeArena,
    mut idx: NodeIndex,
) -> Option<NodeIndex> {
...
pub(crate) fn collect_non_static_module_specifiers(
    arena: &NodeArena,
...
pub(crate) fn is_dynamic_import_callee(arena: &NodeArena, idx: NodeIndex) -> bool {
...
pub(crate) fn import_type_resolution_mode_override(
    arena: &NodeArena,
...
pub(crate) fn import_attributes_resolution_mode(
    arena: &NodeArena,
...
pub(crate) fn import_attributes_has_type_json(
    arena: &NodeArena,
    attributes_idx: NodeIndex,
) -> bool {
    use tsz::parser::syntax_kind_ext;
...
pub(crate) fn extract_require_specifier(arena: &NodeArena, idx: NodeIndex) -> Option<String> {
    use tsz::parser::syntax_kind_ext;
...
pub(crate) fn collect_import_local_names(
    arena: &NodeArena,
