...
impl<'a> DecoratorTransformer<'a> {
    ...
    pub fn collect_decorators(&self, modifiers: &Option<tsz_parser::parser::NodeList>) -> Vec<NodeIndex> {
...
fn has_static_modifier(arena: &NodeArena, modifiers: &Option<tsz_parser::parser::NodeList>) -> bool {
    if let Some(mods) = modifiers {
        for &mod_idx in &mods.nodes {
