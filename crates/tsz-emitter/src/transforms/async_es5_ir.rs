...
impl<'a> AsyncES5Transformer<'a> {
    ...
    fn process_for_await_statement_in_async(
        &mut self,
        idx: NodeIndex,
        cases: &mut Vec<IRGeneratorCase>,
        current_statements: &mut Vec<IRNode>,
        ...
        if !for_in_of.await_modifier
            || super::emit_utils::for_of_using_info(self.arena, for_in_of.initializer).is_some()
        {
            return false;
        }
    ...
    fn simple_for_await_iteration_target(
        ...
        if node.kind == tsz_scanner::SyntaxKind::Identifier as u16 {
            let name = super::emit_utils::identifier_text_or_empty(self.arena, initializer);
