...
fn probe_abstract_class_emit() {
    ...
    assert!(output.contains("export declare abstract class Shape"), "Missing abstract: {output}");
    assert!(output.contains("abstract area(): number;"), "Missing abstract method: {output}");
    assert!(output.contains("abstract readonly name: string;"), "Missing abstract readonly: {output}");
}
...
fn probe_const_enum_emit() {
    ...
    assert!(output.contains("export declare const enum Direction"), "Missing const enum: {output}");
    assert!(output.contains("Up = \"UP\""), "Missing enum member: {output}");
}
...
export function foo(x: any): any { return x; }"#,
    ...
    assert!(output.contains("export declare function foo(x: string): number;"), "Missing overload 1: {output}");
    assert!(output.contains("export declare function foo(x: number): string;"), "Missing overload 2: {output}");
    // Implementation should NOT be emitted
    assert!(!output.contains("x: any): any"), "Implementation leaked: {output}");
}
...
fn probe_default_export_function() {
    ...
    assert!(output.contains("export default function foo(): void;"), "Missing default fn: {output}");
}
...
fn probe_template_literal_type() {
    ...
    assert!(output.contains("`${'click' | 'scroll'}_handler`") || output.contains("`${\"click\" | \"scroll\"}_handler`"), "Missing template literal: {output}");
}

#[test]
fn probe_mapped_type_as_clause() {
    let output = emit_dts("export type Getters<T> = { [K in keyof T as `get${string & K}`]: () => T[K] };");
    println!("PROBE mapped as:\n{output}");
    assert!(output.contains("as `get${string & K}`"), "Missing as clause: {output}");
}
...
fn probe_conditional_type() {
    ...
    assert!(output.contains("T extends string ? true : false"), "Missing conditional: {output}");
}
...
fn probe_call_construct_signatures() {
    ...
    assert!(output.contains("(arg: string): object;"), "Missing call sig: {output}");
    assert!(output.contains("new (arg: string): object;"), "Missing construct sig: {output}");
}
...
fn probe_named_tuple_members() {
    ...
    assert!(output.contains("x: number"), "Missing named member x: {output}");
    assert!(output.contains("z?: number"), "Missing optional named member z: {output}");
}
...
fn probe_unique_symbol() {
    ...
    assert!(output.contains("unique symbol"), "Missing unique symbol: {output}");
}
...
fn probe_type_predicate() {
    ...
    assert!(output.contains("x is string"), "Missing type predicate: {output}");
}
...
fn probe_assertion_function_with_type() {
    ...
    assert!(output.contains("asserts x is string"), "Missing assertion: {output}");
}
...
fn probe_parameter_properties() {
    ...
    assert!(output.contains("readonly x: number;"), "Missing readonly x: {output}");
    // tsc strips type annotations from private members in .d.ts
    assert!(output.contains("private y;"), "Missing private y: {output}");
    assert!(output.contains("protected z: boolean;"), "Missing protected z: {output}");
}
...
fn probe_constructor_type() {
    ...
    assert!(output.contains("new (x: string) => object"), "Missing constructor type: {output}");
}
...
fn probe_abstract_constructor_type() {
    ...
    assert!(output.contains("abstract new (x: string) => object"), "Missing abstract constructor: {output}");
}
...
fn probe_declare_module() {
    ...
    assert!(output.contains("declare module 'my-module'") || output.contains("declare module \"my-module\""), "Missing declare module: {output}");
    assert!(output.contains("function foo(): void;"), "Missing fn in module: {output}");
}
...
fn probe_generic_class_with_constraint() {
    ...
    assert!(output.contains("T extends object"), "Missing constraint: {output}");
...
fn probe_readonly_array_type() {
    ...
    assert!(output.contains("readonly string[]"), "Missing readonly array: {output}");
}
...
fn probe_indexed_access_type() {
    ...
    assert!(output.contains("string[][0]"), "Missing indexed access: {output}");
}
...
fn probe_optional_tuple_element() {
    ...
    assert!(output.contains("string?"), "Missing optional element: {output}");
}
...
fn probe_rest_tuple_element() {
    ...
    assert!(output.contains("...number[]"), "Missing rest element: {output}");
}
...
export interface C extends A, B { c: boolean; }",
    ...
    assert!(output.contains("extends A, B"), "Missing multi extends: {output}");
}
...
fn probe_export_default_abstract_class() {
    ...
    assert!(output.contains("export default abstract class"), "Missing default abstract: {output}");
    assert!(output.contains("abstract foo(): void;"), "Missing abstract method: {output}");
}
...
export declare enum E { A }",
    ...
    assert!(output.contains("export declare function foo(): void;"), "Missing declare fn: {output}");
    assert!(output.contains("export declare class Bar"), "Missing declare class: {output}");
    assert!(output.contains("export declare const baz: number;"), "Missing declare const: {output}");
    assert!(output.contains("export declare enum E"), "Missing declare enum: {output}");
}
...
export = Foo;",
    ...
    assert!(output2.contains("import Foo"), "Missing import (with binding): {output2}");
    assert!(output2.contains("export = Foo;"), "Missing export = (with binding): {output2}");
}
...
export class Doc implements Printable {
    ...
    assert!(output.contains("implements Printable"), "Missing implements: {output}");
}
...
export class Derived extends Base<string> {
    ...
    assert!(output.contains("extends Base<string>"), "Missing generic extends: {output}");
}
...
fn probe_mapped_type_modifiers() {
    ...
    assert!(output.contains("readonly [K in string]"), "Missing readonly mapped: {output}");
}
...
fn probe_infer_with_extends_constraint() {
    ...
    assert!(output.contains("infer V extends string"), "Missing infer extends: {output}");
}
...
fn probe_class_method_overloads() {
    ...
    assert!(output.contains("bar(x: string): number;"), "Missing overload 1: {output}");
    assert!(output.contains("bar(x: number): string;"), "Missing overload 2: {output}");
    assert!(!output.contains("x: any): any"), "Implementation leaked: {output}");
}
...
fn probe_export_star_as_namespace() {
    ...
    assert!(output.contains("export * as ns from"), "Missing star-as: {output}");
}
...
fn probe_type_only_reexport() {
    ...
    assert!(output.contains("export type {") || output.contains("export type{"), "Missing type reexport: {output}");
}
...
fn probe_default_type_parameter() {
    ...
    assert!(output.contains("U = string"), "Missing default type param: {output}");
}
...
fn probe_class_static_method() {
    ...
    assert!(output.contains("static create(): Foo;") || output.contains("static create():"), "Missing static method: {output}");
}
...
fn probe_class_protected_abstract_method() {
    ...
    assert!(output.contains("protected abstract init(): void;"), "Missing protected abstract: {output}");
}
...
fn probe_readonly_property_in_interface() {
    ...
    assert!(output.contains("readonly bar: string;"), "Missing readonly: {output}");
}
...
fn probe_optional_method_in_interface() {
    ...
    assert!(output.contains("bar?(x: number): void;"), "Missing optional method: {output}");
}
...
fn probe_export_default_type_alias() {
    ...
    assert!(output.contains("export default interface Foo"), "Missing default interface: {output}");
}
...
fn probe_enum_string_values() {
    ...
    assert!(output.contains("Active = \"active\"") || output.contains("Active = 'active'"), "Missing string value: {output}");
}
...
fn probe_enum_computed_values() {
    ...
    assert!(output.contains("C = 3") || output.contains("C ="), "Missing computed enum: {output}");
}
...
fn probe_class_with_index_signature() {
    ...
    assert!(output.contains("[key: string]: any;"), "Missing index sig: {output}");
...
fn probe_never_return_type() {
    ...
    assert!(output.contains(": never;"), "Missing never return: {output}");
}
...
fn probe_variadic_tuple() {
    let output = emit_dts("export type Concat<T extends unknown[], U extends unknown[]> = [...T, ...U];");
    println!("PROBE variadic tuple:\n{output}");
    assert!(output.contains("[...T, ...U]"), "Missing variadic: {output}");
}
...
fn probe_nested_namespace() {
    ...
    assert!(output.contains("function foo(): void;"), "Missing foo: {output}");
}
...
fn probe_export_namespace_with_type_and_value() {
    ...
    assert!(output.contains("interface I"), "Missing interface I: {output}");
    assert!(output.contains("function f(): I;"), "Missing fn f: {output}");
    assert!(output.contains("const c: number;"), "Missing const c: {output}");
}
...
declare global {
    ...
    assert!(output.contains("declare global"), "Missing global: {output}");
    assert!(output.contains("interface Window"), "Missing Window: {output}");
}
...
fn probe_function_with_this_param() {
    ...
    assert!(output.contains("this: HTMLElement"), "Missing this param: {output}");
}
...
fn probe_class_constructor_overloads() {
    ...
    assert!(output.contains("constructor(x: string);"), "Missing ctor overload 1: {output}");
    assert!(output.contains("constructor(x: number);"), "Missing ctor overload 2: {output}");
    assert!(!output.contains("x: any)"), "Ctor implementation leaked: {output}");
}
...
fn probe_rest_parameter() {
    ...
    assert!(output.contains("...args: string[]"), "Missing rest param: {output}");
}
...
fn probe_optional_parameter() {
    ...
    assert!(output.contains("x?: number"), "Missing optional param: {output}");
}
...
fn probe_parameter_with_default() {
    ...
    assert!(output.contains("x?: number"), "Default param should be optional: {output}");
}
...
fn probe_class_accessor_keyword() {
    ...
    assert!(output.contains("accessor bar: string;"), "Missing accessor keyword: {output}");
}
...
fn probe_satisfies_stripped() {
    ...
    assert!(!output.contains("satisfies"), "satisfies should be stripped: {output}");
}
...
fn probe_private_constructor() {
    ...
    assert!(output.contains("private constructor();"), "Missing private ctor: {output}");
}
...
fn probe_abstract_class_with_protected_constructor() {
    ...
    assert!(output.contains("protected constructor(x: number);"), "Missing protected ctor: {output}");
}
...
fn exact_probe_method_with_optional_and_rest() {
    ...
    let expected = "export declare function foo(a: string, b?: number, ...rest: boolean[]): void;\n";
...
fn exact_probe_export_default_function_no_name() {
    ...
    assert!(output.contains("export default function"), "Missing default fn: {output}");
}
...
fn exact_probe_export_default_class_no_name() {
    ...
    assert!(output.contains("export default class"), "Missing default class: {output}");
}
...
fn exact_probe_async_function() {
    ...
    assert!(!output.contains("async"), "async should be stripped in .d.ts: {output}");
    assert!(output.contains("foo(): Promise<number>;"), "Missing return type: {output}");
}
...
fn exact_probe_generator_function() {
    ...
    // tsc strips * in .d.ts
    assert!(!output.contains("*"), "* should be stripped in .d.ts: {output}");
}
...
export class Derived extends Base implements I {
    ...
    assert!(output.contains("extends Base implements I"), "Missing extends+implements: {output}");
}
...
fn exact_probe_complex_mapped_type() {
    ...
    assert!(output.contains("[P in keyof T]-?: T[P]"), "Missing mapped type body: {output}");
}
...
fn exact_probe_intersection_of_unions() {
    ...
    assert!(output.contains("(string | number) & (boolean | null)"), "Missing parens in intersection: {output}");
}

#[test]
fn exact_probe_nested_conditional() {
    let output = emit_dts("export type T<U> = U extends string ? 'str' : U extends number ? 'num' : 'other';");
    println!("EXACT nested conditional:\n{output}");
    assert!(output.contains("U extends string"), "Missing outer extends: {output}");
    assert!(output.contains("U extends number"), "Missing inner extends: {output}");
}
...
fn exact_probe_typeof_import() {
    ...
    assert!(output.contains("typeof import"), "Missing typeof import: {output}");
}
...
fn exact_probe_class_with_optional_property() {
    ...
    assert!(output.contains("bar?: string;"), "Missing optional prop: {output}");
    // tsc strips ! definite assignment in .d.ts
    assert!(output.contains("baz: number;") || output.contains("baz!: number;"), "Missing definite prop: {output}");
...
fn exact_probe_interface_with_generic_method() {
    ...
    assert!(output.contains("bar<T>(x: T): T;"), "Missing generic method: {output}");
}
...
fn exact_probe_function_type_in_union() {
    ...
    assert!(output.contains("((x: number) => void)") || output.contains("(x: number) => void"), "Missing fn type: {output}");
}
...
fn exact_probe_this_type() {
    ...
    assert!(output.contains("set(key: string): this;"), "Missing this type: {output}");
}
...
fn exact_probe_string_index_signature() {
    ...
    assert!(output.contains("[key: string]: unknown;"), "Missing index sig: {output}");
}
...
fn exact_probe_number_index_signature() {
    ...
    assert!(output.contains("[index: number]: string;"), "Missing index sig: {output}");
}
...
fn compare_tsc_complex_class_emit() {
    ...
    //     map<U>(fn: (item: T) => U): U[];
    // }
    assert!(output.contains("private items;"), "Private should strip type: {output}");
    assert!(output.contains("add(item: T): void;"), "Missing add: {output}");
    assert!(output.contains("get count(): number;"), "Missing getter: {output}");
    assert!(output.contains("map<U>(fn: (item: T) => U): U[];"), "Missing map: {output}");
}
...
fn compare_tsc_deep_partial_type() {
    ...
    //     [P in keyof T]?: T[P] extends object ? DeepPartial<T[P]> : T[P];
    // };
    assert!(output.contains("[P in keyof T]?: T[P] extends object ? DeepPartial<T[P]> : T[P]"), "Missing mapped type body: {output}");
}
...
fn compare_tsc_promisified_type() {
    ...
    assert!(output.contains("Promise<R>"), "Missing Promise<R>: {output}");
}
...
fn compare_tsc_factory_function() {
    ...
    assert!(output.contains("createFactory<T extends new (...args: any[]) => any>"), "Missing generics: {output}");
    assert!(output.contains("ctor: T"), "Missing param: {output}");
    assert!(output.contains("InstanceType<T>"), "Missing return: {output}");
}
...
fn edge_private_field_emits_as_hash_private() {
    ...
    assert!(output.contains("#"), "Missing private field marker: {output}");
...
export class Derived extends Base {
    ...
    assert!(output.contains("greet(): string;"), "Missing greet: {output}");
}
...
fn edge_recursive_type() {
    ...
    assert!(output.contains("[key: string]: Json"), "Missing index sig: {output}");
}
...
fn edge_nested_conditional_type() {
    ...
    assert!(output.contains("undefined extends T ? true : null extends T ? true : false"), "Missing nested conditional: {output}");
}
...
fn edge_template_literal_simple() {
    ...
    assert!(output.contains("`--${string}`"), "Missing template literal: {output}");
}
...
fn edge_interface_method_overloads() {
    ...
    assert!(output.contains("call(x: string): number;"), "Missing overload 1: {output}");
    assert!(output.contains("call(x: number): string;"), "Missing overload 2: {output}");
}
...
fn edge_generic_default_conditional() {
    ...
    assert!(output.contains("D = T extends string"), "Missing default conditional: {output}");
}
...
export namespace Status {
    ...
    assert!(output.contains("namespace Status"), "Missing namespace: {output}");
    assert!(output.contains("parse(s: string): Status;"), "Missing parse fn: {output}");
}
...
export type { Base };"#,
    ...
    assert!(output.contains("export type { Base };") || output.contains("export type {Base}"), "Missing type re-export: {output}");
}
...
export interface WithSymbol {
    ...
    assert!(output.contains("[sym]: string;"), "Missing computed symbol property: {output}");
}
...
fn edge_const_assertion_readonly_object() {
    ...
    assert!(output.contains("readonly port: 3000;"), "Missing readonly port: {output}");
    assert!(output.contains("readonly host: \"localhost\";"), "Missing readonly host: {output}");
}
...
fn fix_string_literal_escaped_quote() {
    ...
    assert!(output.contains(r#""quote\"mark""#), "Missing escaped quote: {output}");
}
...
fn fix_string_literal_escaped_backslash() {
    ...
    assert!(output.contains(r#""backslash\\path""#), "Missing escaped backslash: {output}");
}
...
fn fix_string_literal_escaped_newline() {
    ...
    assert!(output.contains(r#""line\nbreak""#), "Missing escaped newline: {output}");
}
...
fn fix_string_literal_escaped_tab() {
    ...
    assert!(output.contains(r#""tab\there""#), "Missing escaped tab: {output}");
}
...
fn fix_string_literal_single_quote_escape() {
    ...
    assert!(output.contains("'it\\'s'"), "Missing escaped single quote: {output}");
}
...
fn fix_string_literal_no_escape_needed() {
    ...
    assert!(output.contains(r#""normal""#), "Missing normal string: {output}");
}
...
fn fix_string_literal_combined_escapes() {
    ...
    assert!(output.contains(r#""a\\b\"c\nd""#), "Missing combined escapes: {output}");
}
...
fn fix_enum_string_value_escaped_newline() {
    ...
    assert!(output.contains(r#""hello\nworld""#), "Enum string value should escape newline: {output}");
}
...
fn fix_enum_string_value_escaped_tab() {
    ...
    assert!(output.contains(r#""tab\there""#), "Enum string value should escape tab: {output}");
}
...
fn fix_enum_string_value_escaped_backslash() {
    ...
    assert!(output.contains(r#""back\\slash""#), "Enum string value should escape backslash: {output}");
}
...
fn fix_enum_string_value_escaped_quote() {
    ...
    assert!(output.contains(r#""he said \"hi\"""#), "Enum string value should escape quote: {output}");
}
