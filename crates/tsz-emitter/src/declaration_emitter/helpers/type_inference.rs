...
impl<'a> DeclarationEmitter<'a> {
        ...
        if type_args.is_empty() {
            if let Some(type_id) = self.get_node_type_or_names(&[expr_idx]) {
                ...
                if let Some(type_text) = self
                    .new_expression_variable_constructor_alias_instance_type_text(
                        new_expr.expression,
                        &inferred,
                    )
                {
    ...
    fn new_expression_variable_constructor_alias_instance_type_text(
        &self,
        expr_idx: NodeIndex,
    ...
    fn variable_constructor_alias_initializer(&self, expr_idx: NodeIndex) -> Option<NodeIndex> {
        ...
        let Some(symbol) = self.binder.and_then(|binder| binder.symbols.get(sym_id)) else {
            return None;
        };
