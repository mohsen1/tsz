...
impl<'a> DeclarationEmitter<'a> {
    ...
    fn parse_jsdoc_function_type_signature(type_text: &str) -> Option<JsdocFunctionTypeSignature> {
        ...
        if let Some(after_open) = rest.strip_prefix('<') {
            let close = Self::find_matching_delimiter(after_open, '<', '>')?;
            type_params = Self::split_top_level(after_open[..close].trim(), ',')
                .into_iter()
                .map(str::trim)
        ...
        let close = Self::find_matching_delimiter(after_params, '(', ')')?;
        ...
        for raw_param in Self::split_top_level(params_text, ',') {
            let raw_param = raw_param.trim();
            if raw_param.is_empty() {
                continue;
            }
            let colon = Self::find_top_level_char(raw_param, ':')?;
