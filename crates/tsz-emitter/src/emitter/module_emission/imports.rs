...
impl<'a> Printer<'a> {
    pub(in crate::emitter) fn import_has_value_usage_after_node(
        ...
        let value_haystack = Self::strip_type_only_content(haystack);
        names
            .iter()
            .any(|name| Self::contains_identifier_occurrence(&value_haystack, name))
    }
    ...
    fn strip_type_only_content(source: &str) -> String {
        ...
        for line in source.lines() {
            ...
            let stripped = Self::strip_type_annotations_safe(line);
        ...
        while i < len {
            match bytes[i] {
                    ...
                    && (bytes[i - 1] == b')' || bytes[i - 1] == b'?' || bytes[i - 1] == b']') =>
                {
                    ...
                    i += 1;
                    i = Self::skip_type_annotation(bytes, i);
                }
                // On const/let/var lines, `:` before the first `=` is a type annotation
                b':' if is_var_line && Self::is_var_type_annotation_colon(line, i) => {
                    i += 1;
                    i = Self::skip_type_annotation(bytes, i);
                }
                _ => {
    ...
    pub(in crate::emitter) fn import_equals_has_value_usage_after_node(
        ...
        let haystack = &source_text[start..];
        let value_haystack = Self::strip_type_only_content(haystack);
        Self::contains_identifier_occurrence(&value_haystack, &name)
    }
