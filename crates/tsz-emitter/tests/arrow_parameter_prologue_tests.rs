...
#[test]
fn es5_statement_class_computed_instance_field_stays_inside_iife() {
    // Statement-level class declarations: temps stay inside the IIFE (tsc behavior).
    let output = emit_with_target("class C { [x] = x }", ScriptTarget::ES5);

    assert!(
        output.contains("var _a;\n    _a = x;"),
        "Computed instance-field temp must stay inside the IIFE for a statement class.\nOutput:\n{output}"
    );
    assert!(
        !output.contains("var _a;\nvar C"),
        "Computed instance-field temp must NOT be hoisted outside the IIFE for a statement class.\nOutput:\n{output}"
    );
}
