...
///
/// Drives the underlying scanner and returns a JSON array of `TokenInfo`
/// entries — `{ kind, text, start, end }` — covering every token in the
/// source, including trivia, up to (but excluding) the final
/// `EndOfFileToken`. Positions are byte offsets into the input.
#[wasm_bindgen(js_name = scanTokens)]
pub fn scan_tokens(source_text: &str) -> String {
    let mut scanner = tsz_scanner::scanner_impl::ScannerState::new(source_text.to_string(), false);
    let mut tokens: Vec<TokenInfo> = Vec::new();

    loop {
        let kind = scanner.scan();
        if kind == tsz_scanner::SyntaxKind::EndOfFileToken {
            break;
        }
