...
// =============================================================================
// Enriched SemanticDefEntry tests (enum members, is_const, is_abstract)
// =============================================================================

#[test]
fn semantic_defs_enum_captures_member_names() {
    let binder = bind_source("enum Color { Red, Green, Blue }");
    let sym_id = binder.file_locals.get("Color").expect("expected Color");
    let entry = binder
        .semantic_defs
        .get(&sym_id)
        .expect("expected semantic def for enum Color");
    assert_eq!(entry.enum_member_names, vec!["Red", "Green", "Blue"]);
}

#[test]
fn semantic_defs_const_enum_captures_is_const() {
    let binder = bind_source("const enum Direction { Up, Down, Left, Right }");
    let sym_id = binder
        .file_locals
        .get("Direction")
        .expect("expected Direction");
    let entry = binder
        .semantic_defs
        .get(&sym_id)
        .expect("expected semantic def for const enum Direction");
    assert!(entry.is_const, "const enum should have is_const=true");
    assert_eq!(entry.enum_member_names, vec!["Up", "Down", "Left", "Right"]);
}

#[test]
fn semantic_defs_non_const_enum_not_const() {
    let binder = bind_source("enum Status { Active, Inactive }");
    let sym_id = binder.file_locals.get("Status").expect("expected Status");
    let entry = binder
        .semantic_defs
        .get(&sym_id)
        .expect("expected semantic def for enum Status");
    assert!(!entry.is_const, "non-const enum should have is_const=false");
}

#[test]
fn semantic_defs_abstract_class_captures_is_abstract() {
    let binder = bind_source("abstract class Base { abstract getValue(): number; }");
    let sym_id = binder.file_locals.get("Base").expect("expected Base");
    let entry = binder
        .semantic_defs
        .get(&sym_id)
        .expect("expected semantic def for abstract class Base");
    assert!(
        entry.is_abstract,
...
#[test]
fn semantic_defs_non_abstract_class_not_abstract() {
    let binder = bind_source("class Concrete {}");
    let sym_id = binder
        .file_locals
        .get("Concrete")
        .expect("expected Concrete");
    let entry = binder
        .semantic_defs
        .get(&sym_id)
        .expect("expected semantic def for class Concrete");
    assert!(
        !entry.is_abstract,
...
#[test]
fn semantic_defs_enum_member_names_empty_for_non_enum() {
    let binder = bind_source("class Foo {}");
    let sym_id = binder.file_locals.get("Foo").expect("expected Foo");
    let entry = binder
        .semantic_defs
        ...
        .expect("expected semantic def for class Foo");
    assert!(
        entry.enum_member_names.is_empty(),
        "non-enum should have empty enum_member_names"
    );
}

#[test]
fn semantic_defs_enum_members_survive_rebuild() {
    let source = "enum Color { Red, Green, Blue }";
    let binder1 = bind_source(source);
    let binder2 = bind_source(source);

    let sym1 = binder1.file_locals.get("Color").unwrap();
    let sym2 = binder2.file_locals.get("Color").unwrap();
    let entry1 = &binder1.semantic_defs[&sym1];
    let entry2 = &binder2.semantic_defs[&sym2];

    assert_eq!(
        entry1.enum_member_names, entry2.enum_member_names,
        "enum member names should be identical across rebuilds"
    );
    assert_eq!(entry1.is_const, entry2.is_const);
}

#[test]
fn semantic_defs_enum_members_survive_lib_merge() {
    // Enum member names should survive the lib merge path.
    let lib_source = "enum Direction { Up, Down, Left, Right }";
    let mut lib_parser = ParserState::new("lib.d.ts".to_string(), lib_source.to_string());
    let lib_root = lib_parser.parse_source_file();
    let mut lib_binder = BinderState::new();
    lib_binder.bind_source_file(lib_parser.get_arena(), lib_root);
    ...
    let sym_id = main_binder
        .file_locals
        .get("Direction")
        .expect("expected Direction after merge");
    let entry = main_binder
        .semantic_defs
        .get(&sym_id)
        .expect("expected semantic def for Direction after merge");
    assert_eq!(
        entry.enum_member_names,
        vec!["Up", "Down", "Left", "Right"],
        "enum member names should survive lib merge"
    );
}

#[test]
fn semantic_defs_abstract_class_survives_rebuild() {
    let source = "abstract class Base {}";
    let binder1 = bind_source(source);
    let binder2 = bind_source(source);

    let sym1 = binder1.file_locals.get("Base").unwrap();
    let sym2 = binder2.file_locals.get("Base").unwrap();
    let entry1 = &binder1.semantic_defs[&sym1];
    let entry2 = &binder2.semantic_defs[&sym2];

    assert_eq!(
        entry1.is_abstract, entry2.is_abstract,
        "is_abstract flag should be identical across rebuilds"
    );
    assert!(entry1.is_abstract);
}
