...
impl BinderState {
    ...
    fn bind_node_by_node_kind(&mut self, arena: &NodeArena, node: &Node, idx: NodeIndex) {
        match node.kind {
            ...
            // Namespace Export (UMD)
            k if k == syntax_kind_ext::NAMESPACE_EXPORT_DECLARATION => {
                if let Some(decl) = arena.get_export_decl(node)
                    && let Some(name_node) = arena.get(decl.export_clause)
                    && let Some(ident) = arena.get_identifier(name_node)
                {
                    ...
                    if self.scope_stack.is_empty() {
                        ...
                        self.declare_symbol(name, flags, decl_node, false);
                    } else {
                        // We are in a module, need to inject into global scope (stack[0])
                        // 1. Check if symbol exists in global scope
                        let existing_id = self.scope_stack[0].get(name);

                        if let Some(id) = existing_id {
                            // Symbol exists, merge declaration
                            if let Some(sym) = self.symbols.get_mut(id) {
                                sym.declarations.push(decl_node);
                                sym.flags |= flags;
                            }
                        } else {
                            // Create new symbol
                            ...
                            self.scope_stack[0].set(name.to_string(), sym_id);
                        }
                    }
                }
            }
