...
// ─── TEMPORARY PROBES (zzprobe_) — remove before commit ───────────────

const ZZ_PRELUDE: &str = r#"
interface TypeLambda {
    readonly In: unknown;
    readonly Out: unknown;
}
...
fn zzcheck(body: &str) -> Vec<(u32, String)> {
    let source = format!("{ZZ_PRELUDE}\n{body}");
    relevant_diagnostics(&source)
}

#[test]
fn zzprobe_p12_annotated_callback_param() {
    // Explicitly annotated callback param — not context-sensitive, no round 2.
    let diags = zzcheck(
        r#"
function use<F extends TypeLambda, R, A, B>(
    self: Kind<F, R, [A, B]>,
    ...
    return map(self, (ab: [A, B]) => f(ab[0], ab[1]));
}
"#,
    );
    assert!(diags.is_empty(), "P12 annotated callback param: {diags:#?}");
}

#[test]
fn zzprobe_p13_callback_ignores_param() {
    let diags = zzcheck(
        r#"
function use<F extends TypeLambda, R, A, B>(
    self: Kind<F, R, [A, B]>
...
"#,
    ...
    assert!(diags.is_empty(), "P13 callback ignores param: {diags:#?}");
}

#[test]
fn zzprobe_p14_reveal_ab_type() {
    // Reveal what the callback param actually is via an assignment inside.
    let diags = zzcheck(
        r#"
function use<F extends TypeLambda, R, A, B>(
    self: Kind<F, R, [A, B]>
...
"#,
    ...
    assert!(diags.is_empty(), "P14 reveal ab: {diags:#?}");
}

#[test]
fn zzprobe_p15_second_use_nonfunction() {
    // Second parameter mentions A2 but is not a function.
    let diags = zzcheck(
        r#"
declare const map2: <F extends TypeLambda, R, A>(self: Kind<F, R, A>, other: A) => A;
function use<F extends TypeLambda, R, A, B>(
    self: Kind<F, R, [A, B]>,
    t: [A, B]
): [A, B] {
...
"#,
    ...
    assert!(diags.is_empty(), "P15 non-function second use: {diags:#?}");
}

#[test]
fn zzprobe_p16_bare_b_with_callback() {
    // Target is bare B (no tuple), callback present.
    let diags = zzcheck(
        r#"
function use<F extends TypeLambda, R, A, B>(
    self: Kind<F, R, B>,
...
"#,
    ...
    assert!(diags.is_empty(), "P16 bare B with callback: {diags:#?}");
}

#[test]
fn zzprobe_p17_tuple_a_only_with_callback() {
    // Tuple of a single param [A] — is it tuple-ness or the second slot?
    let diags = zzcheck(
        r#"
function use<F extends TypeLambda, R, A>(
    self: Kind<F, R, [A]>,
    f: (a: A) => string
): Kind<F, R, string> {
    return map(self, (ab) => f(ab[0]));
}
"#,
    );
    assert!(diags.is_empty(), "P17 single-element tuple: {diags:#?}");
}

#[test]
fn zzprobe_p18_tuple_of_same_param_twice() {
    let diags = zzcheck(
        r#"
function use<F extends TypeLambda, R, A>(
    self: Kind<F, R, [A, A]>,
    ...
    return map(self, (ab) => f(ab[1]));
}
"#,
    );
    assert!(diags.is_empty(), "P18 [A, A] tuple: {diags:#?}");
}

#[test]
fn zzprobe_p19_renamed_outer_binders() {
    // Same shape as P16 but outer params renamed — name-collision detector.
    let diags = zzcheck(
        r#"
...
"#,
    ...
    assert!(diags.is_empty(), "P19 renamed outer binders: {diags:#?}");
}

#[test]
fn zzprobe_p20_third_param_bare_with_four() {
    // Four outer params but callback touches the THIRD (A), not B.
    let diags = zzcheck(
        r#"
function use<F extends TypeLambda, R, A, B>(
    self: Kind<F, R, A>,
    ...
    return map(self, (a) => f(a));
}
"#,
    );
    assert!(diags.is_empty(), "P20 bare A with four params: {diags:#?}");
}

#[test]
fn zzprobe_p21_no_return_context() {
    // Same as P16 but the call is NOT in return position.
    let diags = zzcheck(
        r#"
function use<F extends TypeLambda, R, A, B>(
    self: Kind<F, R, B>,
...
"#,
    ...
    assert!(diags.is_empty(), "P21 no return context: {diags:#?}");
}
