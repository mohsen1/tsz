...
"#,
    ...
    // (`typeof import("p1/node_modules/csv-parse/lib/index")`), not the
    // original specifier — see PR replacing #2650.
    assert!(
...
"#,
    ...
    // tsc renders bare imports that resolve to a `.d.ts` typings file
    // inside `node_modules/` with the resolved-path form
    // (`typeof import("node_modules/mdast-util-to-string/index")`), not
    // the original specifier — see PR replacing #2650.
    assert!(
        diagnostics.iter().any(|(code, message)| {
