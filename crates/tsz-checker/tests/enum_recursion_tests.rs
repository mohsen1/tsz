...
fn enum_direct_self_reference_property_access() {
    let diags = check_source_codes("enum E { A = E.A }");
    assert!(
        diags.contains(&2565),
...
fn enum_self_reference_in_binary_expression() {
    let diags = check_source_codes("enum E { A = 1, B = E.B + 1 }");
    assert!(
        diags.contains(&2565),
...
fn enum_direct_self_reference_bare_identifier() {
    let diags = check_source_codes("enum E { A = A }");
    assert!(
        diags.contains(&2565),
...
fn enum_self_reference_in_bitwise_expression() {
    let diags = check_source_codes("enum E { A = 1, B = B | A }");
    assert!(
        diags.contains(&2565),
...
fn enum_mutual_recursion_two_enums() {
    let diags = check_source_codes("enum E { A = F.B }\nenum F { B = E.A }");
...
fn enum_mutual_recursion_with_auto_increment() {
    let diags = check_source_codes("enum E { X, A = F.B }\nenum F { Y, B = E.A }");
    let _ = diags;
}
...
fn enum_mutual_recursion_partial_cycle() {
    let diags =
        check_source_codes("enum E { X = 1, Y = F.B, Z = 3 }\nenum F { A = 10, B = E.Y, C = 20 }");
    let _ = diags;
}
...
fn enum_mutual_recursion_in_binary_expression() {
    let diags = check_source_codes("enum E { A = F.B + 1 }\nenum F { B = E.A + 1 }");
    let _ = diags;
}
...
fn const_enum_mutual_recursion_two_enums() {
    let diags = check_source_codes("const enum E { A = F.B }\nconst enum F { B = E.A }");
...
fn const_enum_direct_self_reference() {
    let diags = check_source_codes("const enum E { A = E.A }");
...
fn const_enum_mutual_recursion_in_binary_expression() {
    let diags = check_source_codes("const enum E { A = F.B + 1 }\nconst enum F { B = E.A + 1 }");
    assert!(
        diags.contains(&2474),
...
fn enum_forward_reference_bare_identifier() {
    let diags = check_source_codes("enum E { A = B, B = 1 }");
    assert!(
        diags.contains(&2651),
...
fn enum_forward_reference_property_access() {
    let diags = check_source_codes("enum E { A = E.B, B = 1 }");
    assert!(
        diags.contains(&2651),
...
fn enum_forward_reference_in_binary_expression() {
    let diags = check_source_codes("enum E { A = B + C, B = 1, C = 2 }");
    assert!(
        diags.contains(&2651),
...
fn const_enum_forward_reference() {
    let diags = check_source_codes("const enum E { A = B, B = 1 }");
    assert!(
        diags.contains(&2651),
...
fn enum_indirect_self_reference_two_members() {
    let diags = check_source_codes("enum E { A = B, B = A }");
...
fn enum_three_member_cycle_within_single_enum() {
    let diags = check_source_codes("enum E { A = E.B, B = E.C, C = E.A }");
...
fn const_enum_deep_chain_resolves_correctly() {
    ...
    let ts2474 = diags.iter().filter(|d| **d == 2474).count();
    assert_eq!(
        ts2474,
        0,
...
fn enum_non_const_chain_resolves_correctly() {
    let diags = check_source_codes("enum A { X = 10 }\nenum B { Y = A.X }\nenum C { Z = B.Y }");
...
fn enum_valid_back_reference() {
    let diags = check_source_codes("enum E { A = 1, B = A }");
    let ts2651 = diags.iter().filter(|d| **d == 2651).count();
    let ts2565 = diags.iter().filter(|d| **d == 2565).count();
    assert_eq!(
        ts2651,
        0,
...
fn enum_valid_back_reference_property_access() {
    let diags = check_source_codes("enum E { A = 1, B = E.A }");
    let ts2651 = diags.iter().filter(|d| **d == 2651).count();
    let ts2565 = diags.iter().filter(|d| **d == 2565).count();
    assert_eq!(
        ts2651, 0,
...
fn const_enum_valid_chain_expression() {
    let diags = check_source_codes("const enum E { A = 1, B = A + 1, C = B * 2 }");
    let ts2474 = diags.iter().filter(|d| **d == 2474).count();
    assert_eq!(
        ts2474,
        0,
...
fn const_enum_valid_cross_enum_unary() {
    let diags = check_source_codes("const enum A { X = 5 }\nconst enum B { Y = -A.X }");
    let ts2474 = diags.iter().filter(|d| **d == 2474).count();
    assert_eq!(
        ts2474, 0,
...
fn enum_cycle_through_auto_increment_member() {
    let diags = check_source_codes("enum E { A = F.C }\nenum F { B = E.A, C }");
    // The critical invariant is no panic / stack overflow.
    let _ = diags;
}
...
fn const_enum_cycle_through_auto_increment_member() {
    let diags = check_source_codes("const enum E { A = F.C }\nconst enum F { B = E.A, C }");
...
fn enum_mutual_auto_increment_cycle() {
    let diags = check_source_codes("enum E { A, B = F.D }\nenum F { C = E.B, D }");
    let _ = diags;
}
...
fn enum_three_way_cycle_through_auto_increment() {
    let diags =
        check_source_codes("enum E { A = F.C }\nenum F { B = G.C, C }\nenum G { B = E.A, C }");
    let _ = diags;
}
...
fn enum_bare_identifier_three_way_cycle() {
    let diags = check_source_codes("enum E { A = C, B = A, C = B }");
...
fn enum_bare_identifier_back_reference_resolves() {
    let diags = check_source_codes("enum E { A = 10, B = A + 5 }");
...
fn enum_mixed_bare_and_property_access() {
    let diags = check_source_codes("enum E { A = 1, B = A | E.A }");
    let ts2651 = diags.iter().filter(|d| **d == 2651).count();
    let ts2565 = diags.iter().filter(|d| **d == 2565).count();
...
fn enum_cross_reference_multiple_members_no_cycle() {
    let diags = check_source_codes("enum E { A = F.X, B = F.Y }\nenum F { X = E.B, Y = 1 }");
    let _ = diags;
}
...
fn enum_cross_reference_double_cycle() {
    let diags = check_source_codes("enum E { A = F.B, C = F.D }\nenum F { B = E.C, D = E.A }");
    let _ = diags;
}
...
fn const_enum_cross_reference_double_cycle() {
    let diags =
        check_source_codes("const enum E { A = F.B, C = F.D }\nconst enum F { B = E.C, D = E.A }");
    assert!(
        diags.contains(&2474),
...
fn const_enum_self_reference_in_binary_expression() {
    let diags = check_source_codes("const enum E { A = E.A + 1 }");
    assert!(
        diags.contains(&2565),
...
fn const_enum_self_reference_bare_identifier() {
    let diags = check_source_codes("const enum E { A = A }");
    assert!(
        diags.contains(&2565),
...
fn const_enum_forward_reference_property_access() {
    let diags = check_source_codes("const enum E { A = E.B, B = 1 }");
    assert!(
        diags.contains(&2651),
...
fn enum_mixed_valid_and_cycling_members() {
    let diags = check_source_codes("enum E { X = 1, Y = F.Z, W = 3 }\nenum F { Z = E.Y }");
...
fn const_enum_mixed_valid_and_cycling_members() {
    let diags =
        check_source_codes("const enum E { X = 1, Y = F.Z, W = 3 }\nconst enum F { Z = E.Y }");
...
fn enum_auto_increment_indirect_cycle_two_enums() {
    let diags = check_source_codes("enum A { X = B.Z, Y }\nenum B { W = A.Y, Z }");
    let _ = diags;
}
...
fn const_enum_auto_increment_indirect_cycle() {
    let diags = check_source_codes("const enum A { X = B.Z, Y }\nconst enum B { W = A.Y, Z }");
...
fn mixed_nonconst_references_const_references_back() {
    let diags = check_source_codes("enum A { X = B.Y }\nconst enum B { Y = A.X }");
    let _ = diags;
}
...
fn enum_shared_reference_resolves_with_memoization() {
    let diags = check_source_codes("enum A { X = B.V, Y = B.V + 1 }\nenum B { V = 42 }");
    // No errors expected — both should resolve
    let error_codes: Vec<u32> = diags.to_vec();
    assert!(
...
fn enum_repeated_element_access_same_member() {
    let diags = check_source_codes("enum E { A = 1, B = E[\"A\"] + E[\"A\"] }");
    let ts2651 = diags.iter().filter(|d| **d == 2651).count();
    let ts2565 = diags.iter().filter(|d| **d == 2565).count();
    assert_eq!(
        ts2651, 0,
...
fn enum_repeated_bare_identifier_references() {
    let diags = check_source_codes("enum E { A = 1, B = A + A + A }");
    let ts2651 = diags.iter().filter(|d| **d == 2651).count();
    let ts2565 = diags.iter().filter(|d| **d == 2565).count();
...
fn merged_const_enum_cross_declaration_cycle() {
    ...
    assert!(
        diags.iter().any(|d| *d == 2474 || *d == 2651),
...
fn enum_conditional_self_reference_true_branch() {
    let diags = check_source_codes("enum E { A = true ? E.A : 0 }");
    assert!(
        diags.contains(&2565),
...
fn enum_conditional_self_reference_false_branch() {
    let diags = check_source_codes("enum E { A = true ? 0 : E.A }");
    assert!(
        diags.contains(&2565),
...
fn enum_conditional_self_reference_condition() {
    let diags = check_source_codes("enum E { A = E.A ? 1 : 0 }");
    assert!(
        diags.contains(&2565),
...
fn enum_reference_non_existent_enum() {
    let diags = check_source_codes("enum E { A = NonExistent.B }");
...
fn const_enum_reference_non_existent_enum() {
    let diags = check_source_codes("const enum E { A = NonExistent.B }");
    assert!(
        diags.contains(&2474),
...
fn enum_reference_non_existent_member() {
    let diags = check_source_codes("enum E { A = 1 }\nenum F { B = E.NonExistent }");
    // Should not panic; value is treated as non-constant
    let _ = diags;
}
...
fn enum_exponentiation_self_reference() {
    let diags = check_source_codes("enum E { A = 2, B = E.B ** 2 }");
    assert!(
        diags.contains(&2565),
...
fn enum_valid_exponentiation_cross_reference() {
    let diags = check_source_codes("enum E { A = 2, B = E.A ** 3 }");
    // No TS2565 or TS2651 expected
    assert!(
        !diags.iter().any(|d| *d == 2565 || *d == 2651),
