...
/// `any`/`unknown`/`never` operand, so the *checked* type is already the
/// reduced union.
///
...
/// repainted as `PropertyKey`. `annotation_is_keyof_over_degenerate_operand`
/// (`error_reporter/core/diagnostic_source/type_query_alias.rs`) stops the
/// written-text fallback from reproducing `keyof any` verbatim;
/// `diagnostic_source_annotation_is_keyof_degenerate_operand` (`error_reporter/
/// render_failure/{type_mismatch,nested_application_property_mismatch}.rs`)
/// then opts a confirmed degenerate-operand `keyof` annotation into the
/// always-expanded formatter used by the analogous TS2344 constraint message,
/// without touching the general `PropertyKey` alias display other surfaces
/// (`Object.groupBy<K extends PropertyKey, T>`) still rely on.
#[test]
