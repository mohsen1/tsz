...
//! "class with no AST type-parameters" case. Previously this fell
//! through to a `with_parent_cache_attributed` child checker just to
//! run a JSDoc @template scan and return `Some(Vec::new())`.
//!
...
//! > leading JSDoc `@template` tag, or (b) empty — and both can be
