...
// apparent element type (matching `tsc`'s `getApparentType`). Previously the
// solver's `ElementAccessEvaluator` rejected `T` as `NotIndexable` and the
// access type collapsed to `ERROR`, silently swallowing TS2322.
#[test]
fn ts2322_indexed_access_on_array_constrained_type_param_emits_mismatch() {
    let source = r#"
...
"#;
    assert_eq!(
        ...
        1,
        "T[0] for T extends number[] must produce a TS2322 number→string mismatch"
    );
}

#[test]
fn ts2322_indexed_access_on_unknown_array_constrained_type_param_emits_mismatch() {
    let source = r#"
...
"#;
    assert_eq!(
        count_errors_with_code(source, 2322),
        1,
        "T[0] for T extends unknown[] must produce a TS2322 unknown→number mismatch"
    );
}

#[test]
fn ts2322_indexed_access_on_renamed_array_constrained_type_param_emits_mismatch() {
    // Rename the type parameter to verify the rule is structural, not a
    // hardcoded match on `T`.
    let source = r#"
...
"#;
    assert_eq!(
        count_errors_with_code(source, 2322),
        1,
        "P[0] for P extends string[] must produce a TS2322 string→number mismatch"
    );
}

#[test]
fn ts2322_indexed_access_on_tuple_constrained_type_param_emits_mismatch() {
    let source = r#"
...
#[test]
fn ts2322_indexed_access_on_chained_array_constrained_type_param_emits_mismatch() {
    // Indirect chain: `U extends T extends number[]`. Apparent-type walking
    // must follow chains; substituting at only one level would mask the bug.
    let source = r#"
...
"#;
    assert_eq!(
        count_errors_with_code(source, 2322),
        1,
        "U[0] for U extends T extends number[] must produce a TS2322 number→string mismatch"
    );
}
