...
// Was `test_js_prototype_read_before_assignment_reports_ts2565`, asserting
// TS2565 for this exact source. `tsc` 7.0.2 (`--strict --allowJs --checkJs`)
// reports TS2339 ("Property '...' does not exist on type 'NewAjax'."), never
// TS2565: an ES `class`'s prototype is the closed instance type, so a member
// it never declares simply does not exist — it is not a legitimate expando
// root the way a function-as-constructor's prototype is. See #16049.
#[test]
fn test_js_class_prototype_read_before_assignment_reports_ts2339() {
    let source = r#"
class NewAjax {}
...
"#;
    ...
    assert!(
        ...
        "Expected an ES class's closed prototype to report TS2339 for a member it never declares, got: {diagnostics:?}"
    );
    assert!(
        diagnostics.iter().all(|(code, _)| *code != 2565),
        "An ES class's prototype is closed, not an expando root — it must never report TS2565, got: {diagnostics:?}"
    );
}
