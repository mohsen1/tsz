...
/// Regression: the TS2322 diagnostic source display must preserve a literal
/// element union, matching tsc. `widen_type_for_display` historically
/// collapsed small literal-only unions to their primitive base, producing
/// `Type 'number[]'` where tsc shows `Type '(1 | 2)[]'`.
#[test]
fn ts2322_source_display_preserves_literal_array_element_union() {
    let source = r#"
const o = { a: 1 as const, b: 2 as const };
const v = Object.values(o);
const bad: boolean = v;
"#;
    let diags = relevant_default_lib_diagnostics(source);
    let ts2322_messages = diagnostics_with_code(&diags, 2322);
    assert!(
        ts2322_messages
            .iter()
            .any(|m| m.contains("(1 | 2)[]") || m.contains("(2 | 1)[]")),
        "TS2322 source display must keep `(1 | 2)[]`, not widen to `number[]`. Messages: {ts2322_messages:#?}"
    );
    assert!(
        ts2322_messages.iter().all(|m| !m.contains("'number[]'")),
        "TS2322 source display must NOT show the widened `number[]`. Messages: {ts2322_messages:#?}"
    );
}

/// Companion to the assignability display test: an explicit annotation
/// `(1 | 2)[]` on a let binding must also survive `widen_type_for_display`
/// without collapsing to `number[]`.
#[test]
fn ts2322_source_display_preserves_annotated_literal_array_element_union() {
    let source = r#"
const a: (1 | 2)[] = [1];
const bad: boolean = a;
"#;
    ...
    assert!(
        ts2322_messages
            .iter()
            .any(|m| m.contains("(1 | 2)[]") || m.contains("(2 | 1)[]")),
        "TS2322 source display must keep `(1 | 2)[]` from explicit annotation. Messages: {ts2322_messages:#?}"
    );
}
