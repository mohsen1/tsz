...
#[test]
fn ts2708_pure_namespace_member_value_access_does_not_fall_back_to_global_value() {
    let diags = check(
        r#"
...
"#,
    ...
    assert!(
        diags.iter().any(|d| d.code == 2708),
        "Expected TS2708 for uninstantiated namespace value access, got: {diags:?}"
    );
    assert!(
        !diags.iter().any(|d| d.code == 2339),
        "Should not recover a global value type and emit TS2339, got: {diags:?}"
    );
}

#[test]
fn namespace_import_alias_merge_reports_namespace_value_and_missing_type_member() {
    let files = [
        (
            "file1.ts",
            r#"
...
namespace Lib { export const foo: string = ""; }
Lib.foo;
var x: Lib.Bar;
export { Lib }
"#,
    ...
    let file1_diags = check_named_files(&files, "file1.ts");
    assert!(
        file1_diags.iter().any(|d| d.code == 2708),
        "Expected TS2708 for file-local pure namespace used as value, got: {file1_diags:?}"
    );
    assert!(
        !file1_diags.iter().any(|d| d.code == 2339),
        "Should not emit TS2339 on file-local namespace member access, got: {file1_diags:?}"
    );

    let file2_diags = check_named_files(&files, "file2.ts");
    assert!(
        file2_diags.iter().any(|d| d.code == 2440),
        "Expected TS2440 for import alias/local namespace conflict, got: {file2_diags:?}"
    );
    assert!(
        file2_diags.iter().any(|d| d.code == 2694),
        "Expected TS2694 for missing exported type member through namespace import, got: {file2_diags:?}"
    );
}
