...
/// Regression test for issue #5808: `const X = {...} as const; type X = typeof X[keyof typeof X]`
/// produced a false-positive TS2456 (circular reference) and a TS2322 (wrong type for assignees).
#[test]
fn test_merged_const_type_alias_indexed_access_no_false_positive() {
    let codes = get_error_codes(
        r#"
const Direction = {
    Up: 0,
...
"#,
    ...
    let real_errors: Vec<_> = codes.iter().filter(|&&c| c != 2318).copied().collect();
    assert!(
        !real_errors.contains(&2456),
        "Should not emit TS2456 (false circular reference) for merged const+type alias, got: {real_errors:?}"
    );
    assert!(
        !real_errors.contains(&2322),
        "Should not emit TS2322 when assigning a member value to the merged type alias, got: {real_errors:?}"
    );
}

/// Ensure the fix works regardless of the iteration variable name chosen by the user.
#[test]
fn test_merged_const_type_alias_indexed_access_different_names() {
    let codes = get_error_codes(
        r#"
...
"#,
    ...
    assert!(
        !real_errors.contains(&2456),
        "Should not emit TS2456 for merged const+type alias (string values), got: {real_errors:?}"
    );
    assert!(
        !real_errors.contains(&2322),
        "Should not emit TS2322 for merged const+type alias (string values), got: {real_errors:?}"
    );
}

/// Non-merged `type V = Obj[keyof Obj]` pattern should also evaluate the alias.
#[test]
fn test_non_generic_indexed_access_type_alias_evaluates() {
    let codes = get_error_codes(
        r#"
interface Config { host: string; port: number; }
type ConfigValue = Config[keyof Config];

const h: ConfigValue = "localhost";
const p: ConfigValue = 8080;
"#,
    );
    let real_errors: Vec<_> = codes.iter().filter(|&&c| c != 2318).copied().collect();
    assert!(
        !real_errors.contains(&2322),
        "Should not emit TS2322 for non-generic indexed access type alias, got: {real_errors:?}"
    );
}
