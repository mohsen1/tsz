...
fn test_ts2306_emitted_for_non_module_import() {
    ...
    let mut parser_b = ParserState::new("tsx.tsx".to_string(), "".to_string());
    ...
    resolved_module_paths.insert((0, "./tsx".to_string()), 1);
    checker
        .ctx
        .set_resolved_module_paths(Arc::new(resolved_module_paths));

    let mut resolved_modules: FxHashSet<String> = FxHashSet::default();
    resolved_modules.insert("./tsx".to_string());
    checker.ctx.set_resolved_modules(resolved_modules);
    checker.ctx.report_unresolved_imports = true;

    checker.check_source_file(root_a);

    let ts2306_errors: Vec<_> = checker
        .ctx
        .diagnostics
        .iter()
        .filter(|d| d.code == 2306)
        .collect();
    assert!(
        !ts2306_errors.is_empty(),
        "Expected TS2306 error for non-module import, got: {:?}",
