...
#[test]
fn union_type_reduction2_preserves_tsc_callable_arity() {
    let diagnostics = check_source_with_strict_null(
        r#"
...
interface ReturnVal { something(): void; }
...
"#,
    ...
    let ts2554_messages: Vec<_> = diagnostics
        .iter()
    ...
    assert_eq!(
        ...
        "Expected only f6 to report TS2554, got diagnostics: {diagnostics:#?}"
    );
    assert!(
        diagnostics.iter().all(|d| d.code != 2345),
        "Expected no TS2345 argument mismatch diagnostics, got: {diagnostics:#?}"
    );
}
