...
impl<'a> CheckerState<'a> {
    ...
    fn collect_numeric_literal_union_display_replacements(
        ...
        match self.ctx.types.lookup(type_id) {
            Some(tsz_solver::TypeData::Application(app_id)) => {
                ...
                let other_args = other_type.and_then(|other| {
                    crate::query_boundaries::common::application_info(self.ctx.types, other)
                        .and_then(|(base, args)| (base == app.base).then_some(args))
                });
            ...
            Some(
                tsz_solver::TypeData::Union(list_id) | tsz_solver::TypeData::Intersection(list_id),
            ) => {
            ...
            Some(tsz_solver::TypeData::Array(element)) => {
                self.collect_numeric_literal_union_display_replacements(
                    element,
            ...
            Some(tsz_solver::TypeData::Tuple(elements)) => {
            ...
            Some(
                ...
                let shape = self.ctx.types.object_shape(shape_id);
                let other_shape = other_type.and_then(|other| match self.ctx.types.lookup(other) {
                    Some(
                        tsz_solver::TypeData::Object(shape_id)
                        | tsz_solver::TypeData::ObjectWithIndex(shape_id),
                    ) => Some(self.ctx.types.object_shape(shape_id)),
                    _ => None,
                });
    ...
    fn numeric_literal_value(&self, type_id: TypeId) -> Option<u64> {
        match self.ctx.types.lookup(type_id) {
            Some(tsz_solver::TypeData::Literal(tsz_solver::LiteralValue::Number(value))) => {
                Some(value.0.to_bits())
            }
            _ => None,
        }
    }
                ...
                members.len() > 1
                    && members.iter().all(|&member| {
                        matches!(
                            ...
                            Some(tsz_solver::TypeData::Literal(
