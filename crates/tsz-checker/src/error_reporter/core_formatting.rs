...
impl<'a> CheckerState<'a> {
    ...
    fn nullish_stripped_members(&mut self, ty: TypeId, other: TypeId) -> Option<Vec<TypeId>> {
        ...
        // The same deferral applies to any other *deferred* generic operand
        // that relates to a union through its base constraint — `T[K]`,
        // `keyof T`, a distributive conditional, a generic-alias application
        // over one of those, or an intersection carrying one — so the full
        // declared union is kept (see the classifier's doc).
