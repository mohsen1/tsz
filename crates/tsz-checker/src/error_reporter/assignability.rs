...
impl<'a> CheckerState<'a> {
    ...
    fn missing_property_is_satisfied_by_source(
        ...
        let read_ok = if source_prop.is_method || target_prop.is_method {
            self.relation_boolean_guard_bivariant(source_prop.type_id, target_prop.type_id)
        } else {
            self.relation_boolean_guard(source_prop.type_id, target_prop.type_id)
        };
        let write_ok = target_prop.readonly
            || self.relation_boolean_guard(target_prop.write_type, source_prop.write_type);
    ...
    fn exact_optional_source_for_message(
        ...
        let mismatched: Vec<TypeId> = members
            .iter()
            .copied()
            .filter(|&m| !self.relation_boolean_guard(m, target_eval))
            .collect();
