...
impl<'a> CheckerState<'a> {
    ...
    fn keyof_source_assignment_display(
        &mut self,
        source: TypeId,
        target: TypeId,
    ) -> Option<String> {
        ...
        let operand = crate::query_boundaries::common::keyof_inner_type(self.ctx.types, resolved)?;
    ...
    fn keyof_operand_writable_name(&self, operand: TypeId) -> Option<String> {
        ...
        if let Some(shape_id) =
            crate::query_boundaries::common::object_shape_id(self.ctx.types, operand)
        {
    ...
    fn resolve_non_generic_alias_body_for_display(&self, ty: TypeId) -> TypeId {
        if crate::query_boundaries::common::keyof_inner_type(self.ctx.types, ty).is_some() {
            return ty;
        }
        let Some(def_id) = crate::query_boundaries::common::lazy_def_id(self.ctx.types, ty)
