//! Assignment-*target* type formatting for TS2322-family diagnostics, plus
//! the shared tuple/alias display helpers.
//!
//! Extracted from `error_reporter/core/diagnostic_source.rs` to keep that
//! file under the LOC ceiling; the source-side pipeline later moved to
//! `assignment_source_formatting.rs` along the source-vs-target seam for the
//! same reason. Pure file-organization moves; no logic changes.
...
impl<'a> CheckerState<'a> {
    ...
    pub(in crate::error_reporter) fn new_expression_nominal_source_display(
        &mut self,
        expr_idx: NodeIndex,
        display_type: TypeId,
    ) -> Option<String> {
    ...
    pub(in crate::error_reporter) fn js_constructor_instance_assignment_source_display(
        &mut self,
        source: TypeId,
        anchor_idx: NodeIndex,
    ) -> Option<String> {
    ...
    pub(in crate::error_reporter) fn call_unknown_array_source_display(
        &mut self,
        expr_idx: NodeIndex,
        target: TypeId,
    ) -> Option<String> {
    ...
    pub(in crate::error_reporter) fn preferred_evaluated_source_display(
        &mut self,
        source: TypeId,
        target: TypeId,
    ) -> Option<String> {
    ...
    pub(in crate::error_reporter) fn type_assertion_mapped_alias_source_display(
        &mut self,
        expr_idx: NodeIndex,
    ) -> Option<String> {
        let node = self.ctx.arena.get(expr_idx)?;
