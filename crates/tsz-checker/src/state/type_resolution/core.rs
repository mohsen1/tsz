...
impl CheckerState<'_> {
    ...
    pub(super) fn get_type_from_type_reference_inner(&mut self, idx: NodeIndex) -> TypeId {
                ...
                })
        {
            ...
            if !target_owns_declared_type {
                ...
                if let Some(instance_type) =
                    self.instance_type_from_named_import_type_reference(alias_type, alias_target)
                {
            ...
            && name_node.kind == syntax_kind_ext::QUALIFIED_NAME
        {
            ...
            if let TypeSymbolResolution::Type(sym_id) = qn_sym_res {
                ...
                // to the referenced top-level member type (`A`) using the module
                // specifier from `x`. A nested reference such as `x.Root.A` must use
                // the already-resolved `Root` export surface: matching the terminal
                // name against top-level augmentations would merge an unrelated
                ...
                if result != TypeId::ERROR {
                    ...
                    let imported_module = self
                        .ctx
                        .arena
                        .get_identifier_at(type_name_idx)
                        .and_then(|identifier| {
                            self.resolve_named_import_for_local_name(
                                type_name_idx,
                                &identifier.escaped_text,
                            )
                        })
                        .map(|(module_specifier, import_name, _)| (module_specifier, import_name))
                        .or_else(|| {
                            self.ctx
                                .binder
            ...
            && let Some(ident) = self.ctx.arena.get_identifier(name_node)
        {
            ...
            if has_type_args {
                ...
                if let Some(sym_id) = sym_id {
                    let lib_binders = self.get_lib_binders();
                    let imported_module = self
                        .resolve_named_import_for_local_name(type_name_idx, name)
                        .map(|(module_specifier, import_name, _)| (module_specifier, import_name))
                        .or_else(|| {
                            self.ctx
                                .binder
                    ...
                    if let Some((module_specifier, aug_name)) = imported_module {
                                ...
                                query::get_lazy_def_id(self.ctx.types, app_base)
                        {
                            ...
                            if let Some(body) = body {
                                let augmented = self.apply_module_type_augmentations(
                                    &module_specifier,
                                    &aug_name,
                                    body,
                                );
