//! Homomorphic self-referential mapped-type detection helpers.
//!
//! Extracted from `reference_helpers.rs` to keep that module under the
//! per-file line cap. These helpers detect the structural pattern
//! `type A<T> = { [P in keyof T]: A<T> }` (a directly homomorphic mapped
//! type whose template is an identity self-reference) and the application
//! shape that drives `TS2589` instead of `TS2322` when such an alias is
//! applied to a tuple argument against a tuple target.
