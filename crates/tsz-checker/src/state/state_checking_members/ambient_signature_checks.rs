...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn check_method_declaration_with_request(
        ...
        if method.body.is_some() {
            ...
            if (is_ambient_class || is_ambient_file) && !is_async && !skip_implicit_any {
                let method_name = self.get_property_name_for_diagnostic(method.name);
    ...
    pub(crate) fn check_accessor_declaration_with_request(
        ...
        if accessor.body.is_some() {
            ...
            // `member_name_display_or_key` prefers a string-literal name's
            // verbatim source spelling, falls back to `get_property_name`'s
            // bare key otherwise, and further falls back to the structured
            // computed-name display when the key resolver declines a
            // computed name whose expression is a plain identifier reference
            // (`get [k]()` for a `unique symbol` const, `get [Symbol.iterator]()`).
            // It deliberately stops there: `property_name_for_error`'s further
            // fallback to a raw source-text slice would also fire on a
            // genuinely malformed computed name (`get [](); `, a parse
            // error), where tsc reports only the syntax error and stays
            // silent on `TS7033`.
            if self.ctx.no_implicit_any()
                && !self.is_js_file()
                && self.paired_setter_member_for_getter(accessor).is_none()
                && let Some(accessor_name) = self.member_name_display_or_key(accessor.name)
            {
                use crate::diagnostics::diagnostic_codes;
                self.error_at_node_msg(
                    accessor.name,
