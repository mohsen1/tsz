...
// Structural rule: when a cross-file source-file interface is an unmerged option-bag
// shape (only property signatures with scope-independent or direct-lowerable-sibling
// annotations), tsz can lower it directly via `delegate_cross_arena_symbol_resolution`
// without constructing a child checker, even when needs_cross_file_delegation = true
// (where symbol_type_cache_from_symbol_arena is false by definition).
//
// Adjacent cases covered: scope-independent primitives, sibling alias reference, and
// a program-with-module-augmentation case where the old symbol-arena cache guard
// blocked the fast path.

/// Builds a two-file checker state (requester + target) with `symbol_name` from
/// `target_binder` registered as a cross-file-index target. Returns the state and
...
fn delegate_cross_arena_source_option_bag_lowers_directly_via_cross_file_index() {
    // Case 1: scope-independent property types only.
    let (target_arena, target_binder, types) = parse_bound_source_with_name(
        "config.ts",
        r#"
...
{
    // Case 2: property annotation is a sibling non-generic alias (not scope-independent).
    // Different interface name proves the rule is structural, not name-pegged.
    let (target_arena, target_binder, types) = parse_bound_source_with_name(
        "task.ts",
        r#"
...
fn delegate_cross_arena_source_option_bag_resolves_in_program_with_module_augmentations() {
    // Case 3: symbol-arena path with program_has_module_augmentations() = true.
    // Before the fix, the module-augmentation guard blocked symbol_type_cache_from_symbol_arena,
    // so allow_source_file_arena was false and direct lowering was skipped.
    let (target_arena, target_binder, types) = parse_bound_source_with_name(
        "options.ts",
        r#"
        ...
        .declarations[0];
    {
        ...
        Arc::make_mut(&mut rb.declaration_arenas)
            ...
            .or_default()
            .push(Arc::clone(&target_arena));
        // Makes program_has_module_augmentations() return true.
