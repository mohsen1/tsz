...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn delegate_cross_arena_symbol_resolution(
        ...
        if should_delegate {
            ...
            let symbol_type_cache_scope = symbol_type_cache_file_idx.and_then(|_| {
            ...
            if let Some(cache_file_idx) = symbol_type_cache_file_idx
                && let Some((cached_type, cached_params)) =
                    self.ctx.cached_cross_file_symbol_type_in_scope(
                        sym_id,
                        cache_file_idx as u32,
                        symbol_type_cache_scope,
                    )
            {
                if let Some(p) = perf {
            ...
            if let Some(result) = self.try_resolve_cross_arena_named_alias_without_child(sym_id) {
                ...
                {
                    self.ctx.cache_cross_file_symbol_type_in_scope(
                        sym_id,
                        file_idx as u32,
                        symbol_type_cache_scope,
                        result,
