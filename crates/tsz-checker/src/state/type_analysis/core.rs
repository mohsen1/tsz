...
impl<'a> CheckerState<'a> {
    ...
    /// Resolve `typeof X` in type positions.
    ///
    /// For simple identifier expressions (e.g. `typeof c`), applies flow
    /// narrowing so that `typeof c` inside `if (typeof c === 'string') { ... }`
    /// resolves to `string` rather than the declared `string | number`.
    /// Falls back to [`TypeNodeChecker::get_type_from_type_query`] for
    /// qualified names, type arguments, and other complex cases.
    pub(crate) fn get_type_from_type_query(&mut self, idx: NodeIndex) -> TypeId {
        use crate::types_domain::type_node::TypeNodeChecker;

        // For simple identifier typeof expressions, try flow-sensitive resolution
        // first. This ensures `typeof x` inside a narrowing guard picks up the
        // narrowed type rather than the declared type.
        //
        // Skip flow narrowing when the typeof is inside a call/construct/method
        // signature parameter (e.g. `{ (arg: typeof x): boolean }`) — tsc uses
        // the declared type in those positions.
        ...
        {
            // Get the type of the identifier with flow narrowing enabled.
