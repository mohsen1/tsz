...
impl<'a> CheckerState<'a> {
    ...
    /// Unlike [`Self::source_file_type_node_is_scope_independent`], also accepts
    /// type references to file-local non-generic type aliases whose bodies satisfy
    /// the predicate, enabling direct lowering of chains like
    /// `type Foo = Bar; type Bar = string | number` without a child checker.
    ///
    /// `seen` carries [`SymbolId`]s already on the current chain so mutual and
    /// self-referential recursion are detected and rejected.
    fn source_file_type_node_is_local_alias_chain_lowerable(
        ...
        match node.kind {
            k if k == syntax_kind_ext::TYPE_REFERENCE => {
                ...
                // Must be a plain identifier, not a qualified name like `A.B`.
                let Some(name) = arena
                    .get(type_ref.type_name)
                    .and_then(|name_node| arena.get_identifier(name_node))
                    .map(|ident| ident.escaped_text.as_str())
