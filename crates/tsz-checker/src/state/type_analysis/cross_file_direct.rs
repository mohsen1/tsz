...
pub(crate) fn is_direct_lowering_source_file_arena(arena: &NodeArena) -> bool {
    matches!(arena.source_files.first(), Some(sf) if !sf.is_declaration_file)
}
...
impl<'a> CheckerState<'a> {
    ...
    pub(super) fn source_file_local_name_def_id_for_lowering(
        ...
        if symbol.flags & symbol_flags::TYPE_ALIAS != 0 && symbol.declarations.len() != 1 {
            return None;
        }
        if symbol
            .declarations
            ...
            .any(|&decl_idx| Self::lib_declaration_name_matches(symbol_arena, decl_idx, type_name))
        {
            let def_id = self.ctx.get_or_create_def_id(sym_id);
            if symbol.flags & symbol_flags::TYPE_ALIAS != 0 {
                self.register_direct_source_file_type_alias_body_for_lowering(
                    def_id,
                    delegate_binder,
                    symbol_arena,
                    type_name,
...
#[path = "cross_file_direct_source_alias.rs"]
mod source_alias;

#[cfg(test)]
#[path = "cross_file_direct_test_modules.rs"]
mod test_modules;
