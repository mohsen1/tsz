...
fn jsx_intrinsic_elements_merges_global_augmentation_with_existing_namespace() {
    ...
    assert_eq!(
        ts2339_messages.len(),
        1,
        "Expected only the truly missing intrinsic tag to fail, got: {diagnostics:?}"
    );
    assert!(
        ts2339_messages[0].contains("missing-element"),
        "Expected TS2339 for missing-element only, got: {diagnostics:?}"
    );
}

#[test]
fn jsx_generic_component_type_parameter_checks_empty_attrs_against_lma() {
    let diagnostics = check_jsx(
        r#"
        declare namespace JSX {
            interface Element {}
    ...
    assert!(
        ...
        "Expected empty attrs to be checked against JSX.LibraryManagedAttributes<T, {{}}>, got: {diagnostics:?}"
    );
}
