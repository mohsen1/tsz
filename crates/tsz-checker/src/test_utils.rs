...
/// Parse, bind, and type-check a multi-file project where lib symbols are
/// provided **only** through `lib_contexts` and are NOT merged into each
/// file's `file_locals`.
///
/// This mirrors the CLI/program pipeline (`tsz_cli::driver::check`), which
/// binds every source file with [`tsz_binder::BinderState::bind_source_file`]
/// (no per-file lib merge) and resolves globals through shared lib contexts at
/// check time. It differs from [`check_multi_file_with_libs`], which merges the
/// full lib symbol table into every file's `file_locals` via
/// `bind_source_file_with_libs`.
///
/// The distinction matters for cross-arena heritage resolution: a global lib
/// interface (e.g. `ArrayIterator`) is reachable through the lib contexts but
/// absent from a non-entry file's `file_locals`. Tests that need to exercise
/// that exact condition — e.g. a class declared in one file whose inferred
/// member type instantiates a lib generic consumed from another file — must use
/// this helper, because the merged-`file_locals` variant masks the bug.
pub fn check_multi_file_libs_via_contexts(
    files: &[(&str, &str)],
    entry_file: &str,
    options: CheckerOptions,
    lib_files: &[Arc<LibFile>],
    ...
    for (name, source) in files {
        ...
        let root = parser.parse_source_file();
        let mut binder = BinderState::new();
        // CLI-faithful: bind without merging lib symbols into `file_locals`.
        binder.bind_source_file(parser.get_arena(), root);
        arenas.push(Arc::new(parser.get_arena().clone()));
        binders.push(Arc::new(binder));
        roots.push(root);
