...
impl<'a> CheckerState<'a> {
    ...
    /// True iff `class_idx` (walking up its base-class chain) has a member
    /// named `target_name` declared specifically as a `get` / `set` accessor.
    ///
    /// `class_chain_member_kind_name_only` collapses accessors and methods to
    /// `ClassMemberKind::MethodLike`, so it cannot distinguish the two. The
    /// `super.<accessor>` diagnostic (TS2340) needs that distinction —
    /// methods accessed via `super` are allowed, accessors are not.
    pub(crate) fn class_chain_member_is_accessor(
        &mut self,
        class_idx: NodeIndex,
        target_name: &str,
        target_is_static: bool,
    ...
    fn class_own_member_is_accessor(
        &mut self,
        class_idx: NodeIndex,
        target_name: &str,
        target_is_static: bool,
        ...
        for &member_idx in &class.members.nodes {
            ...
            if self
                .member_display_name_for_kind_lookup(accessor.name, target_name)
                .is_some()
            {
                return true;
            }
