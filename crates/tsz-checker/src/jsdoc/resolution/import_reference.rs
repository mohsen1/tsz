...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn resolve_jsdoc_import_type_member_result(
        ...
        // `imported_namespace_display_module_name` — because tsc's TS2694
        // text always names the *resolved file path* (extension stripped)
        // for a real module, even a relative specifier;
        // `imported_namespace_display_module_name` instead preserves the
        // literal specifier for relative imports, which is the correct (but
        // different) rule for `typeof import("...")` type printing.
        let namespace_name =
