...
impl<'a> CheckerState<'a> {
    ...
    /// (or via a JSDoc `@import` and a runtime ES `import` declaration), tsc
    /// emits TS2300 "Duplicate identifier 'X'" at each occurrence.
    ///
    /// Pure runtime/runtime collisions are handled by the symbol-based
    /// duplicate-identifier pass; this helper only fires when at least one
    /// occurrence is a JSDoc `@import` (otherwise it would double-report).
    ...
    fn collect_runtime_import_positions(
            ...
            && let Some(named) = self.ctx.arena.get_named_imports(bindings_node)
        {
            ...
            // Named imports: `import { a, b as c } from "mod"`. The local
            // binding is `spec.name` (the alias when one is present).
