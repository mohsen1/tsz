...
/// (`Holder<Wrap<Elem>>`). Regression for the `infiniteExpansionThrough-
/// Instantiation` recursive-types family (#10867). Binder names are deliberately
/// non-canonical so the assertion checks structure, not spellings.
#[test]
...
/// Linked<Linked<U>>` must still display `Owner<T>`, not `Owner<Linked<T>>`.
/// This mirrors the heritage shape of the canonical conformance witness.
#[test]
