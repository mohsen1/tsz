...
impl<'a, 'ctx> DeclarationChecker<'a, 'ctx> {
    ...
    pub fn check_variable_declaration(&mut self, decl_idx: NodeIndex) {
        ...
        // TS1155 (`'{0}' declarations must be initialized.`) is owned solely by
        // the parser (#17251) and must NOT be re-emitted from any checker path —
        // it is an `is_parser_grammar_code` member, whose contract is that the
        // parser emits the code *instead of* the checker (a checker copy is a
        // double-emission). `CheckerState`'s equivalent block was the live one
        // that doubled `const x;`; this `DeclarationChecker` copy is unreached for
        // variable statements today, removed alongside it so re-wiring this path
        // cannot silently reintroduce the double (#17253 / #16279 audit round 12).
