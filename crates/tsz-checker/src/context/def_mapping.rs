...
impl<'a> CheckerContext<'a> {
    ...
    fn populate_def_ids_from_semantic_defs(
        ...
        // Pass 3: Resolve heritage names to DefIds (extends/implements).
        //
        // This brings parity with the merge path's `resolve_heritage_in_store`
        // (parallel/core.rs Pass 3). Without this, single-file checker mode
        // leaves `DefinitionInfo.extends` and `DefinitionInfo.implements` empty
        // until the checker walk fills them in, meaning early solver queries
        // can't see the class/interface hierarchy.
        self.resolve_heritage_from_semantic_defs(semantic_defs);

        count
    }

    /// Resolve heritage names to DefIds in the DefinitionStore.
    ///
    /// Mirrors the logic in `tsz_core::parallel::core::resolve_heritage_in_store`
    /// but operates on the checker's local `DefinitionStore` and `TypeInterner`.
    /// Called as Pass 3 of `populate_def_ids_from_semantic_defs` to bring
    /// single-file checker mode to parity with the merge path.
    fn resolve_heritage_from_semantic_defs(
        &self,
        semantic_defs: &rustc_hash::FxHashMap<tsz_binder::SymbolId, tsz_binder::SemanticDefEntry>,
    ) {
        ...
        for (&sym_id, entry) in semantic_defs {
                ...
                for name_str in &entry.extends_names {
                    ...
                    if let Some(candidates) = self.definition_store.find_defs_by_name(name_atom) {
                        for &candidate_id in &candidates {
                            ...
                            if let Some(candidate_info) = self.definition_store.get(candidate_id)
                                && matches!(
                                    candidate_info.kind,
            ...
            if !entry.implements_names.is_empty() {
                ...
                if !resolved_implements.is_empty() {
                    self.definition_store
                        .set_implements(def_id, resolved_implements);
                }
            }
        }
    }
