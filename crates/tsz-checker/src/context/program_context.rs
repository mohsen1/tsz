//! Project-wide shared checker environment.
//!
//! [`ProgramContext`] captures the state that is identical across every per-file
//! [`CheckerContext`] in a project check run. It is split out of `context/mod.rs`
//! to keep that module focused on the per-file `CheckerContext` definition and the
//! standalone checker data types; this module owns the project-level shared state
//! plus its `apply_to` / `build_global_indices` orchestration. Pure code
