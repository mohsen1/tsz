...
/// (`T[K]`, `keyof T`, a conditional, a generic-alias application over one of
/// those, or an intersection carrying one). Constraint-relative sibling of
/// the query above, used by the nullable-union display policy.
pub(crate) fn is_deferred_constraint_relative_operand(
    db: &dyn TypeDatabase,
    definitions: &tsz_solver::def::DefinitionStore,
    type_id: TypeId,
) -> bool {
    tsz_solver::type_queries::is_deferred_constraint_relative_operand(db, definitions, type_id)
}
