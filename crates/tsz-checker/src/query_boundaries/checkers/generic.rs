...
pub(crate) fn generic_index_filters_current_type_param_keys(
    ...
    for _ in 0..8 {
        ...
        if type_is_keyof_same_type_param_identity(checker, index_type, type_param) {
            return true;
        }
        ...
        if let Some(name_atom) =
            type_parameter_name(checker.ctx.types.as_type_database(), index_type)
        {
            let name = checker.ctx.types.resolve_atom(name_atom);
            if let Some(&scoped_type_id) = checker.ctx.type_parameter_scope.get(&name)
        ...
        if let Some(candidates) =
            conditional_key_filter_candidates(checker.ctx.types.as_type_database(), index_type)
        {
            return candidates
                .into_iter()
                .filter(|&candidate| candidate != TypeId::NEVER)
                .any(|candidate| {
                    type_is_keyof_same_type_param_identity(checker, candidate, type_param)
                });
        }

        let Some(app) =
            crate::query_boundaries::common::type_application(checker.ctx.types, index_type)
        else {
            let evaluated = checker.evaluate_type_with_env(index_type);
        ...
        index_type = checker.resolve_lazy_type(instantiated);
    }
    false
}

fn type_is_keyof_same_type_param_identity(
    checker: &CheckerState<'_>,
    ty: TypeId,
    type_param: TypeId,
) -> bool {
    if let Some(inner) = crate::query_boundaries::common::keyof_inner_type(checker.ctx.types, ty) {
        return same_type_param_identity(checker, inner, type_param);
    }

    crate::query_boundaries::common::intersection_members(checker.ctx.types, ty).is_some_and(
        |members| {
            members.iter().any(|&member| {
                crate::query_boundaries::common::keyof_inner_type(checker.ctx.types, member)
                    .is_some_and(|inner| same_type_param_identity(checker, inner, type_param))
            })
        },
    )
}

fn same_type_param_identity(checker: &CheckerState<'_>, left: TypeId, right: TypeId) -> bool {
    left == right
        || crate::query_boundaries::common::type_param_info(checker.ctx.types, left)
