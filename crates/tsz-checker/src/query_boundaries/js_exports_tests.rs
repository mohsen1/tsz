...
#[test]
fn bailout_surface_is_dynamic_and_not_cached() {
    let mut parser = ParserState::new("test.js".to_string(), String::new());
    ...
    state::set_cross_arena_bailout_epoch_for_test(11);
    let bailout_epoch_before = CheckerState::<'_>::cross_arena_bailout_epoch();
    state::set_cross_arena_bailout_epoch_for_test(12);

    let surface = checker.cache_js_export_surface_if_authoritative(
        7,
        JsExportSurface {
            direct_export_type: Some(TypeId::STRING),
    ...
    assert!(surface.is_provisional);
    assert!(surface.has_commonjs_exports);
    assert_eq!(
        surface.lookup_named_export("temporarily-unknown", &types),
        Some(TypeId::ANY)
    );
    assert_eq!(surface.to_type_id(&mut checker), Some(TypeId::ANY));
    assert!(!checker.ctx.js_export_surface_cache.contains_key(&7));
