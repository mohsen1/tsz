...
impl<'a> CheckerState<'a> {
    ...
    // =========================================================================
    // Component Props Type Resolution
    // =========================================================================

    /// Get the props type for a component (class or function) JSX element.
    #[allow(dead_code)]
    ///
    /// For class components: gets the construct signature's return type, then
    /// looks up the attribute property via `JSX.ElementAttributesProperty`.
    /// For function components: gets the first parameter type of the call signature.
    fn get_component_props_type(&mut self, component_type: TypeId) -> Option<TypeId> {
        if component_type == TypeId::ANY || component_type == TypeId::ERROR {
            return None;
        }

        // Check for ElementAttributesProperty in JSX namespace
        let attrs_prop_name = self.get_element_attributes_property_name();
    ...
    #[allow(dead_code)]
    ///
    /// If the interface has a property (e.g., `{ prop: any }`), returns Some("prop").
    /// If the interface is empty `{}` or not found, returns None (meaning attributes
    /// come directly from the instance type).
    fn get_element_attributes_property_name(&mut self) -> Option<String> {
        ...
        let symbol = self
            ...
            .get_symbol_with_libs(jsx_sym_id, &lib_binders)?;
        let exports = symbol.exports.as_ref()?;
        let attrs_prop_sym_id = exports.get("ElementAttributesProperty")?;

        // Get the type of ElementAttributesProperty
        let attrs_type = self.type_reference_symbol_type(attrs_prop_sym_id);
        let attrs_type = self.evaluate_type_with_env(attrs_type);
