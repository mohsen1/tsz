...
impl<'a, 'ctx> GeneratorChecker<'a, 'ctx> {
    ...
    fn get_function_body(&self, node: &tsz_parser::parser::node::Node) -> NodeIndex {
        if node.kind == syntax_kind_ext::FUNCTION_DECLARATION {
            if let Some(func) = self.ctx.arena.get_function_declaration(node) {
                return func.body;
            }
    ...
    fn is_iterable(&self, type_id: TypeId) -> bool {
        use tsz_solver::judge::IterableKind;
    ...
    fn get_iterator_return_type(&self, type_id: TypeId) -> TypeId {
        ...
        use tsz_solver::operations::get_iterator_info;
    ...
    fn create_generator_type(
        ...
        let next_method = self.ctx.types.function(tsz_solver::FunctionShape {
            type_params: vec![],
        ...
        let return_method = self.ctx.types.function(tsz_solver::FunctionShape {
            type_params: vec![],
            params: vec![tsz_solver::ParamInfo {
                name: Some(self.ctx.types.intern_string("value")),
                type_id: return_type,
        ...
        let throw_method = self.ctx.types.function(tsz_solver::FunctionShape {
            type_params: vec![],
            params: vec![tsz_solver::ParamInfo {
    ...
    fn lookup_global_type(&self, name: &str) -> Option<TypeId> {
        ...
        if let Some(sym_id) = self.ctx.binder.file_locals.get(name) {
            if let Some(symbol) = self.ctx.binder.get_symbol(sym_id) {
                let lowering = tsz_solver::TypeLowering::new(self.ctx.arena, self.ctx.types);
                if !symbol.declarations.is_empty() {
                    return Some(lowering.lower_interface_declarations(&symbol.declarations));
                }
            }
...
//   use tsz_solver::operations::get_async_iterable_element_type;
...
mod tests {
    ...
    fn test_async_generator_yield_type_extraction() {
        ...
        let promise_result = types.object(vec![tsz_solver::PropertyInfo {
            name: then_name,
        ...
        // Create next method
        let next_method = types.function(tsz_solver::FunctionShape {
            type_params: vec![],
            params: vec![tsz_solver::ParamInfo {
        ...
        let async_gen = types.object(vec![tsz_solver::PropertyInfo {
            name: next_name,
            type_id: next_method,
            write_type: next_method,
            optional: false,
