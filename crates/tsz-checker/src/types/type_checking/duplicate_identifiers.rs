...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn check_duplicate_identifiers(&mut self) {
        ...
        for sym_id in symbol_ids {
            ...
            // TS2300: Check for property-vs-method kind conflicts across merged
            // interface declarations (including across files). When the same member
            // name appears as PROPERTY_SIGNATURE in one interface declaration and
            // METHOD_SIGNATURE in another, it's a duplicate identifier error.
            {
                use tsz_parser::parser::syntax_kind_ext;

                // Collect all interface declarations (including remote ones)
                let all_interface_decls: Vec<(NodeIndex, u32, bool)> = declarations
                    .iter()
                    .filter(|(_, flags, _, _)| (flags & symbol_flags::INTERFACE) != 0)
                    .map(|&(idx, flags, is_local, _)| (idx, flags, is_local))
                    .collect();

                if all_interface_decls.len() > 1 {
                    // Maps member name -> (first-seen kind, first-seen name node, is-local)
                    let mut member_kinds: FxHashMap<String, (u16, NodeIndex, bool)> =
                        FxHashMap::default();

                    for &(decl_idx, _, is_local) in &all_interface_decls {
                        let arena_opt = self
                            .ctx
                            .binder
                            .declaration_arenas
                        ...
                        for &member_idx in &iface.members.nodes {
                            ...
                            if let Some(&(prev_kind, prev_name_idx, prev_is_local)) =
                                member_kinds.get(&name)
                            {
                                if prev_kind != member_node.kind {
                                    let message = format_message(
                                        diagnostic_messages::DUPLICATE_IDENTIFIER,
                                        &[&name],
                                    );
                                ...
                                member_kinds
                                    .insert(name.clone(), (member_node.kind, sig.name, is_local));
                            }
                        }
                    }
                }
            ...
            // TS2300: Check for `prototype` namespace export in class+namespace merges.
            // When a class merges with a namespace that exports a `prototype` member,
            // TSC emits TS2300 on the `prototype` declaration because it conflicts with
            // the class's implicit `prototype` property.
            {
                let has_class = declarations
                    .iter()
                    .any(|(_, flags, _, _)| (flags & symbol_flags::CLASS) != 0);
                let has_namespace = declarations.iter().any(|(_, flags, _, _)| {
                    (flags & (symbol_flags::NAMESPACE_MODULE | symbol_flags::VALUE_MODULE)) != 0
                });
                if has_class && has_namespace {
                    // Check namespace exports for a `prototype` member
                    if let Some(exports) = self
                        .ctx
                        .binder
                        .get_symbol(sym_id)
                        .and_then(|s| s.exports.as_ref().cloned())
                    {
                        for (name, member_id) in exports.iter() {
                            if name == "prototype" {
                                if let Some(export_sym) = self.ctx.binder.get_symbol(*member_id) {
                                    let decl_node = export_sym.value_declaration;
            ...
            // When any pair of declarations conflicts, TSC reports TS2300 on ALL
            // declarations of the symbol (not just the conflicting pair). This is
            // because the symbol as a whole has irreconcilable declarations.
            // Add all local declarations to the conflicts set when any conflict is found.
    ...
    /// TS2300: Check for duplicate member conflicts in global augmentations.
    ///
    /// Handles two cases:
    /// 1. Interface member kind conflicts: when `declare global { interface X { ... } }`
    ///    appears in multiple files, check for property-vs-method name conflicts.
    /// 2. Namespace member conflicts: when `declare global { namespace X { ... } }`
    ///    appears in multiple files, check for duplicate declarations (e.g. type alias
    ///    vs import alias with the same name).
    pub(crate) fn check_global_augmentation_member_kind_conflicts(&mut self) {
        use crate::diagnostics::{diagnostic_codes, diagnostic_messages, format_message};
        use rustc_hash::FxHashMap;
        use tsz_parser::parser::syntax_kind_ext;

        for (_name, augmentations) in &self.ctx.binder.global_augmentations {
            if augmentations.len() <= 1 {
                continue;
            }

            // Track member name -> (first-seen kind, first-seen name node, is-local)
            let mut member_kinds: FxHashMap<String, (u16, NodeIndex, bool)> = FxHashMap::default();

            // Track namespace statements by name for duplicate detection.
            // Each entry stores (name_node_idx, augmentation_index) so we can
            // look up the arena and file name from the augmentation later.
            let mut ns_member_names: FxHashMap<String, Vec<(NodeIndex, usize)>> =
                FxHashMap::default();

            for (aug_idx, aug) in augmentations.iter().enumerate() {
                let arena = aug.arena.as_deref().unwrap_or(self.ctx.arena);
                let is_local = aug.arena.is_none()
                    || aug
                        .arena
                        .as_ref()
                        .is_some_and(|a| std::ptr::eq(&**a, self.ctx.arena));

                let Some(node) = arena.get(aug.node) else {
                    continue;
                };

                // Case 1: Interface member kind conflicts
                ...
                // Case 2: Namespace member duplicate detection
                // Check for duplicate type/value declarations across merged namespaces
                // inside `declare global { namespace X { type VNode = ...; } }` blocks.
                if let Some(module_decl) = arena.get_module(node) {
                    // The module body is typically a MODULE_BLOCK containing statements
                    let body_idx = module_decl.body;
                    if let Some(body_node) = arena.get(body_idx) {
                        if body_node.kind == syntax_kind_ext::MODULE_BLOCK {
                            if let Some(block) = arena.get_module_block(body_node) {
                                if let Some(stmts) = block.statements.as_ref() {
                                    for &stmt_idx in &stmts.nodes {
                                        let Some(stmt_node) = arena.get(stmt_idx) else {
                                            continue;
                                        };
                                        // Get name from type alias, import equals, etc.
                                        let name_and_node = match stmt_node.kind {
                                            syntax_kind_ext::TYPE_ALIAS_DECLARATION => {
                                                arena.get_type_alias(stmt_node).and_then(|ta| {
                                                    arena.get(ta.name).and_then(|n| {
                                                        arena.get_identifier(n).map(|id| {
                                                            (id.escaped_text.clone(), ta.name)
                                                        })
                                                    })
                                                })
                                            }
                                            syntax_kind_ext::IMPORT_EQUALS_DECLARATION => {
                                                arena.get_import_decl(stmt_node).and_then(|ie| {
                                                    // For import-equals, import_clause is the name identifier
                                                    arena.get(ie.import_clause).and_then(|n| {
                                                        arena.get_identifier(n).map(|id| {
                                                            (
                                            ...
                                            syntax_kind_ext::EXPORT_DECLARATION => {
                                                // `export import X = Y.Z;` is parsed as
                                                // EXPORT_DECLARATION wrapping IMPORT_EQUALS_DECLARATION.
                                                arena.get_export_decl(stmt_node).and_then(|ed| {
                                                    let inner_idx = ed.export_clause;
                                                    arena.get(inner_idx).and_then(|inner_node| {
                                                        if inner_node.kind
                                                            == syntax_kind_ext::IMPORT_EQUALS_DECLARATION
                                                        {
                                                            arena.get_import_decl(inner_node).and_then(
                                                                |ie| {
                                                                    arena
                                                                        .get(ie.import_clause)
                                                                        .and_then(|n| {
                                                                            arena
                                                                                .get_identifier(n)
                                                                                .map(|id| {
                                        ...
                                        if let Some((name, name_idx)) = name_and_node {
                                            ns_member_names
                                                .entry(name)
                                                .or_default()
                                                .push((name_idx, aug_idx));
                                        }
                                    }
                                }
                            }
            ...
            // Report TS2300 for namespace member duplicates.
            // Global augmentation duplicates are reported for ALL files (including
            // node_modules) because they affect the global type space. This matches
            // tsc behavior where TS2300 is emitted even in external library files
            // for global augmentation conflicts.
            for (name, nodes) in &ns_member_names {
                if nodes.len() > 1 {
                    let message =
                        format_message(diagnostic_messages::DUPLICATE_IDENTIFIER, &[name]);
                    for &(name_idx, aug_index) in nodes {
                        let aug = &augmentations[aug_index];
                        let arena = aug.arena.as_deref().unwrap_or(self.ctx.arena);
                        let file_name = aug.file_name.as_deref().unwrap_or(&self.ctx.file_name);

                        // Get span from the correct arena
