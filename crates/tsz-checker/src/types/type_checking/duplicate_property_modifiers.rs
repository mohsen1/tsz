    ...
    pub(crate) fn check_type_literal_duplicate_properties(&mut self, members: &[NodeIndex]) {
        ...
        for (name, entries) in &seen_properties {
            ...
            for &(idx, type_ann, _) in entries.iter() {
                ...
                // member as its group siblings. Oracle-confirmed against
                // `typescript@7.0.2` (re-verified for #17203): this fires
                // unconditionally, including for an all-late-bound group
                // (`{ [c0]: T; [c1]: T }` with `c0 === c1`) — tsc does NOT
                // merge that silently.
                self.error_at_node(
                    error_node,
