...
impl<'a> CheckerState<'a> {
    ...
    pub(in crate::types_domain) fn is_expando_function_assignment(
        ...
        if let Some(prototype_root_expr) = prototype_root_expr
            && let Some(root_ident) = self.ctx.arena.get_identifier_at(prototype_root_expr)
            && let Some(root_sym_id) = self.ctx.binder.file_locals.get(&root_ident.escaped_text)
        ...
        if let Some(sym_id) = sym_id
            && prototype_root_expr.is_some()
            && let Some(local_symbol) = self.ctx.binder.get_symbol(sym_id)
            && (local_symbol.flags & symbol_flags::ALIAS) != 0
            && local_symbol.import_module.is_some()
        {
            return false;
        }
        ...
        if let Some(sym_id) = sym_id
            && let Some(symbol) = self
                ...
                .or_else(|| self.ctx.binder.get_symbol(sym_id))
        {
            // Imported aliases should still be checked against the imported shape,
            // even for `Ctor.prototype.foo = ...` writes in checkJs.
            if prototype_root_expr.is_some()
