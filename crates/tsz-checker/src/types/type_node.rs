            ...
            for &elem_idx in &tuple_type.elements.nodes {
                ...
                if elem_node.kind == syntax_kind_ext::OPTIONAL_TYPE {
                    ...
                    if let Some(wrapped) = self.ctx.arena.get_wrapped_type(elem_node) {
                        ...
                        let elem_type = if is_rest_optional {
                            self.try_canonicalize_array_type_reference(inner_idx)
                                .unwrap_or_else(|| self.check(inner_idx))
                        } else {
                            self.check(inner_idx)
                        };
                        if is_rest_optional
                    ...
                    if let Some(wrapped) = self.ctx.arena.get_wrapped_type(elem_node) {
                        let elem_type = self
                            .try_canonicalize_array_type_reference(wrapped.type_node)
                            .unwrap_or_else(|| self.check(wrapped.type_node));
                    ...
                    if let Some(data) = self.ctx.arena.get_named_tuple_member(elem_node) {
                        let elem_type = if data.dot_dot_dot_token {
                            self.try_canonicalize_array_type_reference(data.type_node)
                                .unwrap_or_else(|| self.check(data.type_node))
                        } else {
                            self.check(data.type_node)
                        };
