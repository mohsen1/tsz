...
impl<'a, 'ctx> TypeNodeChecker<'a, 'ctx> {
    ...
    fn compute_type(&mut self, idx: NodeIndex) -> TypeId {
        ...
        match node.kind {
            ...
            // Handled here (not via TypeLowering) so that cross-file import type
            // references in extends position — e.g. `import("./m").T` — are resolved
            // through the checker's module-resolution machinery rather than returning
            // ERROR (the TypeLowering fallback for unrecognised call-expression type names).
            k if k == syntax_kind_ext::CONDITIONAL_TYPE => self.get_type_from_conditional_type(idx),

            // Fall back to TypeLowering for type nodes not handled above
            // (indexed access types, etc.)
    ...
    /// Get type from a conditional type node (`T extends U ? X : Y`).
    ///
    ...
    fn get_type_from_conditional_type(&mut self, idx: NodeIndex) -> TypeId {
        ...
        let Some(data) = self.ctx.arena.get_conditional_type(node) else {
            return TypeId::ERROR;
        };
        // NodeIndex is Copy — extract fields before dropping the arena borrow.
        ...
        let is_distributive = self.is_naked_type_param_node(check_node);
        ...
        let saved: Vec<(String, Option<TypeId>)> = if extends_type == TypeId::ERROR {
            ...
            tsz_solver::collect_infer_bindings(self.ctx.types, extends_type)
                .into_iter()
                .map(|(atom, type_id)| {
                    let name = self.ctx.types.resolve_atom_ref(atom).to_string();
                    let prev = self.ctx.type_parameter_scope.insert(name.clone(), type_id);
                    (name, prev)
                })
                .collect()
        };
        ...
        self.ctx.types.factory().conditional(cond)
    }

    /// Return `true` when `node_idx` is a "naked" type parameter reference
    /// (a bare `TYPE_REFERENCE` to an in-scope type parameter, with no type
    /// arguments), used to decide whether a conditional type is distributive.
    fn is_naked_type_param_node(&self, node_idx: NodeIndex) -> bool {
        let mut current = node_idx;
        for _ in 0..tsz_common::limits::MAX_TREE_WALK_ITERATIONS {
            let Some(node) = self.ctx.arena.get(current) else {
                return false;
            };
            match node.kind {
                k if k == syntax_kind_ext::PARENTHESIZED_TYPE => {
                    if let Some(data) = self.ctx.arena.get_wrapped_type(node) {
                ...
                k if k == syntax_kind_ext::TYPE_REFERENCE => {
                    let Some(data) = self.ctx.arena.get_type_ref(node) else {
                        return false;
                    };
                    if data
                        .type_arguments
                    ...
                    if let Some(name_node) = self.ctx.arena.get(data.type_name)
