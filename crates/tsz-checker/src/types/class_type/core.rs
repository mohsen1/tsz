    ...
    pub(crate) fn get_class_instance_type_inner(
        ...
        if !skip_heritage_merge && let Some(ref heritage_clauses) = class.heritage_clauses {
            for &clause_idx in &heritage_clauses.nodes {
                ...
                let raw_base_names_current_class = self
                    .heritage_name_text(expr_idx)
                    .is_none_or(|base_name| base_name == self.get_class_name_from_decl(class_idx));
                let base_in_resolution_set = self
                    .ctx
                    .class_instance_resolution_set
                    .contains(&base_sym_id)
                    && raw_base_names_current_class
                    || canonical_base_sym
                        .is_some_and(|sym| self.ctx.class_instance_resolution_set.contains(&sym));
                let base_visited = (raw_base_names_current_class && visited.contains(&base_sym_id))
                ...
                if let Some(current_sym) = current_sym {
                    if (base_sym_id == current_sym && raw_base_names_current_class)
