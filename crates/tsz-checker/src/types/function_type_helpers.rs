...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn check_function_return_completeness(&mut self, ctx: FunctionReturnCheckCtx) {
        ...
        // A generator's TReturn (see `generator_bare_return_check_type`), extracted
        // once; both consumers are gated on `noImplicitReturns`, so skip it when off.
        let generator_return_completeness =
            if function_is_generator && self.ctx.no_implicit_returns() {
                self.generator_return_type_for_implicit_return_check(effective_return_type)
            } else {
                None
            };
        let ts7030_check_type = if !self.ctx.no_implicit_returns() {
            check_return_type
        } else if function_is_generator {
            generator_return_completeness.unwrap_or(TypeId::UNKNOWN)
        } else {
            self.return_type_for_implicit_return_check(effective_return_type, is_async, false)
        };
        ...
        // TS7030 per bare `return;`: a generator uses its TReturn (#17444).
        let bare_return_check_type = if function_is_generator {
