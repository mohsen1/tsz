...
/// Walk scope chain to find a symbol for a node's identifier name.
...
/// Resolve a symbol name across binder, global index, cross-file binders, and lib contexts.
pub(crate) fn resolve_augmentation_symbol(
    name: &str,
    decl_binder: &tsz_binder::BinderState,
    global_file_locals_index: Option<
        &rustc_hash::FxHashMap<String, Vec<(usize, tsz_binder::SymbolId)>>,
    >,
    all_binders: Option<&[std::sync::Arc<tsz_binder::BinderState>]>,
    lib_contexts: &[crate::context::LibContext],
) -> Option<tsz_binder::SymbolId> {
