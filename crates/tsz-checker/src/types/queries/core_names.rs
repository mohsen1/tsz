...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn computed_property_expression_name_atom(
        ...
        let sym_id = self.resolve_computed_name_expression_symbol(expr_idx)?;
        crate::types_domain::computed_names::symbol_binding_property_atom(&self.ctx, sym_id)
    }
    ...
    pub(crate) fn computed_property_expression_is_symbol_named(&self, expr_idx: NodeIndex) -> bool {
        self.get_symbol_property_name_from_expr(expr_idx).is_some()
            || self
                ...
                .is_some()
            || self
                .resolve_computed_name_expression_symbol(expr_idx)
                .and_then(|sym_id| {
                    crate::types_domain::computed_names::symbol_binding_property_atom(
                        &self.ctx, sym_id,
                    )
                })
                .is_some()
    ...
    pub(crate) fn computed_identifier_unique_symbol_property_ref(
        ...
        let sym_id = self.resolve_computed_name_expression_symbol(expr_idx)?;
        crate::types_domain::computed_names::unique_symbol_property_ref(&self.ctx, sym_id)
    }

    /// Resolve the binding a computed-name expression refers to (identifier
    ...
    pub(crate) fn resolve_computed_name_expression_symbol(
        ...
        if expr_node.kind == syntax_kind_ext::PARENTHESIZED_EXPRESSION {
            ...
            return self.resolve_computed_name_expression_symbol(paren.expression);
        }
