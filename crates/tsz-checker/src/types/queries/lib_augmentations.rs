...
mod tests {
    ...
    fn cached_lib_type_allows_deferred_actual_lib_lazy_dependencies() {
        ...
        let ty = checker
            .resolve_lib_type_by_name("GlobalEventHandlers")
            .expect("DOM GlobalEventHandlers should resolve");
        let has_deferred_actual_lib_lazy =
            crate::query_boundaries::common::collect_lazy_def_ids(checker.ctx.types, ty)
                .into_iter()
                .any(|def_id| {
                    checker.ctx.definition_store.get_body(def_id).is_none()
                        && checker
                            .ctx
                            .definition_store
                            .get_name(def_id)
                            .is_some_and(|atom| {
                                let name = checker.ctx.types.resolve_atom(atom);
                                checker.ctx.actual_lib_def_id_for_bare_name(name.as_str())
                                    == Some(def_id)
                            })
                });

        assert!(
            has_deferred_actual_lib_lazy,
            "fixture should contain an unresolved actual-lib lazy dependency"
        );
        assert!(
            checker.cached_lib_type_is_usable("GlobalEventHandlers", Some(ty)),
