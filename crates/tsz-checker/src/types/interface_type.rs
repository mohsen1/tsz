    ...
    fn merge_interface_types_impl(
        ...
        match (derived_kind, base_kind) {
            ...
            (
                ...
                let properties =
                    self.merge_properties(&derived_shape.properties, &base_shape.properties, mode);
                let result = interface_merge_boundary::merged_object_with_index_type(
                    self.ctx.types,
                    MergedObjectSurface::new(
                        properties,
                        base_shape.string_index,
                        base_shape.number_index,
            ...
            (
                ...
                let properties =
                    self.merge_properties(&derived_shape.properties, &base_shape.properties, mode);
                interface_merge_boundary::merged_object_with_index_type(
                    self.ctx.types,
                    MergedObjectSurface::new(
                        properties,
                        derived_shape.string_index,
                        derived_shape.number_index,
                        derived_shape.symbol_index,
                        derived_shape.symbol,
                    ),
                )
            }
