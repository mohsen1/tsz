    ...
    pub(crate) fn emit_uninferred_callback_unknown_body_diagnostics(
        ...
        for (index, &arg_idx) in args.iter().enumerate() {
            ...
            for tp in &shape.type_params {
                ...
                let has_non_callback_evidence =
                    args.iter()
                        .enumerate()
                        .any(|(other_index, &other_arg_idx)| {
                            if other_index == index || self.is_callback_like_argument(other_arg_idx)
                            {
                                return false;
                            }
                            let Some(other_param_type) = shape
                                .params
                                .get(other_index)
                                .map(|p| p.type_id)
                                .or_else(|| {
                            ...
                            if !common::contains_type_parameter_named(
                                self.ctx.types,
                                other_param_type,
                                tp.name,
                            ) {
