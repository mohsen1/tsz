...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn direct_round1_literal_index_key_type_params(
        ...
        for (i, &arg_type) in arg_types.iter().enumerate() {
            ...
            if !self.type_param_feeds_sensitive_indexed_callback_param(
                shape,
                sensitive_args,
                tp_info.name,
            ) {
            ...
            let literal_arg_type = args
                ...
                .and_then(|&arg_idx| self.literal_type_from_initializer(arg_idx))
                .unwrap_or(arg_type);
            if self.widen_literal_type(literal_arg_type) == literal_arg_type {
                continue;
            }
    ...
    fn type_param_feeds_sensitive_indexed_callback_param(
        ...
        shape.params.iter().enumerate().any(|(i, param)| {
            sensitive_args.get(i).copied().unwrap_or(false)
                && self.type_contains_index_access_indexed_by_param(
                    param.type_id,
                    type_param_name,
                    8,
                )
    ...
    fn type_contains_index_access_indexed_by_param(
        ...
        {
            return common::contains_type_parameter_named(self.ctx.types, index, type_param_name)
                || self.type_contains_index_access_indexed_by_param(
                    object,
                    type_param_name,
                    depth - 1,
        ...
        match common::classify_for_traversal(self.ctx.types, type_id) {
            common::TypeTraversalKind::Application { base, args, .. } => {
                self.type_contains_index_access_indexed_by_param(base, type_param_name, depth - 1)
            ...
            common::TypeTraversalKind::TypeParameter {
                ...
                self.type_contains_index_access_indexed_by_param(ty, type_param_name, depth - 1)
            }),
            ...
            | common::TypeTraversalKind::StringIntrinsic(inner) => {
                self.type_contains_index_access_indexed_by_param(inner, type_param_name, depth - 1)
            }
            common::TypeTraversalKind::Tuple(elements_id) => self
                .ctx
                .types
