    ...
    pub(crate) fn compute_single_call_argument_type(
        ...
        let pushed_this_type = if let Some(et) = expected_type {
            ...
            if let Some(this_type) = ctx_helper.get_this_type_from_marker() {
                ...
                // The expected type may be a type alias Application
                // (e.g. `ConstructorOptions<Data>` wrapping `Props<Data> & ThisType<Instance<Data>>`).
                // Expand the alias body and retry ThisType extraction.
                let mut found = false;
                if let Some(tsz_solver::TypeData::Application(app_id)) = self.ctx.types.lookup(et) {
                        ...
                        self.ctx.types.lookup(app.base)
                    {
                        ...
                        if let Some(body) = env.resolve_lazy(def_id, self.ctx.types) {
                            ...
                            if let Some(this_type) = expanded_ctx.get_this_type_from_marker() {
                                self.ctx.this_type_stack.push(this_type);
                                found = true;
                            }
                        }
