    ...
    pub(crate) fn get_type_of_identifier_with_request(
        ...
        if let Some(sym_id) = self.resolve_identifier_symbol(idx) {
            ...
            if self.alias_resolves_to_type_only(sym_id) {
                if !self.is_identifier_in_type_position(idx)
                    && let Some((value_sym_id, value_type)) =
                        self.current_file_value_type_named(name)
                {
                    return self.check_flow_usage(idx, value_type, value_sym_id);
                }
                if !self.is_identifier_in_type_position(idx)
                    && let Some((value_sym_id, value_decl, value_file_idx)) =
                        self.same_file_value_symbol_for_type_symbol(sym_id)
                {
                    let mut value_type = self.type_of_value_declaration_for_cross_file_symbol(
                        value_sym_id,
                        value_decl,
                        value_file_idx,
                    );
                ...
                if !self.is_identifier_in_type_position(idx)
                    && let Some((value_sym_id, value_type)) =
                        self.same_scope_value_type_shadowing_symbol(idx, sym_id)
                {
                    return self.check_flow_usage(idx, value_type, value_sym_id);
                }
            ...
            if has_alias
                && !self.is_identifier_in_type_position(idx)
                && let Some((value_sym_id, value_type)) = self.current_file_value_type_named(name)
            {
                return self.check_flow_usage(idx, value_type, value_sym_id);
            }
            if has_alias
