...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn narrowable_type_for_reference(
        ...
        if !crate::query_boundaries::common::is_generic_type_with_union_constraint(
        ...
        if let ConstraintPosition::ElementObject(argument) = position
            && crate::query_boundaries::common::is_generic_type_without_nullable_constraint(
                self.ctx.types.as_type_database(),
                type_id,
            )
        {
        ...
        crate::query_boundaries::common::substitute_reference_base_constraints(
            self.ctx.types.as_type_database(),
            type_id,
        )
    }
