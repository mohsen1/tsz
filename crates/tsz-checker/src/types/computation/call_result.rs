...
impl<'a> CheckerState<'a> {
    pub(super) fn correlated_union_call_recovery_return(
        &mut self,
        callee_type: TypeId,
        arg_index: usize,
        actual: TypeId,
    ...
    pub(super) fn finalize_call_return_like_success(
        &mut self,
    ...
    pub(super) fn polymorphic_this_indexed_conditional_target(
        &mut self,
        callee_type: TypeId,
        args: &[NodeIndex],
        arg_types: &[TypeId],
    ...
    pub(super) fn report_polymorphic_this_indexed_conditional_arg(
        &mut self,
        callee_type: TypeId,
        args: &[NodeIndex],
        arg_types: &[TypeId],
    ...
    pub(super) fn error_argument_not_assignable_preserving_param_display(
        ...
        // This fallback path renders its own parameter display (variadic
        // tuple/mapped/`NoInfer` overrides above) instead of going through
        // `render_failure_reason`, so it does not automatically pick up the
        // TS5075/TS5082 bare-type-parameter-target elaboration the way the
        // direct-assignment (TS2322) surface does. Attach it explicitly when
        // `param_type` is itself a bare type parameter the argument fails to
        // relate to (#17449).
        let related_note = self
            .get_node_span(arg_idx)
            .map(|(start, end)| {
                self.normalized_anchor_span(arg_idx, start, end.saturating_sub(start))
            })
            .and_then(|(start, length)| {
                self.unrelated_type_parameter_target_related_info(
                    arg_type,
                    param_type,
                    &actual_display,
                    &target_display,
        ...
        match related_note {
            Some(note) => self.error_at_node_with_related(arg_idx, &message, code, vec![note]),
            None => self.error_at_node(arg_idx, &message, code),
        }
    }
    ...
    pub(super) fn stable_call_recovery_return_type_with_default_type_args(
        &self,
        callee_type: TypeId,
    ) -> Option<TypeId> {
    ...
    pub(super) fn is_spread_argument_marker_type(&self, type_id: TypeId) -> bool {
        common::is_spread_marker_tuple(self.ctx.types.as_type_database(), type_id)
    }

    pub(super) fn literalized_aggregate_actual_for_call_args(
        &mut self,
        args: &[NodeIndex],
        index: usize,
        actual: TypeId,
    ...
    pub(super) fn declared_rest_parameter_index_for_call(
        &self,
        callee_expr: NodeIndex,
    ) -> Option<usize> {
        let callee_sym = self
    ...
    pub(super) fn aggregate_actual_after_declared_rest_start(
        &mut self,
        actual: TypeId,
        index: usize,
    ...
    pub(super) fn spread_rest_tuple_diagnostic_types(
        &mut self,
        arg_idx: NodeIndex,
        expected: TypeId,
    ...
    pub(super) fn should_attempt_deferred_literal_elaboration(&mut self, expected: TypeId) -> bool {
        let expected = self.evaluate_type_with_env(expected);
        let expected = self.resolve_type_for_property_access(expected);
        let expected = self.resolve_lazy_type(expected);
        let expected = self.evaluate_application_type(expected);
    ...
    pub(super) fn callback_prefers_argument_level_return_mismatch(
        &self,
        arg_idx: NodeIndex,
    ) -> bool {
        let Some(func) = self
    ...
    pub(super) fn preferred_literal_expected_for_mismatch(
        &self,
    ...
    pub(super) fn is_generic_callable_against_nongeneric_target(
        &self,
        actual: TypeId,
        expected: TypeId,
    ) -> bool {
