...
fn type_param_feeds_sensitive_indexed_callback_param(
    ...
    shape.params.iter().enumerate().any(|(i, param)| {
        sensitive_args.get(i).copied().unwrap_or(false)
            && type_contains_index_access_indexed_by_param(db, param.type_id, type_param, 8)
    })
}
...
fn type_contains_index_access_indexed_by_param(
    ...
    {
        return crate::query_boundaries::generic_instantiation::type_contains_type_parameter_binder(
            db, index, type_param,
        ) || type_contains_index_access_indexed_by_param(db, object, type_param, depth - 1)
            || type_contains_index_access_indexed_by_param(db, index, type_param, depth - 1);
    }

    if let Some(shape) = common::function_shape_for_type(db, type_id) {
        return shape.params.iter().any(|param| {
            type_contains_index_access_indexed_by_param(db, param.type_id, type_param, depth - 1)
    ...
    match common::classify_for_traversal(db, type_id) {
        common::TypeTraversalKind::Application { base, args, .. } => {
            type_contains_index_access_indexed_by_param(db, base, type_param, depth - 1)
                || args.into_iter().any(|arg| {
                    type_contains_index_access_indexed_by_param(db, arg, type_param, depth - 1)
                })
        }
        common::TypeTraversalKind::TypeParameter {
            ...
            .into_iter()
            .chain(default)
            .any(|ty| type_contains_index_access_indexed_by_param(db, ty, type_param, depth - 1)),
        common::TypeTraversalKind::Members(members)
        | common::TypeTraversalKind::TemplateLiteral(members) => {
            members.into_iter().any(|member| {
                type_contains_index_access_indexed_by_param(db, member, type_param, depth - 1)
            })
        }
        ...
        | common::TypeTraversalKind::StringIntrinsic(inner) => {
            type_contains_index_access_indexed_by_param(db, inner, type_param, depth - 1)
        }
