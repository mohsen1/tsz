...
use super::callee_prep::{CallCalleePrep, PreparedCallCallee};
use super::finalize_after_args::CallAfterArgumentCollectionCtx;
...
impl<'a> CheckerState<'a> {
    pub(crate) fn get_type_of_call_expression_inner(
        ...
        let PreparedCallCallee {
            callee_type,
            is_super_call,
            explicit_call_type_arguments,
            nullish_cause,
        } = match self.prepare_call_callee(idx, request, node, call, contextual_type, args) {
            CallCalleePrep::Continue(prepared) => prepared,
            CallCalleePrep::Return(return_type) => return return_type,
        };
        ...
        self.finish_call_after_argument_collection(CallAfterArgumentCollectionCtx {
            idx,
            callee_expr: call.expression,
            args,
            arg_types,
            ...
            non_generic_contextual_types: non_generic_contextual_types.as_deref(),
            check_excess_properties,
            callable_ctx,
            is_generic_call,
            contextual_type,
            ...
            checker_round2_substitution: checker_round2_substitution.as_ref(),
            checker_round2_shape: checker_round2_shape.as_ref(),
            direct_literal_conflict_substitution: direct_literal_conflict_substitution.as_ref(),
