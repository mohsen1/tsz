...
impl<'a> CheckerState<'a> {
    ...
    pub(super) fn report_direct_constructor_type_param_constraint_mismatches(
        ...
        for (i, param) in shape.params.iter().enumerate() {
            ...
            let actual_for_check = common::widen_literal_type(self.ctx.types, actual);
    ...
    pub(super) fn generic_constructor_nested_constraint_failure_return(
        ...
        for (i, param) in shape.params.iter().enumerate() {
            if common::type_param_info(self.ctx.types, param.type_id).is_some() {
                continue;
            }
            let Some(&actual) = arg_types.get(i) else {
                continue;
            };
            for param_part in common::collect_all_types(self.ctx.types, param.type_id) {
                let Some(param_info) = common::type_param_info(self.ctx.types, param_part) else {
                    continue;
                };
                let Some(type_param) = shape
                    .type_params
    ...
    fn primitive_parts(&self, type_id: TypeId) -> Vec<TypeId> {
        common::collect_all_types(self.ctx.types, type_id)
            .into_iter()
            .map(|part| common::widen_literal_type(self.ctx.types, part))
            .filter(|&part| {
                matches!(
                    part,
                    TypeId::STRING
