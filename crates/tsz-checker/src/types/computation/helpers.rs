...
impl<'a> CheckerState<'a> {
    ...
    /// binder symbol.
    ///
    /// TypeScript gives an unannotated `const` initialized with a global
    ...
    /// upgrade for the initializer form, so every value-typing path — direct
    /// variable typing, the merged `type X = typeof X` value path, and
    /// cross-arena value-declaration delegation — agrees on the same identity.
    /// Without it, a `Symbol.for` const that is name-merged with a same-named
    /// type alias loses its identity and degrades to wide `symbol`, so an
    /// object literal `{ [X]: v }` synthesizes a `[k: symbol]: V` index
    /// signature instead of the symbol-keyed member the interface expects.
    ///
    ...
    pub(crate) fn const_symbol_factory_unique_value_type(
        ...
        let sym_id = self.get_symbol_id_for_variable_name(var_decl.name)?;
        Some(
            self.ctx
                .types
                .unique_symbol(tsz_solver::SymbolRef(sym_id.0)),
