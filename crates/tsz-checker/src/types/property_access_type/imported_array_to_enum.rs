...
impl<'a> CheckerState<'a> {
    pub(crate) fn imported_array_to_enum_member_literal_type(
        ...
        let base_name = self
            ...
            .escaped_text
            .clone();
        let (mut target_sym_id, mut target_file_idx, is_type_only) =
            self.resolve_value_import_target(&base_name, base_expr)?;
        if is_type_only {
            return None;
        }

        let mut target_symbol = self.get_cross_file_symbol(target_sym_id).or_else(|| {
            self.ctx
