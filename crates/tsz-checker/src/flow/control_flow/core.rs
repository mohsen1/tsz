...
pub struct FlowAnalyzer<'a> {
    ...
    /// Nesting depth of in-progress loop fixed-point iterations.
    ///
    /// While this is non-zero, every flow walk is reading the *provisional*
    /// loop assumption that `analyze_loop_fixed_point` injects to break its own
    /// recursion, so its results are incomplete in `tsc`'s sense and must not
    /// be committed to the shared flow cache.
    loop_fixed_point_depth: std::cell::Cell<u32>,
...
impl<'a> FlowAnalyzer<'a> {
    ...
    pub fn new(
        ...
        Self {
            ...
            loop_fixed_point_depth: std::cell::Cell::new(0),
