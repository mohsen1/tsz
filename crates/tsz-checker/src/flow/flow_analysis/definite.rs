...
impl<'a> CheckerState<'a> {
    ...
    pub(crate) fn apply_flow_narrowing_with_initial_type(
        ...
        // concrete literal result type, flow re-analysis at the access node is
        // redundant — a literal type cannot be narrowed further.
        //
        // NOTE: Primitive types (NUMBER, STRING, BOOLEAN, etc.) are NOT skipped here
        // because they CAN be narrowed to literals by equality checks. For example,
        // after `if (this.no === 1)`, `this.no: number` should narrow to `1`.
        //
        // Keep full flow narrowing for primitives, unions, objects, and type-parameters,
        // where access-level narrowing may still materially change the type.
        if let Some(node) = self.ctx.arena.get(idx)
            && (node.kind == syntax_kind_ext::PROPERTY_ACCESS_EXPRESSION
                || node.kind == syntax_kind_ext::ELEMENT_ACCESS_EXPRESSION)
            && matches!(
                declared_type,
                TypeId::UNDEFINED | TypeId::NULL | TypeId::VOID
            )
        {
            return declared_type;
        }
        // Literal types are already maximally specific — skip narrowing for them.
        if let Some(node) = self.ctx.arena.get(idx)
            && (node.kind == syntax_kind_ext::PROPERTY_ACCESS_EXPRESSION
                || node.kind == syntax_kind_ext::ELEMENT_ACCESS_EXPRESSION)
