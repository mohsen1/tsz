...
fn test_env_eval_seed_entries_skip_identity_and_depth_entries() {
    ...
    assert!(
        seeds.iter().any(|&(k, v)| k == useful_key && v == useful_value),
        "non-identity env-eval entries remain available as speed-only seeds"
    );
    assert!(
        !seeds.iter().any(|&(k, v)| k == depth_key && v == depth_value),
...
fn test_evaluate_type_with_cache_call_sites_pass_collection_flag() {
    ...
    for file in files {
        let src = fs::read_to_string(file).unwrap_or_else(|err| panic!("failed to read {file}: {err}"));
    ...
    for file in files.into_iter().filter(|file| *file != "src/state/type_environment/lazy.rs") {
        let src = fs::read_to_string(file).unwrap_or_else(|err| panic!("failed to read {file}: {err}"));
        for (line_no, line) in src.lines().enumerate() {
            if line.contains("evaluate_type_with_cache(") {
                let tail = src
...
fn test_env_eval_persist_empty_entries_fast_path_is_first() {
    let src = fs::read_to_string("src/context/env_eval_cache.rs")
        .expect("failed to read env_eval_cache");
    let body = src
