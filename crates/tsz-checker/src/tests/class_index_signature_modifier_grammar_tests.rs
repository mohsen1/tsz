...
//! `declare`, `abstract`, `async`, `override`, `accessor`, `const`, `export`,
//! and the `in`/`out` variance markers — is rejected with a single
...
//!
//! (`default [k]` is deliberately absent: it does not reach `checkGrammarModifiers`
//! as an index-signature modifier — the parser diverts it to a different recovery
//! (tsz `TS1068` vs tsc `TS1434`), a separate pre-existing gap unrelated to this
//! rule.)
//!
...
//! were silently accepted (the follow-up gap from #17276, fixed in #17287 /
//! #17280). This suite is the regression pin that fix shipped without: it locks
//! in the full rule so a future refactor of the class-member modifier walk
//! cannot quietly re-narrow the illegal set or start over-reporting.
//!
...
fn each_illegal_single_modifier_reports_one_ts1071() {
    ...
    for (modifier, needs_abstract_class) in cases {
        ...
        // Pin exactly one TS1071, anchored on this modifier — but NOT the total
        // diagnostic set: some modifiers legitimately draw a *second*, unrelated
        // diagnostic under a newer oracle (e.g. `override [k]` also gets TS4112,
        // "override in a class that extends nothing"), which this suite does not
        // own. Asserting on the TS1071 anchors alone keeps the pin at the right
        // altitude while still catching over-reporting (two TS1071s would fail).
        assert_eq!(
