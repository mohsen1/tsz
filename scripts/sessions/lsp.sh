...
5) Run: ./scripts/run-fourslash.sh --max 200
6) Analyze failures for highest-impact issues
7) Pick one high-signal LSP issue (hover/definition/references/completions/rename/diagnostics)
8) Implement the smallest targeted fix
...
10) Re-run fourslash tests to verify improvement
...
13) Append any issues you investigated but punted on (too complex, needs
    ...
    you skipped it.
14) git add docs/todos/lsp.md (if changed) and amend or create a
