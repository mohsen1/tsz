...
Goal: improve TypeScript conformance parity (start at 25% of test suite).
...
5) Compute 25% start offset and run:
   TOTAL_TESTS=$(find TypeScript/tests/cases -type f \( -name '*.ts' -o -name '*.tsx' -o -name '*.d.ts' \) | wc -l | tr -d ' ')
   OFFSET=$((TOTAL_TESTS / 4))
   ./scripts/conformance.sh run --offset "$OFFSET"
