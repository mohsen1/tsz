...
# report per pass into --junit-dir. Callers then adjudicate the union of those
# reports against scripts/ci/known-failures.txt via
# scripts/ci/known-failures-check.mjs — THAT check is the pass/fail gate; this
# script only distinguishes "tests ran to completion" from infrastructure
# failure.
#
# Pass structure (mirrors the memory constraints documented in
# scripts/ci/full-ci.sh and Cargo.toml's [profile.ci-unit] note):
...
#
# Exit codes:
#   0 - every pass completed and recorded a junit report (individual tests may
#       have FAILED; per-pass rcs are logged and the junit reports carry the
#       inventory — run known-failures-check.mjs to adjudicate)
...
#       nextest crash, bad arguments); remaining passes are not attempted
#
...
#     [--skip-checker-integration] [--checker-batch-size N]
#
# Environment:
#   CARGO_BUILD_JOBS               forwarded as --build-jobs when set
#   UNIT_NEXTEST_TEST_THREADS      forwarded as --test-threads for the general
#                                  pass when set (checker batches keep the
#                                  profile default, matching the previous
#                                  unit-lane behavior)
...
if (( checker_selected )) && [[ "$SKIP_CHECKER_INTEGRATION" == "0" ]]; then
  # The tsz-checker lib-test binary is larger than hosted runners can compile
