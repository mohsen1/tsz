...
Its exit code must distinguish "tests ran and were recorded" (0, even with
test failures) from infrastructure failures (build error, missing junit) —
that split is what lets known-failures-check.mjs be the only pass/fail gate
while batch masking (#15404) cannot return.

The tests copy the script into a temp workspace whose `cargo` is a stub, so
every nextest invocation and its junit side effects are scripted.
"""
...
#    the junit report first unless `nojunit` is present or rc is 4).
...
class UnitNextestTests(unittest.TestCase):
    ...
    def run_script(self, *args, fail_spec=""):
        ...
        env.pop("UNIT_NEXTEST_TEST_THREADS", None)
    ...
    def test_infrastructure_failure_stops_the_run(self):
        result = self.run_script(
            "--packages", "tsz-core tsz-checker", fail_spec="t_c 101 nojunit\n"
        )
    ...
    def test_build_jobs_and_test_threads_are_forwarded(self):
        env_result = subprocess.run(
            [str(self.script), "--junit-dir", str(self.junit_dir), "--packages", "tsz-core tsz-checker"],
            capture_output=True,
            text=True,
            env={
                **os.environ,
                "PATH": f"{self.tmp / 'bin'}{os.pathsep}{os.environ['PATH']}",
                "METADATA_FIXTURE": str(self.metadata_fixture),
                "CALL_LOG": str(self.call_log),
                "FAIL_SPEC": str(self.fail_spec),
        ...
        # checker batches keep the profile's thread default (previous unit-lane
        # behavior) but still get the build-jobs cap
        self.assertIn("--build-jobs 7", calls[1])
        self.assertNotIn("--test-threads", calls[1])
