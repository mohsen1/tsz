...
function getAdaptiveMemoryThreshold(workerCount) {
    const freeMem = os.freemem();
    const availablePerWorker = freeMem / Math.max(workerCount, 1);
    return Math.min(Math.max(availablePerWorker * MEMORY_PERCENT_THRESHOLD, MIN_MEMORY_THRESHOLD), MAX_MEMORY_THRESHOLD);
}
const colors = {
    reset: '\x1b[0m',
    red: '\x1b[31m',
...
async function collectTestFiles(dir, maxFiles) {
    const files = [];
...
async function collectProjectFiles(dir, maxFiles) {
    const files = [];
...
async function* walkDirectoryStream(dir) {
    try {
...
async function* streamTestFiles(testsBasePath, categories, maxTests) {
    let yielded = 0;
...
class WorkerPool {
    ...
    async terminate() {
        ...
        await Promise.all(this.workers.map(async (info) => {
            try {
...
export async function runConformanceTests(config = {}) {
    ...
    if (cfg.useWasm && process.env.TSZ_WASM_MAX_WORKERS) {
        ...
        if (Number.isFinite(maxWorkers) && maxWorkers > 0 && cfg.workers > maxWorkers) {
            log(`⚠️  WASM workers capped to ${maxWorkers} (TSZ_WASM_MAX_WORKERS)`, colors.yellow);
    ...
    const pool = new WorkerPool(cfg.workers, workerPath, workerDataBase, cfg.testTimeout, cfg.testsBasePath);
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    for (const arg of args) {
        if (arg.startsWith('--max='))
            ...
            config.useWasm = arg.split('=')[1] === 'true';
        else if (arg === '--wasm')
            config.useWasm = true;
        else if (arg === '--native')
            config.useWasm = false;
        else if (arg === '--server')
            useServer = true;
        else if (arg.startsWith('--filter='))
            config.filter = arg.split('=')[1];
        else if (arg.startsWith('--error-code='))
            config.errorCode = parseInt(arg.split('=')[1].replace(/^TS/i, ''), 10);
        else if (arg === '--print-test')
            config.printTest = true;
        else if (arg.startsWith('--dump-results='))
            config.dumpResults = arg.split('=')[1];
    }
