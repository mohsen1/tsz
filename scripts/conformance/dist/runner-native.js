...
import { parseTestCase } from './test-utils.js';
...
let passed = 0;
let failed = 0;
const missingErrors = new Map();
const extraErrors = new Map();
/**
 * Parse test directives from TypeScript test files.
 * Converts from shared ParsedTestCase to local ParsedTestCase format.
 */
function parseTestDirectives(source, filePath) {
    const parsed = parseTestCase(source, filePath);
...
function runTsc(testCase) {
    ...
    try {
        ...
        const program = ts.createProgram(testCase.files.map(f => path.join(tmpDir, f.name)), compilerOptions);
    ...
    return new Promise((resolve) => {
        ...
        try {
            ...
            if (!testCase.options.nolib) {
                const libOption = testCase.options.lib;
                if (libOption) {
...
async function main() {
    ...
    for (let i = 0; i < testsToRun.length; i++) {
        ...
        try {
            ...
            // Run native
            const { codes: nativeCodes, crashed, error } = await runNative(testCase);
            if (crashed) {
                if (CONFIG.verbose) {
                    console.log(`\n  💥 Crash: ${error}`);
                }
...
//# sourceMappingURL=runner-native.js.map