...
function generateProgressBar(percentage, passed, total, width = 20) {
    const filled = Math.round((percentage / 100) * width);
    const empty = width - filled;
    const bar = '\u2588'.repeat(filled) + '\u2591'.repeat(empty);
    return `Progress: [${bar}] ${percentage.toFixed(1)}% (${passed.toLocaleString()} / ${total.toLocaleString()} tests)`;
}
function updateSection(readme, startMarker, endMarker, newContent) {
...
function updateTsVersion(tsVersion) {
...
function updateReadme(stats, section) {
    let readme = fs.readFileSync(README_PATH, 'utf-8');
...
function updateEmitReadme(stats) {
    let readme = fs.readFileSync(README_PATH, 'utf-8');
