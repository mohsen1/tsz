...
export function parseLibReferences(source) {
    const refs = [];
    for (const match of source.matchAll(LIB_REFERENCE_RE)) {
        if (match[1])
            refs.push(match[1].trim().toLowerCase());
    }
    return refs;
}
export function resolveLibFilePath(libName, libDir) {
    ...
    if (!libDir || !fs.existsSync(path.join(libDir, 'es5.d.ts')))
        return null;
    const candidates = [
...
export function readLibContent(libName, libDir) {
    // Apply lib name aliasing (es6 -> es2015, etc.)
    const normalized = normalizeLibName(libName);
    const cacheKey = `${libDir}:${normalized}`;
    if (libContentCache.has(cacheKey))
        return libContentCache.get(cacheKey);
...
function loadLibRecursive(libName, libDir, out, seen) {
    // Apply lib name aliasing (es6 -> es2015, etc.)
    const normalized = normalizeLibName(libName);
...
function defaultCoreLibNameForTarget(targetName) {
    switch (targetName) {
        case 'es3':
        case 'es5':
            return 'es5';
...
export function collectLibFiles(libNames, libDir) {
    const out = new Map();
    const seen = new Set();
 ...
 */
function firstEnumValue(val) {
    return String(val).split(',')[0].trim().toLowerCase();
}
// ============================================================================
// Test Parsing
// ============================================================================
export function parseTestDirectives(code, filePath) {
...
function toCompilerOptions(opts) {
    ...
    if (opts.strict !== undefined)
        options.strict = toBool(opts.strict);
    if (opts.noimplicitany !== undefined)
        options.noImplicitAny = toBool(opts.noimplicitany);
    if (opts.strictnullchecks !== undefined)
        options.strictNullChecks = toBool(opts.strictnullchecks);
    if (opts.strictfunctiontypes !== undefined)
        options.strictFunctionTypes = toBool(opts.strictfunctiontypes);
    if (opts.strictbindcallapply !== undefined)
        options.strictBindCallApply = toBool(opts.strictbindcallapply);
    if (opts.strictpropertyinitialization !== undefined)
        options.strictPropertyInitialization = toBool(opts.strictpropertyinitialization);
    if (opts.strictbuiltiniteratorreturn !== undefined)
        options.strictBuiltinIteratorReturn = toBool(opts.strictbuiltiniteratorreturn);
    if (opts.noimplicitthis !== undefined)
        options.noImplicitThis = toBool(opts.noimplicitthis);
    if (opts.useunknownincatchvariables !== undefined)
        options.useUnknownInCatchVariables = toBool(opts.useunknownincatchvariables);
    if (opts.alwaysstrict !== undefined)
        options.alwaysStrict = toBool(opts.alwaysstrict);
    if (opts.nolib !== undefined)
        options.noLib = toBool(opts.nolib);
    ...
    if (opts.allowjs !== undefined)
        options.allowJs = toBool(opts.allowjs);
    if (opts.checkjs !== undefined)
        options.checkJs = toBool(opts.checkjs);
    ...
    if (opts.outdir !== undefined)
        options.outDir = String(opts.outdir);
    if (opts.outfile !== undefined)
        options.outFile = String(opts.outfile);
    if (opts.rootdir !== undefined)
        options.rootDir = String(opts.rootdir);
    if (opts.declarationdir !== undefined)
        options.declarationDir = String(opts.declarationdir);
    if (opts.sourcemap !== undefined)
        options.sourceMap = toBool(opts.sourcemap);
    if (opts.inlinesourcemap !== undefined)
        options.inlineSourceMap = toBool(opts.inlinesourcemap);
    if (opts.inlinesources !== undefined)
        options.inlineSources = toBool(opts.inlinesources);
    if (opts.removecomments !== undefined)
        options.removeComments = toBool(opts.removecomments);
    if (opts.importhelpers !== undefined)
        options.importHelpers = toBool(opts.importhelpers);
    if (opts.downleveliteration !== undefined)
        options.downlevelIteration = toBool(opts.downleveliteration);
    if (opts.preserveconstenums !== undefined)
        options.preserveConstEnums = toBool(opts.preserveconstenums);
    if (opts.noemithelpers !== undefined)
        options.noEmitHelpers = toBool(opts.noemithelpers);
    if (opts.noemitonerror !== undefined)
        options.noEmitOnError = toBool(opts.noemitonerror);
    if (opts.emitdeclarationonly !== undefined)
        options.emitDeclarationOnly = toBool(opts.emitdeclarationonly);
    if (opts.declarationmap !== undefined)
        options.declarationMap = toBool(opts.declarationmap);
    // Interop constraints
 ...
 */
export function runTsc(testCase, libDir, libSource = '', includeMessages = false, rootFilePath = '') {
    ...
    const libFiles = libNames.length ? collectLibFiles(libNames, libDir) : new Map();
    ...
    const virtualFileContents = new Map();
    for (const file of testCase.files) {
        ...
        const sf = ts.createSourceFile(resolvedFileName, content, compilerOptions.target ?? ts.ScriptTarget.ES2020, true, scriptKind);
    ...
    host.fileExists = (name) => {
        if (sourceFiles.has(name))
            return true;
        if (virtualFileContents.has(name))
            return true;
        // Fallback: check by basename (for lib files)
        if (sourceFiles.has(path.basename(name)))
            return true;
        return false;
    };
    host.readFile = (name) => {
        ...
        if (virtualFileContents.has(name))
            return virtualFileContents.get(name);
        ...
        if (libFiles.has(baseName))
            return libFiles.get(baseName);
        if (libSource && baseName === 'lib.d.ts')
            return libSource;
        return undefined;
    };
    ...
    const codes = [];
    const diagnostics = [];
    const processDiagnostic = (d) => {
        codes.push(d.code);
        if (includeMessages) {
            const message = ts.flattenDiagnosticMessageText(d.messageText, '\n');
            const info = { code: d.code, message };
    ...
    for (const sf of program.getSourceFiles()) {
        if (sf.fileName.startsWith('lib.') && sf.fileName.endsWith('.d.ts'))
            continue;
        if (sf.isDeclarationFile && libFiles.has(sf.fileName))
            continue;
 ...
 */
export function runTscOnFiles(files, options, libDir, includeMessages = false) {
    // Convert to ParsedTestCase format
    const testFiles = Object.entries(files).map(([name, content]) => ({ name, content }));
