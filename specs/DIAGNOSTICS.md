...
*   **Root Cause**: The Zang `ThinParser` is currently too strict. It often bails out or emits an error node for syntax that `tsc` (which is very lenient) accepts or recovers from.
