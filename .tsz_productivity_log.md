...
- 2026-02-16 02:19:26: TS2307 loop continued with import/eager-resolution fixes. Updated `crates/tsz-cli/src/driver.rs` to keep TS2792 for non-bundler module systems while avoiding bundler/classic regressions; updated `check_import_equals_declaration` in `crates/tsz-checker/src/import_checker.rs` to force TS2307 for ambient-relative `import = require("...")` plus correct TS2439 span at import statement start. Verified pass for `allowJsCrossMonorepoPackage.ts`, `ambientExternalModuleWithRelativeExternalImportDeclaration.ts`, `resolutionCandidateFromPackageJsonField2.ts`, and `pathMappingBasedModuleResolution6_classic.ts`.

- 2026-02-16 08:34:41: TS2307 focused iteration: changed `crates/tsz-cli/src/driver_resolution.rs` to treat bare module specifiers (`!/,.!)` as non-classic walkup in classic resolver candidates and `crates/tsz-cli/src/driver.rs` to gate TS2792 remap by module kind only. Ran focused checks: `es6ExportAssignment3.ts`, `es6ExportAll.ts`, and `es6Export*` filter now pass under `--error-code 2307`; full `scripts/conformance.sh analyze --error-code 2307` now reports TS2307 all-missing=177 (from 207), extra=34 (from 38), close matches remaining large but improved.

- 2026-02-16 08:39:36: TS2307 loop recheck after re-apply: kept `is_bare_module_specifier` skip for classic walkup and removed `resolution_prefers_2792` gating from TS2792 remap. Focus checks: `es6ExportAssignment3.ts` and `es6ExportAll.ts` pass in cached `--error-code 2307` runs. Full `scripts/conformance.sh analyze --error-code 2307` reports TS2307 missing=177 (extra=34), unchanged vs prior recheck despite scoped edits.

- 2026-02-16 08:40:41: Finalizing this TS2307 iteration, only CLI edits kept (`crates/tsz-cli/src/driver.rs`, `crates/tsz-cli/src/driver_resolution.rs`). Focused checks re-run: `es6ExportAssignment3.ts` and `es6ExportAll.ts` pass in cached `--error-code 2307` runs after these edits. `scripts/conformance.sh analyze --error-code 2307` still reports TS2307 missing=177, extra=34 (unchanged in aggregate), so next pass should likely target additional false-positive paths.
- 2026-02-16 09:16:45: TS2307 iteration: fixed bare specifier fallback behavior for `ModuleResolutionKind::Node16/NodeNext` while keeping Bundler compatibility. Removed permissive `node_modules/<pkg>.d.ts` fallback from `resolve_bare_specifier` and `resolve_node_module_specifier` unless bundler. Verified `nodeNextModuleResolution1.ts` now passes (`TS2307` emitted) and cached bundler cases remain pass (`cachedModuleResolution1/2/5`). `scripts/conformance.sh analyze --error-code 2307` now reports missing=176 (from 177), false-positive TS2307=37 (from 43).
