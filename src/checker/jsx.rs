...
impl<'a> JsxChecker<'a> {
    ...
    fn check_jsx_full_element(&self, idx: NodeIndex, errors: &mut Vec<JsxError>) {
        ...
        if let Some(opening_node) = self.arena.get(jsx.opening_element) {
            if let Some(opening) = self.arena.get_jsx_opening(opening_node) {
                ...
                // Check attributes
                self.check_jsx_attributes(opening.attributes, &opening_tag_clone.unwrap_or_default(), errors);
            }
        }
    ...
    fn check_jsx_attributes(&self, attrs_idx: NodeIndex, _tag_name: &str, errors: &mut Vec<JsxError>) {
