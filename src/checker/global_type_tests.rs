...
"#.to_string();
    ...
    assert!(!ts2583_errors.is_empty(), "Expected at least one TS2583 error for Promise, got: {:?}", diagnostics);

    // Verify error message mentions changing target library
    let promise_error = ts2583_errors.iter().find(|d| d.message_text.contains("Promise"));
    assert!(promise_error.is_some(), "Expected TS2583 error to mention Promise");

    let error = promise_error.unwrap();
    assert!(error.message_text.contains("target library") || error.message_text.contains("lib"),
        "Expected error message to suggest changing target library, got: {}", error.message_text);
}
...
"#.to_string();
    ...
    assert!(!ts2583_errors.is_empty(), "Expected TS2583 error for Symbol");
}
...
"#.to_string();
    ...
    assert!(!ts2318_errors.is_empty(), "Expected TS2318 error for Date, got: {:?}", diagnostics);

    // Verify error message
    let date_error = ts2318_errors.iter().find(|d| d.message_text.contains("Date"));
    assert!(date_error.is_some(), "Expected TS2318 error to mention Date");
}
...
"#.to_string();
    ...
    assert!(!ts2318_errors.is_empty(), "Expected TS2318 error for RegExp");
}
...
"#.to_string();
    ...
    assert!(ts2583_errors.len() >= 2, "Expected at least 2 TS2583 errors for Promise, got {}", ts2583_errors.len());
}
...
"#.to_string();
    ...
    assert!(!ts2583_errors.is_empty(), "Expected TS2583 error for Map type reference");
}
...
"#.to_string();
    ...
    assert!(ts2318_errors.is_empty() && ts2583_errors.is_empty(),
        "Array should not emit TS2318 or TS2583 errors, got: {:?}", diagnostics);
}
...
"#.to_string();
    ...
    assert!(ts2318_errors.is_empty() && ts2583_errors.is_empty(),
        "Object should not emit TS2318 or TS2583 errors, got: {:?}", diagnostics);
}
