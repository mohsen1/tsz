...
/// const point2d: Point2D = point3d;       // ❌ TS9001: 'z' is excess
///
/// // Explicit opt-out via type annotation
/// const wide: { x: number; y: number; z: number } = point3d;  // ✅ OK
/// const point2d2: Point2D = wide;                              // ✅ OK (no longer fresh)
/// ```
///
/// ## Implementation
///
/// Freshness is tracked by **SymbolId** (variable binding), not by TypeId.
/// This avoids the "Zombie Freshness" bug where structurally identical types
/// share freshness state due to TypeInterner deduplication.
...
pub struct SoundLawyer<'a> {
    ...
    /// Freshness tracker for sticky freshness
...
impl<'a> SoundLawyer<'a> {
    ...
    pub fn check_excess_properties(
        ...
        // Check if source is fresh (either syntactically or via sticky tracking)
        let is_fresh = source_symbol
            .map(|s| self.freshness.is_binding_fresh(s))
            .unwrap_or(false);

        if !is_fresh {
            return None; // Not fresh, no excess check needed
        }
        ...
        let source_props = match &source_key {
        ...
        // Find excess properties in source
        for s_prop in &source_props {
            let found = target_props.iter().any(|t_prop| t_prop.name == s_prop.name);
            if !found {
                // Check if target has an index signature that could accept this
                if let TypeKey::ObjectWithIndex(shape_id) = &target_key {
                    let shape = self.db.object_shape(*shape_id);
                    if shape.string_index.is_some() {
                        continue; // Index signature covers this property
                    }
                }
...
mod tests {
    ...
    fn test_sticky_freshness_basic() {
        ...
        assert!(!tracker.is_binding_fresh(sym1));

        tracker.mark_binding_fresh(sym1, TypeId::NUMBER);
        assert!(tracker.is_binding_fresh(sym1));
        assert!(!tracker.is_binding_fresh(sym2));

        tracker.consume_freshness(sym1);
        assert!(!tracker.is_binding_fresh(sym1));
    }
    ...
    fn test_sticky_freshness_transfer() {
        ...
        tracker.mark_binding_fresh(sym1, TypeId::NUMBER);
        tracker.transfer_freshness(sym1, sym2);
    ...
    #[test]
    fn test_sticky_freshness_property_access() {
        let mut tracker = StickyFreshnessTracker::new();
