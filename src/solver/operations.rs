...
impl<'a> PropertyAccessEvaluator<'a> {
    ...
    fn resolve_property_access_inner(
        ...
        match key {
            ...
            // Application: evaluate and resolve, with special handling for Promise
            TypeKey::Application(app_id) => {
                let app = self.interner.type_application(app_id);

                // Special case for Promise<T> before general evaluation
                // Check if the application is Promise<T> by looking at the base
                let is_promise_type = app.base == TypeId::PROMISE_BASE ||
                    // Also check if base is a Ref that might resolve to Promise
                    match self.interner.lookup(app.base) {
                        Some(TypeKey::Ref(_)) => {
                            // For Ref types, check if we can identify it as Promise
                            // Since we don't parse lib.d.ts, we use a heuristic:
                            // If it's a Ref with type args and the property name is a Promise method,
                            // assume it's Promise and handle it
                            Self::matches_promise_method(prop_name)
                        }
                        _ => false
                    };
                ...
                if is_promise_type {
                    ...
                    return self.resolve_promise_property(obj_type, prop_name, prop_atom);
                }

                // General Application handling for other generic types
    ...
    /// Check if a property name is a known Promise method
    fn matches_promise_method(prop_name: &str) -> bool {
        matches!(
            prop_name,
    ...
    fn resolve_promise_property(
        ...
        let type_arg = match self.interner.lookup(promise_type) {
            Some(TypeKey::Application(app_id)) => {
                let app = self.interner.type_application(app_id);
                // Accept either PROMISE_BASE (synthetic) or a Ref (from lib.d.ts)
                if app.base == TypeId::PROMISE_BASE
                    || matches!(self.interner.lookup(app.base), Some(TypeKey::Ref(_)))
                {
                    // First type argument is T
                    app.args.first().copied().unwrap_or(TypeId::ANY)
                } else {
                    // Not a Promise, fall through to PropertyNotFound
                    return PropertyAccessResult::PropertyNotFound {
                        type_id: promise_type,
                        property_name: prop_atom,
                    };
                }
            }
            _ => TypeId::ANY, // Should be Promise<T>, but fallback to ANY
        };

        let promise_of_type_arg = self
            .interner
            .application(TypeId::PROMISE_BASE, vec![type_arg]);

        match prop_name {
            // Promise static methods and instance methods
            "resolve" | "reject" => {
                // Promise.resolve<T>(value: T): Promise<T>
                // Promise.reject<T>(reason: any): Promise<T>
                self.function_result(
                    vec![self.type_param("T")],
                    vec![self.param(type_arg, false, false)],
                    promise_of_type_arg,
                )
            }
            ...
            "then" => {
                ...
                let t_param = self.type_param("TResult");
                let t_type = self.type_param_type(&t_param);
                let promise_t = self
                    .interner
                    .application(TypeId::PROMISE_BASE, vec![t_type]);
                let callback = self.function_type(
                    vec![],
                    vec![self.param(type_arg, false, false)],
                    self.interner.union2(t_type, promise_t),
                );
                self.function_result(
                    vec![t_param],
                    vec![self.param(callback, false, false)],
                    promise_t,
                )
            }
            ...
            "catch" => {
                ...
                let promise_t = self
                    .interner
                    .application(TypeId::PROMISE_BASE, vec![t_type]);
                // The callback receives 'any' as the rejection reason
                self.function_result(
                    vec![t_param],
                    vec![self.param(TypeId::ANY, false, false)],
                    promise_t,
                )
            }
            ...
            "finally" => {
                ...
                // Returns the same promise type
                self.method_result(promise_type)
            }
            ...
            "race" | "all" | "any" => {
                ...
                let param_type = if prop_name == "all" {
                    self.interner.array(type_arg)
                } else {
                    // race and any take Promise<T>[]
                    self.interner.array(promise_of_type_arg)
                };
                self.function_result(
                    vec![],
                    vec![self.param(param_type, false, true)],
                    if prop_name == "all" {
                        self.interner.array(type_arg)
                    } else {
                        type_arg
