...
impl<'a> ArrayLiteralBuilder<'a> {
    ...
    pub fn build_array_type(&self, element_types: Vec<TypeId>, contextual: Option<TypeId>) -> TypeId {
        ...
        if let Some(ctx_type) = contextual {
            if let Some(TypeKey::Array(context_elem)) = self.interner.lookup(ctx_type) {
                let all_match = element_types.iter().all(|&elem| self.is_subtype(elem, context_elem));
                if all_match {
    ...
    pub fn expand_spread(&self, spread_type: TypeId) -> Vec<TupleElement> {
        if spread_type == TypeId::ANY || spread_type == TypeId::UNKNOWN || spread_type == TypeId::ERROR {
            return vec![TupleElement { type_id: spread_type, name: None, optional: false, rest: true }];
        }
        ...
        match self.interner.lookup(ty) {
            Some(TypeKey::Tuple(tuple_id)) => {
                ...
                elems.iter().map(|e| TupleElement { type_id: e.type_id, name: None, optional: false, rest: false }).collect()
            }
            Some(TypeKey::Array(elem)) => {
                vec![TupleElement { type_id: elem, name: None, optional: false, rest: true }]
            }
            ...
            _ => vec![TupleElement { type_id: spread_type, name: None, optional: false, rest: true }],
        }
    }

    pub fn extract_iterable_element_type(&self, iterable_type: TypeId) -> TypeId {
        if iterable_type == TypeId::ANY || iterable_type == TypeId::UNKNOWN || iterable_type == TypeId::ERROR {
        ...
        match self.interner.lookup(ty) {
            ...
            Some(TypeKey::Tuple(tuple_id)) => {
                ...
                if types.is_empty() { TypeId::NEVER } else if types.len() == 1 { types[0] } else { self.interner.union(types) }
            }
            Some(TypeKey::Union(members_id)) => {
                let members = self.interner.type_list(members_id);
                let types: Vec<TypeId> = members.iter().map(|&m| self.extract_iterable_element_type(m)).collect();
    ...
    pub fn best_common_type(&self, types: &[TypeId]) -> TypeId {
        if types.is_empty() { TypeId::NEVER } else if types.len() == 1 { types[0] } else { self.interner.union(types.to_vec()) }
    }
