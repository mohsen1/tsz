...
impl<'a> ThinCheckerState<'a> {
    ...
    /// Iteratively checks the expression tree for await expressions to avoid stack overflow.
    fn check_await_expression(&mut self, expr_idx: NodeIndex) {
        // Use explicit stack to avoid stack overflow on deeply nested expressions
        let mut stack = vec![expr_idx];
        ...
        while let Some(current_idx) = stack.pop() {
            ...
            match node.kind {
                ...
                syntax_kind_ext::AWAIT_EXPRESSION => {
                    // Already checked above, but still need to check nested expression
                    if let Some(unary_expr) = self.ctx.arena.get_unary_expr_ex(node) {
                        stack.push(unary_expr.expression);
                    }
                }
