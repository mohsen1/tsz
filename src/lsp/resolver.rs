        ...
        match node.kind {
                ...
                || k == syntax_kind_ext::ARROW_FUNCTION =>
            {
                if let Some(func) = self.arena.get_function(node) {
                    ...
                    if !func.name.is_none() {
                        if let Some(res) = f(self, func.name) {
                            return Some(res);
                        }
                    }
                    ...
                    if !func.type_annotation.is_none() {
                        if let Some(res) = f(self, func.type_annotation) {
                            return Some(res);
                        }
                    }
                    if !func.body.is_none() {
                        if let Some(res) = f(self, func.body) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::METHOD_DECLARATION => {
                if let Some(method) = self.arena.get_method_decl(node) {
                    ...
                    if !method.name.is_none() {
                        if let Some(res) = f(self, method.name) {
                            return Some(res);
                        }
                    }
                    ...
                    if !method.type_annotation.is_none() {
                        if let Some(res) = f(self, method.type_annotation) {
                            return Some(res);
                        }
                    }
                    if !method.body.is_none() {
                        if let Some(res) = f(self, method.body) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::CONSTRUCTOR => {
                if let Some(ctor) = self.arena.get_constructor(node) {
                    ...
                    if !ctor.body.is_none() {
                        if let Some(res) = f(self, ctor.body) {
                            return Some(res);
                        }
                    }
                }
                ...
                || k == syntax_kind_ext::CLASS_EXPRESSION =>
            {
                if let Some(class) = self.arena.get_class(node) {
                    ...
                    if !class.name.is_none() {
                        if let Some(res) = f(self, class.name) {
                            return Some(res);
                        }
                    }
            ...
            k if k == syntax_kind_ext::VARIABLE_DECLARATION => {
                if let Some(decl) = self.arena.get_variable_declaration(node) {
                    ...
                    if !decl.type_annotation.is_none() {
                        if let Some(res) = f(self, decl.type_annotation) {
                            return Some(res);
                        }
                    }
                    if !decl.initializer.is_none() {
                        if let Some(res) = f(self, decl.initializer) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::PARAMETER => {
                if let Some(param) = self.arena.get_parameter(node) {
                    ...
                    if !param.type_annotation.is_none() {
                        if let Some(res) = f(self, param.type_annotation) {
                            return Some(res);
                        }
                    }
                    if !param.initializer.is_none() {
                        if let Some(res) = f(self, param.initializer) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::PROPERTY_DECLARATION => {
                if let Some(prop) = self.arena.get_property_decl(node) {
                    ...
                    if !prop.type_annotation.is_none() {
                        if let Some(res) = f(self, prop.type_annotation) {
                            return Some(res);
                        }
                    }
            ...
            k if k == syntax_kind_ext::DECORATOR => {
                if let Some(decorator) = self.arena.get_decorator(node) {
                    if let Some(res) = f(self, decorator.expression) {
                        return Some(res);
                    }
                }
            }
            k if k == syntax_kind_ext::GET_ACCESSOR || k == syntax_kind_ext::SET_ACCESSOR => {
                if let Some(accessor) = self.arena.get_accessor(node) {
                    ...
                    if !accessor.type_annotation.is_none() {
                        if let Some(res) = f(self, accessor.type_annotation) {
                            return Some(res);
                        }
                    }
                    if !accessor.body.is_none() {
                        if let Some(res) = f(self, accessor.body) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::INTERFACE_DECLARATION => {
                if let Some(iface) = self.arena.get_interface(node) {
                    if !iface.name.is_none() {
                        if let Some(res) = f(self, iface.name) {
                            return Some(res);
                        }
                    }
            ...
            k if k == syntax_kind_ext::TYPE_ALIAS_DECLARATION => {
                if let Some(alias) = self.arena.get_type_alias(node) {
                    if !alias.name.is_none() {
                        if let Some(res) = f(self, alias.name) {
                            return Some(res);
                        }
                    }
                    ...
                    if !alias.type_node.is_none() {
                        if let Some(res) = f(self, alias.type_node) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::ENUM_DECLARATION => {
                if let Some(enum_decl) = self.arena.get_enum(node) {
                    if !enum_decl.name.is_none() {
                        if let Some(res) = f(self, enum_decl.name) {
                            return Some(res);
                        }
                    }
            ...
            k if k == syntax_kind_ext::MODULE_DECLARATION => {
                if let Some(module) = self.arena.get_module(node) {
                    if !module.name.is_none() {
                        if let Some(res) = f(self, module.name) {
                            return Some(res);
                        }
                    }
                    if !module.body.is_none() {
                        if let Some(res) = f(self, module.body) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::IMPORT_DECLARATION => {
                if let Some(import) = self.arena.get_import_decl(node) {
                    if !import.import_clause.is_none() {
                        if let Some(res) = f(self, import.import_clause) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::EXPORT_DECLARATION => {
                if let Some(export) = self.arena.get_export_decl(node) {
                    if !export.export_clause.is_none() {
                        if let Some(res) = f(self, export.export_clause) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::IF_STATEMENT => {
                if let Some(stmt) = self.arena.get_if_statement(node) {
                    ...
                    if !stmt.else_statement.is_none() {
                        if let Some(res) = f(self, stmt.else_statement) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::RETURN_STATEMENT => {
                if let Some(ret) = self.arena.get_return_statement(node) {
                    if !ret.expression.is_none() {
                        if let Some(res) = f(self, ret.expression) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::FOR_STATEMENT => {
                if let Some(loop_data) = self.arena.get_loop(node) {
                    if !loop_data.initializer.is_none() {
                        if let Some(res) = f(self, loop_data.initializer) {
                            return Some(res);
                        }
                    }
                    if !loop_data.condition.is_none() {
                        if let Some(res) = f(self, loop_data.condition) {
                            return Some(res);
                        }
                    }
                    if !loop_data.incrementor.is_none() {
                        if let Some(res) = f(self, loop_data.incrementor) {
                            return Some(res);
                        }
                    }
            ...
            k if k == syntax_kind_ext::PARENTHESIZED_EXPRESSION => {
                if let Some(paren) = self.arena.get_parenthesized(node) {
                    if let Some(res) = f(self, paren.expression) {
                        return Some(res);
                    }
                }
            }
                ...
                if node.has_data() {
                        ...
                        self.arena.type_assertions.get(node.data_index as usize)
                    {
                        ...
                        if !assertion.type_node.is_none() {
                            if let Some(res) = f(self, assertion.type_node) {
                                return Some(res);
                            }
                        }
                    }
            ...
            k if k == syntax_kind_ext::BINDING_ELEMENT => {
                if let Some(binding) = self.arena.get_binding_element(node) {
                    if !binding.property_name.is_none() {
                        if let Some(prop_node) = self.arena.get(binding.property_name) {
                            if prop_node.kind == syntax_kind_ext::COMPUTED_PROPERTY_NAME {
                                if let Some(res) = f(self, binding.property_name) {
                                    return Some(res);
                                }
                            }
                        }
                    ...
                    if !binding.initializer.is_none() {
                        if let Some(res) = f(self, binding.initializer) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::COMPUTED_PROPERTY_NAME => {
                if let Some(computed) = self.arena.get_computed_property(node) {
                    if let Some(res) = f(self, computed.expression) {
                        return Some(res);
                    }
                }
            }
            ...
            k if k == syntax_kind_ext::JSX_CLOSING_ELEMENT => {
                if let Some(closing) = self.arena.get_jsx_closing(node) {
                    if let Some(res) = f(self, closing.tag_name) {
                        return Some(res);
                    }
                }
            }
            ...
            k if k == syntax_kind_ext::JSX_ATTRIBUTE => {
                if let Some(attr) = self.arena.get_jsx_attribute(node) {
                    ...
                    if !attr.initializer.is_none() {
                        if let Some(res) = f(self, attr.initializer) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::JSX_SPREAD_ATTRIBUTE => {
                if let Some(spread) = self.arena.get_jsx_spread_attribute(node) {
                    if let Some(res) = f(self, spread.expression) {
                        return Some(res);
                    }
                }
            }
                ...
                || k == syntax_kind_ext::POSTFIX_UNARY_EXPRESSION =>
            {
                if let Some(unary) = self.arena.get_unary_expr(node) {
                    if let Some(res) = f(self, unary.operand) {
                        return Some(res);
                    }
                }
            }
                ...
                || k == syntax_kind_ext::NON_NULL_EXPRESSION =>
            {
                if node.has_data() {
                    if let Some(unary) = self.arena.unary_exprs_ex.get(node.data_index as usize) {
                        if let Some(res) = f(self, unary.expression) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::SHORTHAND_PROPERTY_ASSIGNMENT => {
                if let Some(prop) = self.arena.get_shorthand_property(node) {
                    ...
                    if !prop.object_assignment_initializer.is_none() {
                        if let Some(res) = f(self, prop.object_assignment_initializer) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::TYPE_OPERATOR => {
                if let Some(op) = self.arena.get_type_operator(node) {
                    if let Some(res) = f(self, op.type_node) {
                        return Some(res);
                    }
                }
            }
            k if k == syntax_kind_ext::TYPE_PREDICATE => {
                if let Some(pred) = self.arena.get_type_predicate(node) {
                    ...
                    if !pred.type_node.is_none() {
                        if let Some(res) = f(self, pred.type_node) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::TYPE_PARAMETER => {
                if let Some(param) = self.arena.get_type_parameter(node) {
                    ...
                    if !param.constraint.is_none() {
                        if let Some(res) = f(self, param.constraint) {
                            return Some(res);
                        }
                    }
                    if !param.default.is_none() {
                        if let Some(res) = f(self, param.default) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::FUNCTION_TYPE || k == syntax_kind_ext::CONSTRUCTOR_TYPE => {
                if let Some(func_type) = self.arena.get_function_type(node) {
                    ...
                    if !func_type.type_annotation.is_none() {
                        if let Some(res) = f(self, func_type.type_annotation) {
                            return Some(res);
                        }
                    }
                }
                ...
                || k == syntax_kind_ext::CONSTRUCT_SIGNATURE =>
            {
                if let Some(sig) = self.arena.get_signature(node) {
                    if !sig.name.is_none() {
                        if let Some(res) = f(self, sig.name) {
                            return Some(res);
                        }
                    }
                    ...
                    if !sig.type_annotation.is_none() {
                        if let Some(res) = f(self, sig.type_annotation) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::ARRAY_TYPE => {
                if let Some(array) = self.arena.get_array_type(node) {
                    if let Some(res) = f(self, array.element_type) {
                        return Some(res);
                    }
                }
            }
                ...
                || k == syntax_kind_ext::REST_TYPE =>
            {
                if let Some(wrapped) = self.arena.get_wrapped_type(node) {
                    if let Some(res) = f(self, wrapped.type_node) {
                        return Some(res);
                    }
                }
            }
            k if k == syntax_kind_ext::INFER_TYPE => {
                if let Some(infer) = self.arena.get_infer_type(node) {
                    if let Some(res) = f(self, infer.type_parameter) {
                        return Some(res);
                    }
                }
            }
            ...
            k if k == syntax_kind_ext::MAPPED_TYPE => {
                if let Some(mapped) = self.arena.get_mapped_type(node) {
                    ...
                    if !mapped.name_type.is_none() {
                        if let Some(res) = f(self, mapped.name_type) {
                            return Some(res);
                        }
                    }
                    if !mapped.type_node.is_none() {
                        if let Some(res) = f(self, mapped.type_node) {
                            return Some(res);
                        }
                    }
            ...
            k if k == syntax_kind_ext::LITERAL_TYPE => {
                if let Some(lit) = self.arena.get_literal_type(node) {
                    if let Some(res) = f(self, lit.literal) {
                        return Some(res);
                    }
                }
            }
            ...
            k if k == syntax_kind_ext::TRY_STATEMENT => {
                if let Some(try_stmt) = self.arena.get_try(node) {
                    ...
                    if !try_stmt.catch_clause.is_none() {
                        if let Some(res) = f(self, try_stmt.catch_clause) {
                            return Some(res);
                        }
                    }
                    if !try_stmt.finally_block.is_none() {
                        if let Some(res) = f(self, try_stmt.finally_block) {
                            return Some(res);
                        }
                    }
                }
            ...
            k if k == syntax_kind_ext::CATCH_CLAUSE => {
                if let Some(catch) = self.arena.get_catch_clause(node) {
                    if !catch.variable_declaration.is_none() {
                        if let Some(res) = f(self, catch.variable_declaration) {
                            return Some(res);
                        }
                    }
            ...
            k if k == syntax_kind_ext::EXPORT_ASSIGNMENT => {
                if let Some(assign) = self.arena.get_export_assignment(node) {
                    if let Some(res) = f(self, assign.expression) {
                        return Some(res);
                    }
                }
            }
            ...
            k if k == syntax_kind_ext::CASE_CLAUSE || k == syntax_kind_ext::DEFAULT_CLAUSE => {
                if let Some(case) = self.arena.get_case_clause(node) {
                    if !case.expression.is_none() {
                        if let Some(res) = f(self, case.expression) {
                            return Some(res);
                        }
                    }
                            ...
                            if list_node.kind == syntax_kind_ext::VARIABLE_DECLARATION_LIST {
                                ...
                                w.for_each_child(list_idx, |w2, decl_idx| {
                                    ...
                                    if let Some(decl_node) = w2.arena.get(decl_idx) {
                                        if let Some(decl) =
                                            w2.arena.get_variable_declaration(decl_node)
                                        {
                                            if is_var {
                ...
                // For ExportDeclaration, unwrap to find the inner declaration
                    ...
                    if let Some(export) = walker.arena.get_export_decl(node) {
                        if !export.export_clause.is_none() {
                            ...
                            if let Some(_export_clause_node) =
                                walker.arena.get(export.export_clause)
                            {
