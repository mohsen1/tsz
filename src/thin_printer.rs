//! ThinPrinter - A clean, safe AST-to-JavaScript printer
//!
...
//! - **Transformation**: TypeScript features → ES5/ES6 (thin_emitter internals)
...
//!
//! ```ignore
//! use tsz::thin_parser::ThinParserState;
//! use tsz::thin_printer::{print_to_string, PrintOptions};
//!
...
/// For advanced options, use `ThinPrinter` directly with `PrinterOptions`.
...
pub fn print_to_string(arena: &ThinNodeArena, root: NodeIndex, options: PrintOptions) -> String {
...
impl<'a> Printer<'a> {
    ...
    pub fn new(arena: &'a ThinNodeArena, options: PrintOptions) -> Self {
        let printer_opts = options.to_printer_options();
        let inner = ThinPrinter::with_options(arena, printer_opts);
    ...
    pub fn with_transforms(
        ...
        let printer_opts = options.to_printer_options();
        let inner = ThinPrinter::with_transforms_and_options(arena, transforms, printer_opts);
